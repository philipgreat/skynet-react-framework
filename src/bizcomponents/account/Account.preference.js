...
import styles from './Account.preference.less'
...
const internalImageListOf = (account) =>{
...
const internalSettingListOf = (account) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountService[candidateServiceName] 
...
const executeTrans = (account,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_account/doJob",payload: payload})
...
const buildTransferModal = (account,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(account,targetComponent)}
...
const internalRenderExtraHeader = (account) =>{
	return null
}
const internalRenderExtraFooter = (account) =>{
	return null
}
