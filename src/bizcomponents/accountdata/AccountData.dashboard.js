...
import styles from './AccountData.dashboard.less'
...
const internalImageListOf = (accountData) =>{
...
const internalSettingListOf = (accountData) =>{
...
const internalLargeTextOf = (accountData) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountDataService[candidateServiceName] 
...
const executeTrans = (accountData,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountData/doJob",payload: payload})
...
const buildTransferModal = (accountData,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountData,targetComponent)}
...
const internalRenderExtraHeader = (accountData) =>{
	return null
}
const internalRenderExtraFooter = (accountData) =>{
	return null
}
...
const internalSummaryOf = (accountData,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{accountData.id}</Description> 
<Description term="摘要">{accountData.summary}</Description> 
	
        {buildTransferModal(accountData,targetComponent)}
      </DescriptionList>
	)
...
class AccountDataDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, platformAccountAccessiable, fundationAccountAccessiable, storeAccountAccessiable, transactionTypeAccessiable, platformAccountCount, fundationAccountCount, storeAccountCount, transactionTypeCount } = this.props.accountData
...
}))(Form.create()(AccountDataDashboard))
