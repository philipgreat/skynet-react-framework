...
import styles from './AccountData.search.less'
...
class AccountDataSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const accountDataIds = selectedRows.map((item) => { return item.id })
    console.log('accountDataIds', accountDataIds)
    const parameters = { accountDataIds }
    dispatch({
      type: `${owner.type}/removeAccountDataList`,
      payload: { id: owner.id, type: 'accountData', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'accountData' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'accountData', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'帐户数据列表' } })
  ...
  render() {
    ...
    const {AccountDataTable} = GlobalComponents;
    const {AccountDataModalTable} = GlobalComponents;
    const {AccountDataSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AccountDataModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 帐户数据列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AccountDataSearchForm {...this.props} />
            </div>
...
export default Form.create()(AccountDataSearch)
