...
const internalImageListOf = (actionToken) =>{
...
const internalSettingListOf = (actionToken) =>{
...
const internalLargeTextOf = (actionToken) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ActionTokenService[candidateServiceName] 
...
const executeTrans = (actionToken,targetComponent) =>{
...
const buildTransferModal = (actionToken,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(actionToken,targetComponent)}
...
const internalRenderExtraHeader = (actionToken) =>{
	return null
}
const internalRenderExtraFooter = (actionToken) =>{
	return null
}
...
const internalSummaryOf = (actionToken,targetComponent) =>{
...
<Description term="结束日期">{ moment(actionToken.endDate).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(actionToken,targetComponent)}
      </DescriptionList>
	)
...
}))(Form.create()(ActionTokenDashboard))
