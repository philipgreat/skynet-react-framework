...
import styles from './ApplicationStatus.dashboard.less'
...
const internalImageListOf = (applicationStatus) =>{
...
const internalSettingListOf = (applicationStatus) =>{
...
const internalLargeTextOf = (applicationStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ApplicationStatusService[candidateServiceName] 
...
const executeTrans = (applicationStatus,targetComponent) =>{
...
const buildTransferModal = (applicationStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(applicationStatus,targetComponent)}
...
const internalRenderExtraHeader = (applicationStatus) =>{
	return null
}
const internalRenderExtraFooter = (applicationStatus) =>{
	return null
}
...
const internalSummaryOf = (applicationStatus,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{applicationStatus.id}</Description> 
<Description term="名称">{applicationStatus.name}</Description> 
<Description term="代码">{applicationStatus.code}</Description> 
	
        {buildTransferModal(applicationStatus,targetComponent)}
      </DescriptionList>
	)
...
class ApplicationStatusDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, bookCopySharingApplicationAccessiable, bookCopySharingApplicationCount } = this.props.applicationStatus
...
export default connect(state => ({
  applicationStatus: state._applicationStatus,
}))(Form.create()(ApplicationStatusDashboard))
