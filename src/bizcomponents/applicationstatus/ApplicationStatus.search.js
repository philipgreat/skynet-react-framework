...
import styles from './ApplicationStatus.search.less'
...
class ApplicationStatusSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const applicationStatusIds = selectedRows.map((item) => { return item.id })
    console.log('applicationStatusIds', applicationStatusIds)
    const parameters = { applicationStatusIds }
    dispatch({
      type: `${owner.type}/removeApplicationStatusList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'applicationStatus' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'applicationStatus', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'应用程序状态列表' } })
  ...
  render() {
    ...
    const {ApplicationStatusTable} = GlobalComponents;
    const {ApplicationStatusModalTable} = GlobalComponents;
    const {ApplicationStatusSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ApplicationStatusModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 应用程序状态列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ApplicationStatusSearchForm {...this.props} />
            </div>
...
export default Form.create()(ApplicationStatusSearch)
