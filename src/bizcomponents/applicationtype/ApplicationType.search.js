...
import styles from './ApplicationType.search.less'
...
class ApplicationTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const applicationTypeIds = selectedRows.map((item) => { return item.id })
    console.log('applicationTypeIds', applicationTypeIds)
    const parameters = { applicationTypeIds }
    dispatch({
      type: `${owner.type}/removeApplicationTypeList`,
      payload: { id: owner.id, type: 'applicationType', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'applicationType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'applicationType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'申请类型列表' } })
  ...
  render() {
    ...
    const {ApplicationTypeTable} = GlobalComponents;
    const {ApplicationTypeConfirmationTable} = GlobalComponents;
    const {ApplicationTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ApplicationTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 申请类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ApplicationTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(ApplicationTypeSearch)
