...
import styles from './AvailableHandOverItem.preference.less'
...
const internalImageListOf = (availableHandOverItem) =>{
...
const internalSettingListOf = (availableHandOverItem) =>{
...
const internalLargeTextOf = (availableHandOverItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableHandOverItemService[candidateServiceName] 
...
const executeTrans = (availableHandOverItem,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=availableHandOverItem.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_availableHandOverItem/doJob",payload: payload})
...
const buildTransferModal = (availableHandOverItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableHandOverItem,targetComponent)}
...
const internalRenderExtraHeader = (availableHandOverItem) =>{
	return null
}
const internalRenderExtraFooter = (availableHandOverItem) =>{
	return null
}
...
const internalSummaryOf = (availableHandOverItem,targetComponent) =>{
...
<Description term="产品名称">{availableHandOverItem.product==null?"未分配":availableHandOverItem.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品名称","availableProduct","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",availableHandOverItem.product?availableHandOverItem.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(availableHandOverItem,targetComponent)}
      </DescriptionList>
	)
...
class AvailableHandOverItemPreference extends Component {
...
}))(Form.create()(AvailableHandOverItemPreference))
