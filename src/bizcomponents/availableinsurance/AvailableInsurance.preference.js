...
import styles from './AvailableInsurance.preference.less'
...
const internalImageListOf = (availableInsurance) =>{
...
const internalSettingListOf = (availableInsurance) =>{
...
const internalLargeTextOf = (availableInsurance) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableInsuranceService[candidateServiceName] 
...
const executeTrans = (availableInsurance,targetComponent) =>{
...
const buildTransferModal = (availableInsurance,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableInsurance,targetComponent)}
...
const internalRenderExtraHeader = (availableInsurance) =>{
	return null
}
const internalRenderExtraFooter = (availableInsurance) =>{
	return null
}
...
const internalSummaryOf = (availableInsurance,targetComponent) =>{
...
<Description term="产品名称">{availableInsurance.product==null?"未分配":availableInsurance.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品名称","availableProduct","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",availableInsurance.product?availableInsurance.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(availableInsurance,targetComponent)}
      </DescriptionList>
	)
...
class AvailableInsurancePreference extends Component {
...
}))(Form.create()(AvailableInsurancePreference))
