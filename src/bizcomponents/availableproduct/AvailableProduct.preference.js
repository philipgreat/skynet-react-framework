...
import styles from './AvailableProduct.preference.less'
...
const internalImageListOf = (availableProduct) =>{
...
const internalSettingListOf = (availableProduct) =>{
...
const internalLargeTextOf = (availableProduct) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableProductService[candidateServiceName] 
...
const executeTrans = (availableProduct,targetComponent) =>{
...
const buildTransferModal = (availableProduct,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableProduct,targetComponent)}
...
const internalRenderExtraHeader = (availableProduct) =>{
	return null
}
const internalRenderExtraFooter = (availableProduct) =>{
	return null
}
...
const internalSummaryOf = (availableProduct,targetComponent) =>{
...
<Description term="服务描述">{availableProduct.serviceDescription}</Description> 
        ...
        {buildTransferModal(availableProduct,targetComponent)}
      </DescriptionList>
	)
...
class AvailableProductPreference extends Component {
...
}))(Form.create()(AvailableProductPreference))
