...
import styles from './AvailableRatingItem.preference.less'
...
const internalImageListOf = (availableRatingItem) =>{
...
const internalSettingListOf = (availableRatingItem) =>{
...
const internalLargeTextOf = (availableRatingItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableRatingItemService[candidateServiceName] 
...
const executeTrans = (availableRatingItem,targetComponent) =>{
...
const buildTransferModal = (availableRatingItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableRatingItem,targetComponent)}
...
const internalRenderExtraHeader = (availableRatingItem) =>{
	return null
}
const internalRenderExtraFooter = (availableRatingItem) =>{
	return null
}
...
const internalSummaryOf = (availableRatingItem,targetComponent) =>{
...
<Description term="评分项">{availableRatingItem.ratingName}</Description> 
        ...
        {buildTransferModal(availableRatingItem,targetComponent)}
      </DescriptionList>
	)
...
class AvailableRatingItemPreference extends Component {
...
}))(Form.create()(AvailableRatingItemPreference))
