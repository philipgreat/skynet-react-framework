...
import styles from './AvailableReviewItem.preference.less'
...
const internalImageListOf = (availableReviewItem) =>{
...
const internalSettingListOf = (availableReviewItem) =>{
...
const internalLargeTextOf = (availableReviewItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableReviewItemService[candidateServiceName] 
...
const executeTrans = (availableReviewItem,targetComponent) =>{
...
const buildTransferModal = (availableReviewItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableReviewItem,targetComponent)}
...
const internalRenderExtraHeader = (availableReviewItem) =>{
	return null
}
const internalRenderExtraFooter = (availableReviewItem) =>{
	return null
}
...
const internalSummaryOf = (availableReviewItem,targetComponent) =>{
...
<Description term="评论内容">{availableReviewItem.reviewName}</Description> 
        ...
        {buildTransferModal(availableReviewItem,targetComponent)}
      </DescriptionList>
	)
...
class AvailableReviewItemPreference extends Component {
...
}))(Form.create()(AvailableReviewItemPreference))
