...
import styles from './AvailableService.preference.less'
...
const internalImageListOf = (availableService) =>{
...
const internalSettingListOf = (availableService) =>{
...
const internalLargeTextOf = (availableService) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableServiceService[candidateServiceName] 
...
const executeTrans = (availableService,targetComponent) =>{
...
const buildTransferModal = (availableService,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableService,targetComponent)}
...
const internalRenderExtraHeader = (availableService) =>{
	return null
}
const internalRenderExtraFooter = (availableService) =>{
	return null
}
...
const internalSummaryOf = (availableService,targetComponent) =>{
...
<Description term="产品类型">{availableService.availableProduct==null?"未分配":availableService.availableProduct.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品类型","availableProduct","requestCandidateAvailableProduct",
	      "transferToAnotherAvailableProduct","anotherAvailableProductId",availableService.availableProduct?availableService.availableProduct.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(availableService,targetComponent)}
      </DescriptionList>
	)
...
class AvailableServicePreference extends Component {
...
}))(Form.create()(AvailableServicePreference))
