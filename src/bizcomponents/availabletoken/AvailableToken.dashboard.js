...
const internalImageListOf = (availableToken) =>{
...
const internalSettingListOf = (availableToken) =>{
...
const internalLargeTextOf = (availableToken) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableTokenService[candidateServiceName] 
...
const executeTrans = (availableToken,targetComponent) =>{
...
const buildTransferModal = (availableToken,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableToken,targetComponent)}
...
const internalRenderExtraHeader = (availableToken) =>{
	return null
}
const internalRenderExtraFooter = (availableToken) =>{
	return null
}
...
const internalSummaryOf = (availableToken,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{availableToken.id}</Description> 
<Description term="名称">{availableToken.name}</Description> 
<Description term="标记代码">{availableToken.tokenCode}</Description> 
	
        {buildTransferModal(availableToken,targetComponent)}
      </DescriptionList>
	)
...
class AvailableTokenDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"可用的权益",cardsFor: "availableToken",cardsSource: this.props.availableToken,
  		subItems: [
...
}))(Form.create()(AvailableTokenDashboard))
