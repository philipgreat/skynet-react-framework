...
import styles from './AvailableVehicleType.preference.less'
...
const internalImageListOf = (availableVehicleType) =>{
...
const internalSettingListOf = (availableVehicleType) =>{

	const optionList = [ 
	  {"title":'可下单',"value":availableVehicleType.canPlaceOrder,"parameterName":"canPlaceOrder"},
  {"title":'可6年免检',"value":availableVehicleType.canDoExempt,"parameterName":"canDoExempt"},
]
...
const internalLargeTextOf = (availableVehicleType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableVehicleTypeService[candidateServiceName] 
...
const executeTrans = (availableVehicleType,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=availableVehicleType.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_availableVehicleType/doJob",payload: payload})
...
const buildTransferModal = (availableVehicleType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableVehicleType,targetComponent)}
...
const internalRenderExtraHeader = (availableVehicleType) =>{
	return null
}
const internalRenderExtraFooter = (availableVehicleType) =>{
	return null
}
...
const internalSummaryOf = (availableVehicleType,targetComponent) =>{
...
<Description term="车辆类型别名">{availableVehicleType.vehicleTypeAlias}</Description> 
        ...
        {buildTransferModal(availableVehicleType,targetComponent)}
      </DescriptionList>
	)
...
class AvailableVehicleTypePreference extends Component {
...
}))(Form.create()(AvailableVehicleTypePreference))
