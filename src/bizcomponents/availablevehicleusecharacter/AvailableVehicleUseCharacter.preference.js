...
import styles from './AvailableVehicleUseCharacter.preference.less'
...
const internalImageListOf = (availableVehicleUseCharacter) =>{
...
const internalSettingListOf = (availableVehicleUseCharacter) =>{

	const optionList = [ 
	  {"title":'可6年免检',"value":availableVehicleUseCharacter.canDoExempt,"parameterName":"canDoExempt"},
  {"title":'商用车辆',"value":availableVehicleUseCharacter.commercialVehicle,"parameterName":"commercialVehicle"},
]
...
const internalLargeTextOf = (availableVehicleUseCharacter) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AvailableVehicleUseCharacterService[candidateServiceName] 
...
const executeTrans = (availableVehicleUseCharacter,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=availableVehicleUseCharacter.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_availableVehicleUseCharacter/doJob",payload: payload})
...
const buildTransferModal = (availableVehicleUseCharacter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(availableVehicleUseCharacter,targetComponent)}
...
const internalRenderExtraHeader = (availableVehicleUseCharacter) =>{
	return null
}
const internalRenderExtraFooter = (availableVehicleUseCharacter) =>{
	return null
}
...
const internalSummaryOf = (availableVehicleUseCharacter,targetComponent) =>{
...
<Description term="别名">{availableVehicleUseCharacter.aliasName}</Description> 
        ...
        {buildTransferModal(availableVehicleUseCharacter,targetComponent)}
      </DescriptionList>
	)
...
class AvailableVehicleUseCharacterPreference extends Component {
...
}))(Form.create()(AvailableVehicleUseCharacterPreference))
