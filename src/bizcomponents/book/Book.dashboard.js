...
const internalImageListOf = (book) =>{

  const imageList = [
	   {"title":'封面',"imageLocation":book.bookCover},
]
...
const internalSettingListOf = (book) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookService[candidateServiceName] 
...
const executeTrans = (book,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_book/doJob",payload: payload})
...
const buildTransferModal = (book,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(book,targetComponent)}
...
const internalRenderExtraHeader = (book) =>{
	return null
}
const internalRenderExtraFooter = (book) =>{
	return null
}
...
const internalSummaryOf = (book,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{book.id}</Description> 
<Description term="书名">{book.bookName}</Description> 
<Description term="作者">{book.bookAuthor}</Description> 
<Description term="出版社">{book.bookPublisher}</Description> 
<Description term="出版日期">{ moment(book.bookPubdate).format('YYYY-MM-DD')}</Description> 
<Description term="定价">{book.listPrice}</Description> 
<Description term="ISBN13">{book.bookIsbn13}</Description> 
<Description term="ISBN10">{book.bookIsbn10}</Description> 
<Description term="图书推荐">{book.bookRecommend==null?"未分配":book.bookRecommend.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书推荐","bookRecommend","requestCandidateBookRecommend",
	      "transferToAnotherBookRecommend","anotherBookRecommendId",book.bookRecommend?book.bookRecommend.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="图书天地">{book.bookPlaza==null?"未分配":book.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",book.bookPlaza?book.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="本书总结">{book.bookSummary}</Description> 
	
        {buildTransferModal(book,targetComponent)}
      </DescriptionList>
	)
