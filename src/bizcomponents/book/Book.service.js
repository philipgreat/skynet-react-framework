...
const addBookCopy = (targetObjectId, parameters) => {
  const url = `${PREFIX}bookManager/addBookCopy/bookId/bookCopyVendorId/bookCopySharingType/locationStoreId/evaluationPrice/bookCopyStatusId/wxaId/tokensExpr/`
...
const addBookReview = (targetObjectId, parameters) => {
  const url = `${PREFIX}bookManager/addBookReview/bookId/bookCopyId/reviewerId/reviewContent/bookPlazaId/tokensExpr/`
  const bookId = targetObjectId
  const requestParameters = { ...parameters, bookId, tokensExpr: 'none' }
  return postForm({ url,requestParameters})
}
