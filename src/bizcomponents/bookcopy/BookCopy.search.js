...
class BookCopySearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书籍副本列表' } })
  ...
  render() {
    ...
    const {BookCopyModalTable} = GlobalComponents;
...
<Menu.Item key="bookInfo">关联书籍信息</Menu.Item>
<Menu.Item key="bookCopyVendor">关联书籍副本共享人</Menu.Item>
<Menu.Item key="locationStore">关联所在网点</Menu.Item>
<Menu.Item key="bookCopyStatus">关联书籍副本状态</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopyModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书籍副本列表`}>
        <Card bordered={false}>
