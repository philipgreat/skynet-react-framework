...
import styles from './BookCopyOperateType.dashboard.less'
...
const internalImageListOf = (bookCopyOperateType) =>{
...
const internalSettingListOf = (bookCopyOperateType) =>{
...
const internalLargeTextOf = (bookCopyOperateType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {BookCopyOperateTypeService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = BookCopyOperateTypeService[candidateServiceName] 
...
const executeTrans = (bookCopyOperateType,targetComponent) =>{
...
const buildTransferModal = (bookCopyOperateType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookCopyOperateType,targetComponent)}
...
const internalRenderExtraHeader = (bookCopyOperateType) =>{
	return null
}
const internalRenderExtraFooter = (bookCopyOperateType) =>{
	return null
}
...
const internalSummaryOf = (bookCopyOperateType,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookCopyOperateType.id}</Description> 
<Description term="名称">{bookCopyOperateType.name}</Description> 
<Description term="代码">{bookCopyOperateType.code}</Description> 
	
        {buildTransferModal(bookCopyOperateType,targetComponent)}
      </DescriptionList>
	)
...
class BookCopyOperateTypeDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, bookCopyOperationRecordCount } = this.props.bookCopyOperateType
    const cardsData = {cardsName:"书籍副本操作类型",cardsFor: "bookCopyOperateType",cardsSource: this.props.bookCopyOperateType,
  		subItems: [
{name: 'bookCopyOperationRecordList', displayName:'书籍副本操作记录',type:'bookCopyOperationRecord',count:bookCopyOperationRecordCount},
...
}))(Form.create()(BookCopyOperateTypeDashboard))
