...
import styles from './BookCopyOperationRecord.preference.less'
...
const internalImageListOf = (bookCopyOperationRecord) =>{
...
const internalSettingListOf = (bookCopyOperationRecord) =>{
...
const internalLargeTextOf = (bookCopyOperationRecord) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookCopyOperationRecordService[candidateServiceName] 
...
const executeTrans = (bookCopyOperationRecord,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=bookCopyOperationRecord.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_bookCopyOperationRecord/doJob",payload: payload})
...
const buildTransferModal = (bookCopyOperationRecord,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookCopyOperationRecord,targetComponent)}
...
const internalRenderExtraHeader = (bookCopyOperationRecord) =>{
	return null
}
const internalRenderExtraFooter = (bookCopyOperationRecord) =>{
	return null
}
...
const internalSummaryOf = (bookCopyOperationRecord,targetComponent) =>{
...
<Description term="执行网点">{bookCopyOperationRecord.operateStore==null?"未分配":bookCopyOperationRecord.operateStore.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"执行网点","store","requestCandidateOperateStore",
	      "transferToAnotherOperateStore","anotherOperateStoreId",bookCopyOperationRecord.operateStore?bookCopyOperationRecord.operateStore.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="执行日期">{ moment(bookCopyOperationRecord.operationDatetime).format('YYYY-MM-DD')}</Description> 
<Description term="执行员工">{bookCopyOperationRecord.operationEmployee==null?"未分配":bookCopyOperationRecord.operationEmployee.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"执行员工","employee","requestCandidateOperationEmployee",
	      "transferToAnotherOperationEmployee","anotherOperationEmployeeId",bookCopyOperationRecord.operationEmployee?bookCopyOperationRecord.operationEmployee.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookCopyOperationRecord,targetComponent)}
      </DescriptionList>
	)
...
class BookCopyOperationRecordPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"书籍副本操作记录",cardsFor: "bookCopyOperationRecord",cardsSource: this.props.bookCopyOperationRecord,
  		subItems: [
...
}))(Form.create()(BookCopyOperationRecordPreference))
