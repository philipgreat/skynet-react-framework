...
import styles from './BookCopyStatus.preference.less'
...
const internalImageListOf = (bookCopyStatus) =>{
...
const internalSettingListOf = (bookCopyStatus) =>{

	const optionList = [ 
	  {"title":'是否可借',"value":bookCopyStatus.canBorrow,"parameterName":"canBorrow"},
  {"title":'是否可售',"value":bookCopyStatus.canSell,"parameterName":"canSell"},
  {"title":'是否检查库存',"value":bookCopyStatus.needCheckStock,"parameterName":"needCheckStock"},
  {"title":'检查是是否提醒',"value":bookCopyStatus.alarmWhenCheck,"parameterName":"alarmWhenCheck"},
]
...
const internalLargeTextOf = (bookCopyStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookCopyStatusService[candidateServiceName] 
...
const executeTrans = (bookCopyStatus,targetComponent) =>{
...
const buildTransferModal = (bookCopyStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookCopyStatus,targetComponent)}
...
const internalRenderExtraHeader = (bookCopyStatus) =>{
	return null
}
const internalRenderExtraFooter = (bookCopyStatus) =>{
	return null
}
...
const internalSummaryOf = (bookCopyStatus,targetComponent) =>{
...
<Description term="图书天地">{bookCopyStatus.bookPlaza==null?"未分配":bookCopyStatus.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",bookCopyStatus.bookPlaza?bookCopyStatus.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookCopyStatus,targetComponent)}
      </DescriptionList>
	)
...
class BookCopyStatusPreference extends Component {
...
}))(Form.create()(BookCopyStatusPreference))
