...
import styles from './BookCopyTransfer.preference.less'
...
const internalImageListOf = (bookCopyTransfer) =>{
...
const internalSettingListOf = (bookCopyTransfer) =>{
...
const internalLargeTextOf = (bookCopyTransfer) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookCopyTransferService[candidateServiceName] 
...
const executeTrans = (bookCopyTransfer,targetComponent) =>{
...
const buildTransferModal = (bookCopyTransfer,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookCopyTransfer,targetComponent)}
...
const internalRenderExtraHeader = (bookCopyTransfer) =>{
	return null
}
const internalRenderExtraFooter = (bookCopyTransfer) =>{
	return null
}
...
const internalSummaryOf = (bookCopyTransfer,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookCopyTransfer.id}</Description> 
<Description term="书名">{bookCopyTransfer.bookName}</Description> 
<Description term="书籍副本">{bookCopyTransfer.bookCopy==null?"未分配":bookCopyTransfer.bookCopy.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"书籍副本","bookCopy","requestCandidateBookCopy",
	      "transferToAnotherBookCopy","anotherBookCopyId",bookCopyTransfer.bookCopy?bookCopyTransfer.bookCopy.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="原网点">{bookCopyTransfer.originalStore==null?"未分配":bookCopyTransfer.originalStore.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"原网点","store","requestCandidateOriginalStore",
	      "transferToAnotherOriginalStore","anotherOriginalStoreId",bookCopyTransfer.originalStore?bookCopyTransfer.originalStore.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="新网点">{bookCopyTransfer.newStore==null?"未分配":bookCopyTransfer.newStore.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"新网点","store","requestCandidateNewStore",
...
<Description term="操作员工">{bookCopyTransfer.responsibleEmployee==null?"未分配":bookCopyTransfer.responsibleEmployee.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"操作员工","employee","requestCandidateResponsibleEmployee",
	      "transferToAnotherResponsibleEmployee","anotherResponsibleEmployeeId",bookCopyTransfer.responsibleEmployee?bookCopyTransfer.responsibleEmployee.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="最后更新时间">{ moment(bookCopyTransfer.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(bookCopyTransfer,targetComponent)}
      </DescriptionList>
	)
...
class BookCopyTransferPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.bookCopyTransfer
    const cardsData = {cardsName:"图书副本迁移记录",cardsFor: "bookCopyTransfer",cardsSource: this.props.bookCopyTransfer,
  		subItems: [
...
export default connect(state => ({
  bookCopyTransfer: state._bookCopyTransfer,
}))(Form.create()(BookCopyTransferPreference))
