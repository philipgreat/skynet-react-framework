...
class BookCopyTransferSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookCopyTransferIds = selectedRows.map((item) => { return item.id })
    console.log('bookCopyTransferIds', bookCopyTransferIds)
    const parameters = { bookCopyTransferIds }
    dispatch({
      type: `${owner.type}/removeBookCopyTransferList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyTransfer' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyTransfer', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书副本迁移记录列表' } })
  ...
  render() {
    ...
    const {BookCopyTransferTable} = GlobalComponents;
    const {BookCopyTransferModalTable} = GlobalComponents;
    const {BookCopyTransferSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopyTransferModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书副本迁移记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookCopyTransferSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookCopyTransferSearch)
