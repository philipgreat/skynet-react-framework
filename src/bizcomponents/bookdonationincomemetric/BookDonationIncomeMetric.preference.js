...
import styles from './BookDonationIncomeMetric.preference.less'
...
const internalImageListOf = (bookDonationIncomeMetric) =>{
...
const internalSettingListOf = (bookDonationIncomeMetric) =>{
...
const internalLargeTextOf = (bookDonationIncomeMetric) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookDonationIncomeMetricService[candidateServiceName] 
...
const executeTrans = (bookDonationIncomeMetric,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=bookDonationIncomeMetric.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_bookDonationIncomeMetric/doJob",payload: payload})
...
const buildTransferModal = (bookDonationIncomeMetric,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookDonationIncomeMetric,targetComponent)}
...
const internalRenderExtraHeader = (bookDonationIncomeMetric) =>{
	return null
}
const internalRenderExtraFooter = (bookDonationIncomeMetric) =>{
	return null
}
...
const internalSummaryOf = (bookDonationIncomeMetric,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookDonationIncomeMetric.id}</Description> 
<Description term="共享者分成比">{bookDonationIncomeMetric.vendorRate}</Description> 
<Description term="借出网点分成比">{bookDonationIncomeMetric.lendingStoreRate}</Description> 
<Description term="平台分成比">{bookDonationIncomeMetric.platformRate}</Description> 
<Description term="公益基金分成比">{bookDonationIncomeMetric.publicServiceFundRate}</Description> 
<Description term="图书天地">{bookDonationIncomeMetric.bookPlaza==null?"未分配":bookDonationIncomeMetric.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",bookDonationIncomeMetric.bookPlaza?bookDonationIncomeMetric.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookDonationIncomeMetric,targetComponent)}
      </DescriptionList>
	)
...
class BookDonationIncomeMetricPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.bookDonationIncomeMetric
    const cardsData = {cardsName:"捐赠图书收益分成配置",cardsFor: "bookDonationIncomeMetric",cardsSource: this.props.bookDonationIncomeMetric,
  		subItems: [
...
export default connect(state => ({
  bookDonationIncomeMetric: state._bookDonationIncomeMetric,
}))(Form.create()(BookDonationIncomeMetricPreference))
