...
class BookDonationIncomeMetricSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookDonationIncomeMetricIds = selectedRows.map((item) => { return item.id })
    console.log('bookDonationIncomeMetricIds', bookDonationIncomeMetricIds)
    const parameters = { bookDonationIncomeMetricIds }
    dispatch({
      type: `${owner.type}/removeBookDonationIncomeMetricList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookDonationIncomeMetric' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookDonationIncomeMetric', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'捐赠图书收益分成配置列表' } })
  ...
  render() {
    ...
    const {BookDonationIncomeMetricTable} = GlobalComponents;
    const {BookDonationIncomeMetricModalTable} = GlobalComponents;
    const {BookDonationIncomeMetricSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookDonationIncomeMetricModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 捐赠图书收益分成配置列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookDonationIncomeMetricSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookDonationIncomeMetricSearch)
