...
import styles from './BookDonationIncomeMetric.updateform.less'
...
class BookDonationIncomeMetricUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const bookDonationIncomeMetricId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, bookDonationIncomeMetricId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateBookDonationIncomeMetric`,
          payload: {
            id: owner.id,
            type: 'bookDonationIncomeMetric',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextBookDonationIncomeMetricUpdateRow`,
        payload: {
          id: owner.id,
          type: 'bookDonationIncomeMetric',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新捐赠图书收益分成配置"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新捐赠图书收益分成配置"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.publicServiceFundRate} {...formItemLayout}>
                  {getFieldDecorator('publicServiceFundRate', {
                    initialValue: selectedRow.publicServiceFundRate,
...
}))(Form.create()(BookDonationIncomeMetricUpdateForm))
