...
import styles from './BookPlaza.dashboard.less'
...
const internalImageListOf = (bookPlaza) =>{
...
const internalSettingListOf = (bookPlaza) =>{
...
const internalLargeTextOf = (bookPlaza) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookPlazaService[candidateServiceName] 
...
const executeTrans = (bookPlaza,targetComponent) =>{
...
const buildTransferModal = (bookPlaza,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookPlaza,targetComponent)}
...
const internalRenderExtraHeader = (bookPlaza) =>{
	return null
}
const internalRenderExtraFooter = (bookPlaza) =>{
	return null
}
...
const internalSummaryOf = (bookPlaza,targetComponent) =>{
...
class BookPlazaDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"图书天地",cardsFor: "bookPlaza",cardsSource: this.props.bookPlaza,
  		subItems: [
{name: 'bookRecommendList', displayName:'图书推荐',type:'bookRecommend',count:bookRecommendCount},
{name: 'bookSharingIncomeMetricList', displayName:'共享图书收益分成配置',type:'bookSharingIncomeMetric',count:bookSharingIncomeMetricCount},
...
}))(Form.create()(BookPlazaDashboard))
