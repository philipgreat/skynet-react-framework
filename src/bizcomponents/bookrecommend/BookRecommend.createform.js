...
import styles from './BookRecommend.createform.less'
...
class BookRecommendCreateForm extends Component {
  state = {
  ...
  executeCandidateBookPlazaSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookRecommendService.requestCandidateBookPlaza("bookPlaza", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = BookRecommendBase
    const submitCreateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookRecommend`,
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookRecommend`,
          payload: { id: owner.id, type: 'bookRecommend', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookRecommend',listName:'图书推荐列表' },
      })
    }
...
}))(Form.create()(BookRecommendCreateForm))
