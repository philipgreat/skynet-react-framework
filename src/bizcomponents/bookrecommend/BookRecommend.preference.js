...
import styles from './BookRecommend.preference.less'
...
const internalImageListOf = (bookRecommend) =>{
...
const internalSettingListOf = (bookRecommend) =>{
...
const internalLargeTextOf = (bookRecommend) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookRecommendService[candidateServiceName] 
...
const executeTrans = (bookRecommend,targetComponent) =>{
...
const buildTransferModal = (bookRecommend,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookRecommend,targetComponent)}
...
const internalRenderExtraHeader = (bookRecommend) =>{
	return null
}
const internalRenderExtraFooter = (bookRecommend) =>{
	return null
}
...
const internalSummaryOf = (bookRecommend,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookRecommend.id}</Description> 
<Description term="名称">{bookRecommend.name}</Description> 
<Description term="图书天地">{bookRecommend.bookPlaza==null?"未分配":bookRecommend.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",bookRecommend.bookPlaza?bookRecommend.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookRecommend,targetComponent)}
      </DescriptionList>
	)
...
class BookRecommendPreference extends Component {
...
}))(Form.create()(BookRecommendPreference))
