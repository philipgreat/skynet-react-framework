...
import styles from './BookRecommend.search.less'
...
class BookRecommendSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookRecommendIds = selectedRows.map((item) => { return item.id })
    console.log('bookRecommendIds', bookRecommendIds)
    const parameters = { bookRecommendIds }
    dispatch({
      type: `${owner.type}/removeBookRecommendList`,
      payload: { id: owner.id, type: 'bookRecommend', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookRecommend' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookRecommend', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书推荐列表' } })
  ...
  render() {
    ...
    const {BookRecommendTable} = GlobalComponents;
    const {BookRecommendModalTable} = GlobalComponents;
    const {BookRecommendSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookRecommendModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书推荐列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookRecommendSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookRecommendSearch)
