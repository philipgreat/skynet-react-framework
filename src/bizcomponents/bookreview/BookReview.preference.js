...
import styles from './BookReview.preference.less'
...
const internalImageListOf = (bookReview) =>{
...
const internalSettingListOf = (bookReview) =>{
...
const internalLargeTextOf = (bookReview) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookReviewService[candidateServiceName] 
...
const executeTrans = (bookReview,targetComponent) =>{
...
const buildTransferModal = (bookReview,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookReview,targetComponent)}
...
const internalRenderExtraHeader = (bookReview) =>{
	return null
}
const internalRenderExtraFooter = (bookReview) =>{
	return null
}
...
const internalSummaryOf = (bookReview,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookReview.id}</Description> 
<Description term="书籍信息">{bookReview.bookInfo==null?"未分配":bookReview.bookInfo.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"书籍信息","book","requestCandidateBookInfo",
	      "transferToAnotherBookInfo","anotherBookInfoId",bookReview.bookInfo?bookReview.bookInfo.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="书籍副本">{bookReview.bookCopy==null?"未分配":bookReview.bookCopy.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"书籍副本","bookCopy","requestCandidateBookCopy",
	      "transferToAnotherBookCopy","anotherBookCopyId",bookReview.bookCopy?bookReview.bookCopy.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="评论人">{bookReview.reviewer==null?"未分配":bookReview.reviewer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"评论人","customer","requestCandidateReviewer",
	      "transferToAnotherReviewer","anotherReviewerId",bookReview.reviewer?bookReview.reviewer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="评论发布日期时间">{ moment(bookReview.reviewPublishDatetime).format('YYYY-MM-DD')}</Description> 
<Description term="评论内容">{bookReview.reviewContent}</Description> 
<Description term="图书天地">{bookReview.bookPlaza==null?"未分配":bookReview.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",bookReview.bookPlaza?bookReview.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookReview,targetComponent)}
      </DescriptionList>
	)
...
class BookReviewPreference extends Component {
...
}))(Form.create()(BookReviewPreference))
