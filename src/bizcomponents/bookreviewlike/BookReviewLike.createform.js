...
class BookReviewLikeCreateForm extends Component {
  ...
  executeCandidateBookReviewSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookReviewLikeService.requestCandidateBookReview("bookReview", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = BookReviewLikeBase
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookReviewLike',listName:'书评点赞列表' },
      })
    }
