...
import styles from './BookReviewLike.preference.less'
...
const internalImageListOf = (bookReviewLike) =>{
...
const internalSettingListOf = (bookReviewLike) =>{
...
const internalLargeTextOf = (bookReviewLike) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookReviewLikeService[candidateServiceName] 
...
const executeTrans = (bookReviewLike,targetComponent) =>{
...
const buildTransferModal = (bookReviewLike,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookReviewLike,targetComponent)}
...
const internalRenderExtraHeader = (bookReviewLike) =>{
	return null
}
const internalRenderExtraFooter = (bookReviewLike) =>{
	return null
}
...
const internalSummaryOf = (bookReviewLike,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookReviewLike.id}</Description> 
<Description term="书评">{bookReviewLike.bookReview==null?"未分配":bookReviewLike.bookReview.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"书评","bookReview","requestCandidateBookReview",
	      "transferToAnotherBookReview","anotherBookReviewId",bookReviewLike.bookReview?bookReviewLike.bookReview.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="回复人">{bookReviewLike.replier==null?"未分配":bookReviewLike.replier.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"回复人","customer","requestCandidateReplier",
	      "transferToAnotherReplier","anotherReplierId",bookReviewLike.replier?bookReviewLike.replier.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="点赞发布日期">{ moment(bookReviewLike.likePublishDatetime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(bookReviewLike,targetComponent)}
      </DescriptionList>
	)
...
class BookReviewLikePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"书评点赞",cardsFor: "bookReviewLike",cardsSource: this.props.bookReviewLike,
  		subItems: [
...
}))(Form.create()(BookReviewLikePreference))
