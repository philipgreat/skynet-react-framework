...
import styles from './BookSharingIncomeMetric.createform.less'
...
class BookSharingIncomeMetricCreateForm extends Component {
  state = {
  ...
  executeCandidateBookPlazaSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookSharingIncomeMetricService.requestCandidateBookPlaza("bookPlaza", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookSharingIncomeMetric`,
          payload: { id: owner.id, type: 'bookSharingIncomeMetric', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookSharingIncomeMetric',listName:'共享图书收益分成配置列表' },
      })
    }
    ...
    return (
      <PageHeaderLayout
        title="新建一个共享图书收益分成配置"
        content="新建一个共享图书收益分成配置"
...
}))(Form.create()(BookSharingIncomeMetricCreateForm))
