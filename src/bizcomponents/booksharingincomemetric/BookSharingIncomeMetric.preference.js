...
import styles from './BookSharingIncomeMetric.preference.less'
...
const internalImageListOf = (bookSharingIncomeMetric) =>{
...
const internalSettingListOf = (bookSharingIncomeMetric) =>{
...
const internalLargeTextOf = (bookSharingIncomeMetric) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {BookSharingIncomeMetricService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = BookSharingIncomeMetricService[candidateServiceName] 
...
const executeTrans = (bookSharingIncomeMetric,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=bookSharingIncomeMetric.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_bookSharingIncomeMetric/doJob",payload: payload})
...
const buildTransferModal = (bookSharingIncomeMetric,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookSharingIncomeMetric,targetComponent)}
...
const internalRenderExtraHeader = (bookSharingIncomeMetric) =>{
	return null
}
const internalRenderExtraFooter = (bookSharingIncomeMetric) =>{
	return null
}
...
const internalSummaryOf = (bookSharingIncomeMetric,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookSharingIncomeMetric.id}</Description> 
<Description term="共享者分成比">{bookSharingIncomeMetric.vendorRate}</Description> 
<Description term="借出网点分成比">{bookSharingIncomeMetric.lendingStoreRate}</Description> 
<Description term="平台分成比">{bookSharingIncomeMetric.platformRate}</Description> 
<Description term="图书天地">{bookSharingIncomeMetric.bookPlaza==null?"未分配":bookSharingIncomeMetric.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",bookSharingIncomeMetric.bookPlaza?bookSharingIncomeMetric.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookSharingIncomeMetric,targetComponent)}
      </DescriptionList>
	)
...
class BookSharingIncomeMetricPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.bookSharingIncomeMetric
    const cardsData = {cardsName:"共享图书收益分成配置",cardsFor: "bookSharingIncomeMetric",cardsSource: this.props.bookSharingIncomeMetric,
  		subItems: [
...
export default connect(state => ({
  bookSharingIncomeMetric: state._bookSharingIncomeMetric,
}))(Form.create()(BookSharingIncomeMetricPreference))
