...
const internalImageListOf = (bookTakeStockPlan) =>{
...
const internalSettingListOf = (bookTakeStockPlan) =>{
...
const internalLargeTextOf = (bookTakeStockPlan) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookTakeStockPlanService[candidateServiceName] 
...
const executeTrans = (bookTakeStockPlan,targetComponent) =>{
...
const buildTransferModal = (bookTakeStockPlan,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookTakeStockPlan,targetComponent)}
...
const internalRenderExtraHeader = (bookTakeStockPlan) =>{
	return null
}
const internalRenderExtraFooter = (bookTakeStockPlan) =>{
	return null
}
...
const internalSummaryOf = (bookTakeStockPlan,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookTakeStockPlan.id}</Description> 
<Description term="计划名称">{bookTakeStockPlan.planName}</Description> 
<Description term="服务网点">{bookTakeStockPlan.store==null?"未分配":bookTakeStockPlan.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务网点","store","requestCandidateStore",
	      "transferToAnotherStore","anotherStoreId",bookTakeStockPlan.store?bookTakeStockPlan.store.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="计划日期时间">{ moment(bookTakeStockPlan.planDatetime).format('YYYY-MM-DD')}</Description> 
<Description term="计划创建人">{bookTakeStockPlan.planCreator==null?"未分配":bookTakeStockPlan.planCreator.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"计划创建人","employee","requestCandidatePlanCreator",
...
class BookTakeStockPlanDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, bookTakeStockResultCount } = this.props.bookTakeStockPlan
    const cardsData = {cardsName:"图书盘点计划",cardsFor: "bookTakeStockPlan",cardsSource: this.props.bookTakeStockPlan,
  		subItems: [
{name: 'bookTakeStockResultList', displayName:'图书盘点结果',type:'bookTakeStockResult',count:bookTakeStockResultCount},
...
}))(Form.create()(BookTakeStockPlanDashboard))
