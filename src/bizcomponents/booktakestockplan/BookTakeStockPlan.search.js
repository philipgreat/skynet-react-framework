...
import styles from './BookTakeStockPlan.search.less'
...
class BookTakeStockPlanSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookTakeStockPlanIds = selectedRows.map((item) => { return item.id })
    console.log('bookTakeStockPlanIds', bookTakeStockPlanIds)
    const parameters = { bookTakeStockPlanIds }
    dispatch({
      type: `${owner.type}/removeBookTakeStockPlanList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTakeStockPlan' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTakeStockPlan', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书盘点计划列表' } })
  ...
  render() {
    ...
    const {BookTakeStockPlanTable} = GlobalComponents;
    const {BookTakeStockPlanModalTable} = GlobalComponents;
    const {BookTakeStockPlanSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookTakeStockPlanModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书盘点计划列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookTakeStockPlanSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookTakeStockPlanSearch)
