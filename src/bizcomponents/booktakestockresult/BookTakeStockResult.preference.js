...
import styles from './BookTakeStockResult.preference.less'
...
const internalImageListOf = (bookTakeStockResult) =>{
...
const internalSettingListOf = (bookTakeStockResult) =>{
...
const internalLargeTextOf = (bookTakeStockResult) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookTakeStockResultService[candidateServiceName] 
...
const executeTrans = (bookTakeStockResult,targetComponent) =>{
...
const buildTransferModal = (bookTakeStockResult,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookTakeStockResult,targetComponent)}
...
const internalRenderExtraHeader = (bookTakeStockResult) =>{
	return null
}
const internalRenderExtraFooter = (bookTakeStockResult) =>{
	return null
}
...
const internalSummaryOf = (bookTakeStockResult,targetComponent) =>{
...
<Description term="图书盘点计划">{bookTakeStockResult.bookTakeStockPlan==null?"未分配":bookTakeStockResult.bookTakeStockPlan.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书盘点计划","bookTakeStockPlan","requestCandidateBookTakeStockPlan",
	      "transferToAnotherBookTakeStockPlan","anotherBookTakeStockPlanId",bookTakeStockResult.bookTakeStockPlan?bookTakeStockResult.bookTakeStockPlan.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(bookTakeStockResult,targetComponent)}
      </DescriptionList>
	)
...
class BookTakeStockResultPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.bookTakeStockResult
    const cardsData = {cardsName:"图书盘点结果",cardsFor: "bookTakeStockResult",cardsSource: this.props.bookTakeStockResult,
  		subItems: [
...
export default connect(state => ({
  bookTakeStockResult: state._bookTakeStockResult,
}))(Form.create()(BookTakeStockResultPreference))
