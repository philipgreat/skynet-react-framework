...
import styles from './BookTakeStockResult.search.less'
...
class BookTakeStockResultSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookTakeStockResultIds = selectedRows.map((item) => { return item.id })
    console.log('bookTakeStockResultIds', bookTakeStockResultIds)
    const parameters = { bookTakeStockResultIds }
    dispatch({
      type: `${owner.type}/removeBookTakeStockResultList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTakeStockResult' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTakeStockResult', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书盘点结果列表' } })
  ...
  render() {
    ...
    const {BookTakeStockResultTable} = GlobalComponents;
    const {BookTakeStockResultModalTable} = GlobalComponents;
    const {BookTakeStockResultSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookTakeStockResultModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书盘点结果列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookTakeStockResultSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookTakeStockResultSearch)
