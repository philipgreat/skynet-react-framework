...
import styles from './BookTakeStockStatus.dashboard.less'
...
const internalImageListOf = (bookTakeStockStatus) =>{
...
const internalSettingListOf = (bookTakeStockStatus) =>{
...
const internalLargeTextOf = (bookTakeStockStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BookTakeStockStatusService[candidateServiceName] 
...
const executeTrans = (bookTakeStockStatus,targetComponent) =>{
...
const buildTransferModal = (bookTakeStockStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(bookTakeStockStatus,targetComponent)}
...
const internalRenderExtraHeader = (bookTakeStockStatus) =>{
	return null
}
const internalRenderExtraFooter = (bookTakeStockStatus) =>{
	return null
}
...
const internalSummaryOf = (bookTakeStockStatus,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{bookTakeStockStatus.id}</Description> 
<Description term="名称">{bookTakeStockStatus.name}</Description> 
<Description term="代码">{bookTakeStockStatus.code}</Description> 
	
        {buildTransferModal(bookTakeStockStatus,targetComponent)}
      </DescriptionList>
	)
...
class BookTakeStockStatusDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, bookTakeStockResultAccessiable, bookTakeStockResultCount } = this.props.bookTakeStockStatus
...
export default connect(state => ({
  bookTakeStockStatus: state._bookTakeStockStatus,
}))(Form.create()(BookTakeStockStatusDashboard))
