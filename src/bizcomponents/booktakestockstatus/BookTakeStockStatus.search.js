...
import styles from './BookTakeStockStatus.search.less'
...
class BookTakeStockStatusSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookTakeStockStatusIds = selectedRows.map((item) => { return item.id })
    console.log('bookTakeStockStatusIds', bookTakeStockStatusIds)
    const parameters = { bookTakeStockStatusIds }
    dispatch({
      type: `${owner.type}/removeBookTakeStockStatusList`,
      payload: { id: owner.id, type: 'bookTakeStockStatus', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTakeStockStatus' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTakeStockStatus', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书盘点状态列表' } })
  ...
  render() {
    ...
    const {BookTakeStockStatusTable} = GlobalComponents;
    const {BookTakeStockStatusModalTable} = GlobalComponents;
    const {BookTakeStockStatusSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookTakeStockStatusModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书盘点状态列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookTakeStockStatusSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookTakeStockStatusSearch)
