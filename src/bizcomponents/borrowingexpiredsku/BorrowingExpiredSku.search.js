...
class BorrowingExpiredSkuSearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'借书超期费列表' } })
  ...
  render() {
    ...
    const {BorrowingExpiredSkuModalTable} = GlobalComponents;
...
<Menu.Item key="borrowingHistory">关联图书借还历史</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BorrowingExpiredSkuModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 借书超期费列表`}>
        <Card bordered={false}>
