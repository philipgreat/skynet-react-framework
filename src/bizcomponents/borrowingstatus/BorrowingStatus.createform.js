...
import styles from './BorrowingStatus.createform.less'
...
class BorrowingStatusCreateForm extends Component {
  state = {
  ...
  executeCandidateBookSharingPlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BorrowingStatusService.requestCandidateBookSharingPlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = BorrowingStatusBase
    const submitCreateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBorrowingStatus`,
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBorrowingStatus`,
          payload: { id: owner.id, type: 'borrowingStatus', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'borrowingStatus',listName:'借书状态列表' },
      })
    }
...
}))(Form.create()(BorrowingStatusCreateForm))
