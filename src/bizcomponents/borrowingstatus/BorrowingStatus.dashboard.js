...
import styles from './BorrowingStatus.dashboard.less'
...
const internalImageListOf = (borrowingStatus) =>{
...
const internalSettingListOf = (borrowingStatus) =>{
...
const internalLargeTextOf = (borrowingStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BorrowingStatusService[candidateServiceName] 
...
const executeTrans = (borrowingStatus,targetComponent) =>{
...
const buildTransferModal = (borrowingStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(borrowingStatus,targetComponent)}
...
const internalRenderExtraHeader = (borrowingStatus) =>{
	return null
}
const internalRenderExtraFooter = (borrowingStatus) =>{
	return null
}
...
const internalSummaryOf = (borrowingStatus,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{borrowingStatus.id}</Description> 
<Description term="名称">{borrowingStatus.name}</Description> 
<Description term="代码">{borrowingStatus.code}</Description> 
	
        {buildTransferModal(borrowingStatus,targetComponent)}
      </DescriptionList>
	)
...
class BorrowingStatusDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, borrowingHistoryAccessiable, borrowingHistoryCount } = this.props.borrowingStatus
    const cardsData = {cardsName:"借书状态",cardsFor: "borrowingStatus",cardsSource: this.props.borrowingStatus,
  		subItems: [
...
export default connect(state => ({
  borrowingStatus: state._borrowingStatus,
}))(Form.create()(BorrowingStatusDashboard))
