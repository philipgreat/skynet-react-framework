...
import styles from './BorrowingStatus.search.less'
...
class BorrowingStatusSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const borrowingStatusIds = selectedRows.map((item) => { return item.id })
    console.log('borrowingStatusIds', borrowingStatusIds)
    const parameters = { borrowingStatusIds }
    dispatch({
      type: `${owner.type}/removeBorrowingStatusList`,
      payload: { id: owner.id, type: 'borrowingStatus', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'borrowingStatus' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'borrowingStatus', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'借书状态列表' } })
  ...
  render() {
    ...
    const {BorrowingStatusTable} = GlobalComponents;
    const {BorrowingStatusModalTable} = GlobalComponents;
    const {BorrowingStatusSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BorrowingStatusModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 借书状态列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BorrowingStatusSearchForm {...this.props} />
            </div>
...
export default Form.create()(BorrowingStatusSearch)
