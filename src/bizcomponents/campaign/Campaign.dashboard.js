...
const internalImageListOf = (campaign) =>{

  const imageList = [
	   {"title":'活动图片',"imageLocation":campaign.campaignImage},
]
...
const internalSettingListOf = (campaign) =>{
...
const internalLargeTextOf = (campaign) =>{

	return(<div> 
   <Card title={`活动内容`} ><pre>{campaign.campaignContent}</pre></Card>
</div>)
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CampaignService[candidateServiceName] 
...
const executeTrans = (campaign,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_campaign/doJob",payload: payload})
...
const buildTransferModal = (campaign,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(campaign,targetComponent)}
...
const internalRenderExtraHeader = (campaign) =>{
	return null
}
const internalRenderExtraFooter = (campaign) =>{
	return null
}
...
const internalSummaryOf = (campaign,targetComponent) =>{
...
<Description term="发布网点">{campaign.publishStore==null?"未分配":campaign.publishStore.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"发布网点","store","requestCandidatePublishStore",
	      "transferToAnotherPublishStore","anotherPublishStoreId",campaign.publishStore?campaign.publishStore.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="发布员工">{campaign.publishEmployee==null?"未分配":campaign.publishEmployee.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"发布员工","employee","requestCandidatePublishEmployee",
	      "transferToAnotherPublishEmployee","anotherPublishEmployeeId",campaign.publishEmployee?campaign.publishEmployee.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="活动广场">{campaign.campaignPlaza==null?"未分配":campaign.campaignPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"活动广场","campaignPlaza","requestCandidateCampaignPlaza",
	      "transferToAnotherCampaignPlaza","anotherCampaignPlazaId",campaign.campaignPlaza?campaign.campaignPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(campaign,targetComponent)}
      </DescriptionList>
	)
