...
import styles from './CampaignLike.preference.less'
...
const internalImageListOf = (campaignLike) =>{
...
const internalSettingListOf = (campaignLike) =>{
...
const internalLargeTextOf = (campaignLike) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CampaignLikeService[candidateServiceName] 
...
const executeTrans = (campaignLike,targetComponent) =>{
...
const buildTransferModal = (campaignLike,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(campaignLike,targetComponent)}
...
const internalRenderExtraHeader = (campaignLike) =>{
	return null
}
const internalRenderExtraFooter = (campaignLike) =>{
	return null
}
...
const internalSummaryOf = (campaignLike,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{campaignLike.id}</Description> 
<Description term="活动">{campaignLike.campaign==null?"未分配":campaignLike.campaign.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"活动","campaign","requestCandidateCampaign",
	      "transferToAnotherCampaign","anotherCampaignId",campaignLike.campaign?campaignLike.campaign.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="回复人">{campaignLike.replier==null?"未分配":campaignLike.replier.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"回复人","customer","requestCandidateReplier",
	      "transferToAnotherReplier","anotherReplierId",campaignLike.replier?campaignLike.replier.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class CampaignLikePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.campaignLike
    const cardsData = {cardsName:"活动点赞",cardsFor: "campaignLike",cardsSource: this.props.campaignLike,
  		subItems: [
...
export default connect(state => ({
  campaignLike: state._campaignLike,
}))(Form.create()(CampaignLikePreference))
