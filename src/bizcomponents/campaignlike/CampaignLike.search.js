...
import styles from './CampaignLike.search.less'
...
class CampaignLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const campaignLikeIds = selectedRows.map((item) => { return item.id })
    console.log('campaignLikeIds', campaignLikeIds)
    const parameters = { campaignLikeIds }
    dispatch({
      type: `${owner.type}/removeCampaignLikeList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'活动点赞列表' } })
  ...
  render() {
    ...
    const {CampaignLikeTable} = GlobalComponents;
    const {CampaignLikeModalTable} = GlobalComponents;
    const {CampaignLikeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CampaignLikeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 活动点赞列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CampaignLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(CampaignLikeSearch)
