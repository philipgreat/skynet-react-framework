...
import styles from './CampaignPlaza.search.less'
...
class CampaignPlazaSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const campaignPlazaIds = selectedRows.map((item) => { return item.id })
    console.log('campaignPlazaIds', campaignPlazaIds)
    const parameters = { campaignPlazaIds }
    dispatch({
      type: `${owner.type}/removeCampaignPlazaList`,
      payload: { id: owner.id, type: 'campaignPlaza', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignPlaza' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignPlaza', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'活动广场列表' } })
  ...
  render() {
    ...
    const {CampaignPlazaTable} = GlobalComponents;
    const {CampaignPlazaModalTable} = GlobalComponents;
    const {CampaignPlazaSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CampaignPlazaModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 活动广场列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CampaignPlazaSearchForm {...this.props} />
            </div>
...
export default Form.create()(CampaignPlazaSearch)
