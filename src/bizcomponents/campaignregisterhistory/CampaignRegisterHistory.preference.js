...
import styles from './CampaignRegisterHistory.preference.less'
...
const internalImageListOf = (campaignRegisterHistory) =>{
...
const internalSettingListOf = (campaignRegisterHistory) =>{

	const optionList = [ 
	  {"title":'报名已取消报名',"value":campaignRegisterHistory.cancelled,"parameterName":"cancelled"},
]
...
const internalLargeTextOf = (campaignRegisterHistory) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CampaignRegisterHistoryService[candidateServiceName] 
...
const executeTrans = (campaignRegisterHistory,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=campaignRegisterHistory.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_campaignRegisterHistory/doJob",payload: payload})
...
const buildTransferModal = (campaignRegisterHistory,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(campaignRegisterHistory,targetComponent)}
...
const internalRenderExtraHeader = (campaignRegisterHistory) =>{
	return null
}
const internalRenderExtraFooter = (campaignRegisterHistory) =>{
	return null
}
...
const internalSummaryOf = (campaignRegisterHistory,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{campaignRegisterHistory.id}</Description> 
<Description term="活动">{campaignRegisterHistory.campaign==null?"未分配":campaignRegisterHistory.campaign.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"活动","campaign","requestCandidateCampaign",
	      "transferToAnotherCampaign","anotherCampaignId",campaignRegisterHistory.campaign?campaignRegisterHistory.campaign.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="报名用户">{campaignRegisterHistory.registerMember==null?"未分配":campaignRegisterHistory.registerMember.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"报名用户","customer","requestCandidateRegisterMember",
	      "transferToAnotherRegisterMember","anotherRegisterMemberId",campaignRegisterHistory.registerMember?campaignRegisterHistory.registerMember.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="报名日期">{ moment(campaignRegisterHistory.registerDatetime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(campaignRegisterHistory,targetComponent)}
      </DescriptionList>
	)
...
class CampaignRegisterHistoryPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.campaignRegisterHistory
    const cardsData = {cardsName:"活动报名记录",cardsFor: "campaignRegisterHistory",cardsSource: this.props.campaignRegisterHistory,
  		subItems: [
...
export default connect(state => ({
  campaignRegisterHistory: state._campaignRegisterHistory,
}))(Form.create()(CampaignRegisterHistoryPreference))
