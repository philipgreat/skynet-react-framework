...
import styles from './CampaignRegisterHistory.search.less'
...
class CampaignRegisterHistorySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const campaignRegisterHistoryIds = selectedRows.map((item) => { return item.id })
    console.log('campaignRegisterHistoryIds', campaignRegisterHistoryIds)
    const parameters = { campaignRegisterHistoryIds }
    dispatch({
      type: `${owner.type}/removeCampaignRegisterHistoryList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignRegisterHistory' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignRegisterHistory', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'活动报名记录列表' } })
  ...
  render() {
    ...
    const {CampaignRegisterHistoryTable} = GlobalComponents;
    const {CampaignRegisterHistoryModalTable} = GlobalComponents;
    const {CampaignRegisterHistorySearchForm} = GlobalComponents;
...
<Menu.Item key="registerMember">关联报名用户</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CampaignRegisterHistoryModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 活动报名记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CampaignRegisterHistorySearchForm {...this.props} />
            </div>
...
export default Form.create()(CampaignRegisterHistorySearch)
