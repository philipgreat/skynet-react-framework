...
import styles from './CampaignRegisterHistory.updateform.less'
...
class CampaignRegisterHistoryUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const campaignRegisterHistoryId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, campaignRegisterHistoryId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateCampaignRegisterHistory`,
          payload: {
            id: owner.id,
            type: 'campaignRegisterHistory',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextCampaignRegisterHistoryUpdateRow`,
        payload: {
          id: owner.id,
          type: 'campaignRegisterHistory',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新活动报名记录"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新活动报名记录"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.registerDatetime} {...formItemLayout}>
                  {getFieldDecorator('registerDatetime', {
                    initialValue: selectedRow.registerDatetime,
              ...
              <Col lg={8} md={12} sm={24}>
                <Form.Item label={fieldLabels.cancelled} {...switchFormItemLayout}>
                  {getFieldDecorator('cancelled', {
                    initialValue: selectedRow.cancelled,
...
}))(Form.create()(CampaignRegisterHistoryUpdateForm))
