...
import styles from './CampaignReview.dashboard.less'
...
const internalImageListOf = (campaignReview) =>{
...
const internalSettingListOf = (campaignReview) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CampaignReviewService[candidateServiceName] 
...
const executeTrans = (campaignReview,targetComponent) =>{
...
const buildTransferModal = (campaignReview,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(campaignReview,targetComponent)}
...
const internalRenderExtraHeader = (campaignReview) =>{
	return null
}
const internalRenderExtraFooter = (campaignReview) =>{
	return null
}
...
const internalSummaryOf = (campaignReview,targetComponent) =>{
...
<Description term="活动">{campaignReview.campaign==null?"未分配":campaignReview.campaign.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"活动","campaign","requestCandidateCampaign",
	      "transferToAnotherCampaign","anotherCampaignId",campaignReview.campaign?campaignReview.campaign.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="评论人">{campaignReview.reviewer==null?"未分配":campaignReview.reviewer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"评论人","customer","requestCandidateReviewer",
	      "transferToAnotherReviewer","anotherReviewerId",campaignReview.reviewer?campaignReview.reviewer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="最后更新时间">{ moment(campaignReview.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(campaignReview,targetComponent)}
      </DescriptionList>
	)
...
class CampaignReviewDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, campaignReviewLikeAccessiable, informAccessiable, campaignReviewLikeCount, informCount } = this.props.campaignReview
...
}))(Form.create()(CampaignReviewDashboard))
