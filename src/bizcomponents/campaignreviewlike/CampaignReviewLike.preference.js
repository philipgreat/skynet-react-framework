...
import styles from './CampaignReviewLike.preference.less'
...
const internalImageListOf = (campaignReviewLike) =>{
...
const internalSettingListOf = (campaignReviewLike) =>{
...
const internalLargeTextOf = (campaignReviewLike) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CampaignReviewLikeService[candidateServiceName] 
...
const executeTrans = (campaignReviewLike,targetComponent) =>{
...
const buildTransferModal = (campaignReviewLike,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(campaignReviewLike,targetComponent)}
...
const internalRenderExtraHeader = (campaignReviewLike) =>{
	return null
}
const internalRenderExtraFooter = (campaignReviewLike) =>{
	return null
}
...
const internalSummaryOf = (campaignReviewLike,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{campaignReviewLike.id}</Description> 
<Description term="评论">{campaignReviewLike.review==null?"未分配":campaignReviewLike.review.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"评论","campaignReview","requestCandidateReview",
	      "transferToAnotherReview","anotherReviewId",campaignReviewLike.review?campaignReviewLike.review.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="回复人">{campaignReviewLike.replier==null?"未分配":campaignReviewLike.replier.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"回复人","customer","requestCandidateReplier",
	      "transferToAnotherReplier","anotherReplierId",campaignReviewLike.replier?campaignReviewLike.replier.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class CampaignReviewLikePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.campaignReviewLike
    const cardsData = {cardsName:"活动评论点赞",cardsFor: "campaignReviewLike",cardsSource: this.props.campaignReviewLike,
  		subItems: [
...
export default connect(state => ({
  campaignReviewLike: state._campaignReviewLike,
}))(Form.create()(CampaignReviewLikePreference))
