...
import styles from './CampaignReviewLike.search.less'
...
class CampaignReviewLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const campaignReviewLikeIds = selectedRows.map((item) => { return item.id })
    console.log('campaignReviewLikeIds', campaignReviewLikeIds)
    const parameters = { campaignReviewLikeIds }
    dispatch({
      type: `${owner.type}/removeCampaignReviewLikeList`,
      payload: { id: owner.id, type: 'campaignReviewLike', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignReviewLike' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignReviewLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'活动评论点赞列表' } })
  ...
  render() {
    ...
    const {CampaignReviewLikeTable} = GlobalComponents;
    const {CampaignReviewLikeModalTable} = GlobalComponents;
    const {CampaignReviewLikeSearchForm} = GlobalComponents;
...
<Menu.Item key="review">关联评论</Menu.Item>
<Menu.Item key="replier">关联回复人</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CampaignReviewLikeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 活动评论点赞列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CampaignReviewLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(CampaignReviewLikeSearch)
