...
import styles from './CampaignStatus.dashboard.less'
...
const internalImageListOf = (campaignStatus) =>{
...
const internalSettingListOf = (campaignStatus) =>{
...
const internalLargeTextOf = (campaignStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {CampaignStatusService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = CampaignStatusService[candidateServiceName] 
...
const executeTrans = (campaignStatus,targetComponent) =>{
...
const buildTransferModal = (campaignStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(campaignStatus,targetComponent)}
...
const internalRenderExtraHeader = (campaignStatus) =>{
	return null
}
const internalRenderExtraFooter = (campaignStatus) =>{
	return null
}
...
const internalSummaryOf = (campaignStatus,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{campaignStatus.id}</Description> 
<Description term="名称">{campaignStatus.name}</Description> 
<Description term="代码">{campaignStatus.code}</Description> 
	
        {buildTransferModal(campaignStatus,targetComponent)}
      </DescriptionList>
	)
...
class CampaignStatusDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, campaignCount } = this.props.campaignStatus
    const cardsData = {cardsName:"活动状态",cardsFor: "campaignStatus",cardsSource: this.props.campaignStatus,
  		subItems: [
...
export default connect(state => ({
  campaignStatus: state._campaignStatus,
}))(Form.create()(CampaignStatusDashboard))
