...
import styles from './CampaignStatus.search.less'
...
class CampaignStatusSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const campaignStatusIds = selectedRows.map((item) => { return item.id })
    console.log('campaignStatusIds', campaignStatusIds)
    const parameters = { campaignStatusIds }
    dispatch({
      type: `${owner.type}/removeCampaignStatusList`,
      payload: { id: owner.id, type: 'campaignStatus', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignStatus' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaignStatus', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'活动状态列表' } })
  ...
  render() {
    ...
    const {CampaignStatusTable} = GlobalComponents;
    const {CampaignStatusModalTable} = GlobalComponents;
    const {CampaignStatusSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CampaignStatusModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 活动状态列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CampaignStatusSearchForm {...this.props} />
            </div>
...
export default Form.create()(CampaignStatusSearch)
