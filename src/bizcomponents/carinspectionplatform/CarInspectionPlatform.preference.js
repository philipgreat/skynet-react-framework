...
import styles from './CarInspectionPlatform.preference.less'
...
const internalImageListOf = (carInspectionPlatform) =>{
...
const internalSettingListOf = (carInspectionPlatform) =>{
...
const internalLargeTextOf = (carInspectionPlatform) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CarInspectionPlatformService[candidateServiceName] 
...
const executeTrans = (carInspectionPlatform,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=carInspectionPlatform.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_carInspectionPlatform/doJob",payload: payload})
...
const buildTransferModal = (carInspectionPlatform,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(carInspectionPlatform,targetComponent)}
...
const internalRenderExtraHeader = (carInspectionPlatform) =>{
	return null
}
const internalRenderExtraFooter = (carInspectionPlatform) =>{
	return null
}
...
const internalSummaryOf = (carInspectionPlatform,targetComponent) =>{
...
<Description term="保险联系人手机">{carInspectionPlatform.insuranceContactMobile}</Description> 
        ...
        {buildTransferModal(carInspectionPlatform,targetComponent)}
      </DescriptionList>
	)
...
class CarInspectionPlatformPreference extends Component {
...
}))(Form.create()(CarInspectionPlatformPreference))
