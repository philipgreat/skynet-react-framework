...
import styles from './City.preference.less'
...
const internalImageListOf = (city) =>{
...
const internalSettingListOf = (city) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CityService[candidateServiceName] 
...
const executeTrans = (city,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_city/doJob",payload: payload})
...
const buildTransferModal = (city,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(city,targetComponent)}
...
const internalRenderExtraHeader = (city) =>{
	return null
}
const internalRenderExtraFooter = (city) =>{
	return null
}
...
<Description term="省">{city.province==null?"未分配":city.province.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"省","province","requestCandidateProvince",
	      "transferToAnotherProvince","anotherProvinceId",city.province?city.province.id:"")} 
