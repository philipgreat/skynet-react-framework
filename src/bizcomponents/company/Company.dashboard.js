...
import styles from './Company.dashboard.less'
...
const largeTextOf = (company) =>{

	return(<div> 
   <Card title={`描述`} ><pre>{company.description}</pre></Card>
</div>)
...
const summaryOf = (company) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{company.id}</Description> 
...
class CompanyDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, company } = this.props;
  ...
  render() {
    ...
    const { id,displayName, drugApplicationCount } = this.props.company
    const cardsData = {cardsName:"公司",cardsFor: "company",cardsSource: this.props.company,
  		subItems: [
