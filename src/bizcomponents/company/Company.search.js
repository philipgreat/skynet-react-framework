...
import styles from './Company.search.less'
...
class CompanySearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const companyIds = selectedRows.map((item) => { return item.id })
    console.log('companyIds', companyIds)
    const parameters = { companyIds }
    dispatch({
      type: `${owner.type}/removeCompanyList`,
      payload: { id: owner.id, type: 'company', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'company' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'company', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'公司列表' } })
  ...
  render() {
    ...
    const {CompanyTable} = GlobalComponents;
    const {CompanyConfirmationTable} = GlobalComponents;
    const {CompanySearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 公司列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CompanySearchForm {...this.props} />
            </div>
...
export default Form.create()(CompanySearch)
