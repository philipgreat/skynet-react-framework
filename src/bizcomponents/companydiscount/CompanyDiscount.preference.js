...
import styles from './CompanyDiscount.preference.less'
...
const internalImageListOf = (companyDiscount) =>{
...
const internalSettingListOf = (companyDiscount) =>{

	const optionList = [ 
	  {"title":'代理费全免',"value":companyDiscount.allFreeDiscount,"parameterName":"allFreeDiscount"},
]
...
const internalLargeTextOf = (companyDiscount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CompanyDiscountService[candidateServiceName] 
...
const executeTrans = (companyDiscount,targetComponent) =>{
...
const buildTransferModal = (companyDiscount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(companyDiscount,targetComponent)}
...
const internalRenderExtraHeader = (companyDiscount) =>{
	return null
}
const internalRenderExtraFooter = (companyDiscount) =>{
	return null
}
...
const internalSummaryOf = (companyDiscount,targetComponent) =>{
...
<Description term="商户">{companyDiscount.company==null?"未分配":companyDiscount.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",companyDiscount.company?companyDiscount.company.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="创建时间">{ moment(companyDiscount.createTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(companyDiscount,targetComponent)}
      </DescriptionList>
	)
...
class CompanyDiscountPreference extends Component {
...
}))(Form.create()(CompanyDiscountPreference))
