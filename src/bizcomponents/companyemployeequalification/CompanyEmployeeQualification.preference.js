...
import styles from './CompanyEmployeeQualification.preference.less'
...
const internalImageListOf = (companyEmployeeQualification) =>{
...
const internalSettingListOf = (companyEmployeeQualification) =>{
...
const internalLargeTextOf = (companyEmployeeQualification) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CompanyEmployeeQualificationService[candidateServiceName] 
...
const executeTrans = (companyEmployeeQualification,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=companyEmployeeQualification.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_companyEmployeeQualification/doJob",payload: payload})
...
const buildTransferModal = (companyEmployeeQualification,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(companyEmployeeQualification,targetComponent)}
...
const internalRenderExtraHeader = (companyEmployeeQualification) =>{
	return null
}
const internalRenderExtraFooter = (companyEmployeeQualification) =>{
	return null
}
...
const internalSummaryOf = (companyEmployeeQualification,targetComponent) =>{
...
<Description term="批注">{companyEmployeeQualification.comment}</Description> 
        ...
        {buildTransferModal(companyEmployeeQualification,targetComponent)}
      </DescriptionList>
	)
...
class CompanyEmployeeQualificationPreference extends Component {
...
}))(Form.create()(CompanyEmployeeQualificationPreference))
