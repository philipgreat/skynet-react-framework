...
import styles from './CompanyEmployeeServing.preference.less'
...
const internalImageListOf = (companyEmployeeServing) =>{
...
const internalSettingListOf = (companyEmployeeServing) =>{
...
const internalLargeTextOf = (companyEmployeeServing) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CompanyEmployeeServingService[candidateServiceName] 
...
const executeTrans = (companyEmployeeServing,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=companyEmployeeServing.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_companyEmployeeServing/doJob",payload: payload})
...
const buildTransferModal = (companyEmployeeServing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(companyEmployeeServing,targetComponent)}
...
const internalRenderExtraHeader = (companyEmployeeServing) =>{
	return null
}
const internalRenderExtraFooter = (companyEmployeeServing) =>{
	return null
}
...
const internalSummaryOf = (companyEmployeeServing,targetComponent) =>{
...
<Description term="批注">{companyEmployeeServing.comment}</Description> 
        ...
        {buildTransferModal(companyEmployeeServing,targetComponent)}
      </DescriptionList>
	)
...
class CompanyEmployeeServingPreference extends Component {
...
}))(Form.create()(CompanyEmployeeServingPreference))
