...
import styles from './CompanyEmployeeTermination.preference.less'
...
const internalImageListOf = (companyEmployeeTermination) =>{
...
const internalSettingListOf = (companyEmployeeTermination) =>{
...
const internalLargeTextOf = (companyEmployeeTermination) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CompanyEmployeeTerminationService[candidateServiceName] 
...
const executeTrans = (companyEmployeeTermination,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=companyEmployeeTermination.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_companyEmployeeTermination/doJob",payload: payload})
...
const buildTransferModal = (companyEmployeeTermination,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(companyEmployeeTermination,targetComponent)}
...
const internalRenderExtraHeader = (companyEmployeeTermination) =>{
	return null
}
const internalRenderExtraFooter = (companyEmployeeTermination) =>{
	return null
}
...
const internalSummaryOf = (companyEmployeeTermination,targetComponent) =>{
...
<Description term="批注">{companyEmployeeTermination.comment}</Description> 
        ...
        {buildTransferModal(companyEmployeeTermination,targetComponent)}
      </DescriptionList>
	)
...
class CompanyEmployeeTerminationPreference extends Component {
...
}))(Form.create()(CompanyEmployeeTerminationPreference))
