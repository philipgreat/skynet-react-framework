...
import styles from './CompanyQrcodePromotionRecord.preference.less'
...
const internalImageListOf = (companyQrcodePromotionRecord) =>{
...
const internalSettingListOf = (companyQrcodePromotionRecord) =>{
...
const internalLargeTextOf = (companyQrcodePromotionRecord) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CompanyQrcodePromotionRecordService[candidateServiceName] 
...
const executeTrans = (companyQrcodePromotionRecord,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=companyQrcodePromotionRecord.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_companyQrcodePromotionRecord/doJob",payload: payload})
...
const buildTransferModal = (companyQrcodePromotionRecord,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(companyQrcodePromotionRecord,targetComponent)}
...
const internalRenderExtraHeader = (companyQrcodePromotionRecord) =>{
	return null
}
const internalRenderExtraFooter = (companyQrcodePromotionRecord) =>{
	return null
}
...
const internalSummaryOf = (companyQrcodePromotionRecord,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{companyQrcodePromotionRecord.id}</Description> 
<Description term="客户">{companyQrcodePromotionRecord.customer==null?"未分配":companyQrcodePromotionRecord.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"客户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",companyQrcodePromotionRecord.customer?companyQrcodePromotionRecord.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="商户">{companyQrcodePromotionRecord.company==null?"未分配":companyQrcodePromotionRecord.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",companyQrcodePromotionRecord.company?companyQrcodePromotionRecord.company.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="创建时间">{ moment(companyQrcodePromotionRecord.createTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(companyQrcodePromotionRecord,targetComponent)}
      </DescriptionList>
	)
...
class CompanyQrcodePromotionRecordPreference extends Component {
...
}))(Form.create()(CompanyQrcodePromotionRecordPreference))
