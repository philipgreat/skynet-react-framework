...
import styles from './Contract.preference.less'
...
const internalImageListOf = (contract) =>{
...
const internalSettingListOf = (contract) =>{
...
const internalLargeTextOf = (contract) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ContractService[candidateServiceName] 
...
const executeTrans = (contract,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_contract/doJob",payload: payload})
...
const buildTransferModal = (contract,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(contract,targetComponent)}
...
const internalRenderExtraHeader = (contract) =>{
	return null
}
const internalRenderExtraFooter = (contract) =>{
	return null
}
...
const internalSummaryOf = (contract,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{contract.id}</Description> 
<Description term="商户">{contract.company==null?"未分配":contract.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",contract.company?contract.company.id:"")} 
...
<Description term="结束日期">{ moment(contract.endDate).format('YYYY-MM-DD')}</Description> 
        ...
        {buildTransferModal(contract,targetComponent)}
      </DescriptionList>
	)
...
class ContractPreference extends Component {
...
}))(Form.create()(ContractPreference))
