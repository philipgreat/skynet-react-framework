...
import styles from './CustomerAccountTransaction.preference.less'
...
const internalImageListOf = (customerAccountTransaction) =>{
...
const internalSettingListOf = (customerAccountTransaction) =>{
...
const internalLargeTextOf = (customerAccountTransaction) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CustomerAccountTransactionService[candidateServiceName] 
...
const executeTrans = (customerAccountTransaction,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=customerAccountTransaction.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_customerAccountTransaction/doJob",payload: payload})
...
const buildTransferModal = (customerAccountTransaction,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(customerAccountTransaction,targetComponent)}
...
const internalRenderExtraHeader = (customerAccountTransaction) =>{
	return null
}
const internalRenderExtraFooter = (customerAccountTransaction) =>{
	return null
}
...
const internalSummaryOf = (customerAccountTransaction,targetComponent) =>{
...
<Description term="交易类型">{customerAccountTransaction.transactionType==null?"未分配":customerAccountTransaction.transactionType.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"交易类型","transactionType","requestCandidateTransactionType",
	      "transferToAnotherTransactionType","anotherTransactionTypeId",customerAccountTransaction.transactionType?customerAccountTransaction.transactionType.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="用户">{customerAccountTransaction.customer==null?"未分配":customerAccountTransaction.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"用户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",customerAccountTransaction.customer?customerAccountTransaction.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="交易时间">{ moment(customerAccountTransaction.changeDatetime).format('YYYY-MM-DD')}</Description> 
<Description term="相关订单">{customerAccountTransaction.relatedMainOrder==null?"未分配":customerAccountTransaction.relatedMainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"相关订单","mainOrder","requestCandidateRelatedMainOrder",
	      "transferToAnotherRelatedMainOrder","anotherRelatedMainOrderId",customerAccountTransaction.relatedMainOrder?customerAccountTransaction.relatedMainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(customerAccountTransaction,targetComponent)}
      </DescriptionList>
	)
...
class CustomerAccountTransactionPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.customerAccountTransaction
    const cardsData = {cardsName:"客户账户明细",cardsFor: "customerAccountTransaction",cardsSource: this.props.customerAccountTransaction,
  		subItems: [
...
export default connect(state => ({
  customerAccountTransaction: state._customerAccountTransaction,
}))(Form.create()(CustomerAccountTransactionPreference))
