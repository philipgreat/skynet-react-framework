...
import styles from './CustomerAccountTransaction.search.less'
...
class CustomerAccountTransactionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const customerAccountTransactionIds = selectedRows.map((item) => { return item.id })
    console.log('customerAccountTransactionIds', customerAccountTransactionIds)
    const parameters = { customerAccountTransactionIds }
    dispatch({
      type: `${owner.type}/removeCustomerAccountTransactionList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'customerAccountTransaction' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'customerAccountTransaction', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'客户账户明细列表' } })
  ...
  render() {
    ...
    const {CustomerAccountTransactionTable} = GlobalComponents;
    const {CustomerAccountTransactionModalTable} = GlobalComponents;
    const {CustomerAccountTransactionSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CustomerAccountTransactionModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 客户账户明细列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CustomerAccountTransactionSearchForm {...this.props} />
            </div>
...
export default Form.create()(CustomerAccountTransactionSearch)
