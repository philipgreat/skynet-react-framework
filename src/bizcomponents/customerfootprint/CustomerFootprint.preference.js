...
import styles from './CustomerFootprint.preference.less'
...
const internalImageListOf = (customerFootprint) =>{

  const imageList = [
	   {"title":'相关项图片',"imageLocation":customerFootprint.itemImage},
]
...
const internalSettingListOf = (customerFootprint) =>{

	const optionList = [ 
	  {"title":'是否有相关项',"value":customerFootprint.hasRelatedItem,"parameterName":"hasRelatedItem"},
]
...
const internalLargeTextOf = (customerFootprint) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CustomerFootprintService[candidateServiceName] 
...
const executeTrans = (customerFootprint,targetComponent) =>{
...
const buildTransferModal = (customerFootprint,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(customerFootprint,targetComponent)}
...
const internalRenderExtraHeader = (customerFootprint) =>{
	return null
}
const internalRenderExtraFooter = (customerFootprint) =>{
	return null
}
...
const internalSummaryOf = (customerFootprint,targetComponent) =>{
...
<Description term="用户">{customerFootprint.customer==null?"未分配":customerFootprint.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"用户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",customerFootprint.customer?customerFootprint.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="创建时间">{ moment(customerFootprint.createTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(customerFootprint,targetComponent)}
      </DescriptionList>
	)
...
class CustomerFootprintPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.customerFootprint
    const cardsData = {cardsName:"用户历程",cardsFor: "customerFootprint",cardsSource: this.props.customerFootprint,
  		subItems: [
...
export default connect(state => ({
  customerFootprint: state._customerFootprint,
}))(Form.create()(CustomerFootprintPreference))
