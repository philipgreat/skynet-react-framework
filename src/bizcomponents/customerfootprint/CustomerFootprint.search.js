...
import styles from './CustomerFootprint.search.less'
...
class CustomerFootprintSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const customerFootprintIds = selectedRows.map((item) => { return item.id })
    console.log('customerFootprintIds', customerFootprintIds)
    const parameters = { customerFootprintIds }
    dispatch({
      type: `${owner.type}/removeCustomerFootprintList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'customerFootprint' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'customerFootprint', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'用户历程列表' } })
  ...
  render() {
    ...
    const {CustomerFootprintTable} = GlobalComponents;
    const {CustomerFootprintModalTable} = GlobalComponents;
    const {CustomerFootprintSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CustomerFootprintModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 用户历程列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CustomerFootprintSearchForm {...this.props} />
            </div>
...
export default Form.create()(CustomerFootprintSearch)
