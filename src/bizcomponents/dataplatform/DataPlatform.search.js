...
import styles from './DataPlatform.search.less'
...
class DataPlatformSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const dataPlatformIds = selectedRows.map((item) => { return item.id })
    console.log('dataPlatformIds', dataPlatformIds)
    const parameters = { dataPlatformIds }
    dispatch({
      type: `${owner.type}/removeDataPlatformList`,
      payload: { id: owner.id, type: 'dataPlatform', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'dataPlatform' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'dataPlatform', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'数据平台列表' } })
  ...
  render() {
    ...
    const {DataPlatformTable} = GlobalComponents;
    const {DataPlatformConfirmationTable} = GlobalComponents;
    const {DataPlatformSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DataPlatformConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 数据平台列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DataPlatformSearchForm {...this.props} />
            </div>
...
export default Form.create()(DataPlatformSearch)
