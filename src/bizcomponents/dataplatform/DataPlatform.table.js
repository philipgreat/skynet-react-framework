...
const columns = [
  { title: 'ID', debugtype: 'string', dataIndex: 'id',  render: (text, record)=>(<Link to={`/dataPlatform/${text}/dashboard`}>{text}</Link>) },
  { title: '名称', debugtype: 'string', dataIndex: 'name',},
  { title: '创始人', debugtype: 'string', dataIndex: 'founder',},
  { title: '成立', dataIndex: 'founded', render: (text, record) => moment(record.founded).format('YYYY-MM-DD HH:mm') },
  { title: '描述', debugtype: 'string', dataIndex: 'description',},


]

class DataPlatformTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
export default DataPlatformTable
