...
import styles from './DeliverMethod.createform.less'
import {mapBackToImageValues, mapFromImageValues} from '../../axios/tools'
import GlobalComponents from '../../custcomponents';
import DeliverMethodBase from './DeliverMethod.base'
...
class DeliverMethodCreateForm extends Component {
  state = {
  ...
  executeCandidateBookSharingPlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = DeliverMethodService.requestCandidateBookSharingPlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = DeliverMethodBase
    const submitCreateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addDeliverMethod`,
          payload: { id: owner.id, type: 'deliverMethod', parameters },
        })
      })
    }
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addDeliverMethod`,
          payload: { id: owner.id, type: 'deliverMethod', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'deliverMethod',listName:'共享方式列表' },
      })
    }
...
}))(Form.create()(DeliverMethodCreateForm))
