...
import styles from './DeliverMethod.dashboard.less'
...
const internalImageListOf = (deliverMethod) =>{
...
const internalSettingListOf = (deliverMethod) =>{
...
const internalLargeTextOf = (deliverMethod) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = DeliverMethodService[candidateServiceName] 
...
const executeTrans = (deliverMethod,targetComponent) =>{
...
const buildTransferModal = (deliverMethod,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(deliverMethod,targetComponent)}
...
const internalRenderExtraHeader = (deliverMethod) =>{
	return null
}
const internalRenderExtraFooter = (deliverMethod) =>{
	return null
}
...
const internalSummaryOf = (deliverMethod,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{deliverMethod.id}</Description> 
<Description term="名称">{deliverMethod.name}</Description> 
<Description term="代码">{deliverMethod.code}</Description> 
	
        {buildTransferModal(deliverMethod,targetComponent)}
      </DescriptionList>
	)
...
class DeliverMethodDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, bookCopySharingApplicationAccessiable, bookCopySharingApplicationCount } = this.props.deliverMethod
...
export default connect(state => ({
  deliverMethod: state._deliverMethod,
}))(Form.create()(DeliverMethodDashboard))
