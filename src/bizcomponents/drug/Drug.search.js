...
import styles from './Drug.search.less'
...
class DrugSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const drugIds = selectedRows.map((item) => { return item.id })
    console.log('drugIds', drugIds)
    const parameters = { drugIds }
    dispatch({
      type: `${owner.type}/removeDrugList`,
      payload: { id: owner.id, type: 'drug', parameters },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'drug', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'药物列表' } })
  ...
  render() {
    ...
    const {DrugTable} = GlobalComponents;
    const {DrugConfirmationTable} = GlobalComponents;
    const {DrugSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DrugConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 药物列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DrugSearchForm {...this.props} />
            </div>
...
export default Form.create()(DrugSearch)
