...
import styles from './DrugApplication.search.less'
...
class DrugApplicationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const drugApplicationIds = selectedRows.map((item) => { return item.id })
    console.log('drugApplicationIds', drugApplicationIds)
    const parameters = { drugApplicationIds }
    dispatch({
      type: `${owner.type}/removeDrugApplicationList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'drugApplication' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'drugApplication', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'药物申请列表' } })
  ...
  render() {
    ...
    const {DrugApplicationTable} = GlobalComponents;
    const {DrugApplicationConfirmationTable} = GlobalComponents;
    const {DrugApplicationSearchForm} = GlobalComponents;
...
<Menu.Item key="applicationType">关联申请类型</Menu.Item>
<Menu.Item key="drug">关联药物</Menu.Item>
<Menu.Item key="drugType">关联药物类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DrugApplicationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 药物申请列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DrugApplicationSearchForm {...this.props} />
            </div>
...
export default Form.create()(DrugApplicationSearch)
