...
import styles from './DrugApplication.updateform.less'
...
class DrugApplicationUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        applicationDate: moment(item.applicationDate),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const drugApplicationId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, drugApplicationId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateDrugApplication`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextDrugApplicationUpdateRow`,
        payload: {
          id: owner.id,
          type: 'drugApplication',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新药物申请"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新药物申请"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.applicationNumber} {...formItemLayout}>
                  {getFieldDecorator('applicationNumber', {
                    initialValue: selectedRow.applicationNumber,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.applicationDate} {...formItemLayout}>
                  {getFieldDecorator('applicationDate', {
                    initialValue: selectedRow.applicationDate,
...
}))(Form.create()(DrugApplicationUpdateForm))
