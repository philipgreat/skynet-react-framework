...
import styles from './DrugType.createform.less'
...
class DrugTypeCreateForm extends Component {
  state = {
  ...
  executeCandidatePlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = DrugTypeService.requestCandidatePlatform("dataPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addDrugType`,
          payload: { id: owner.id, type: 'drugType', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'drugType',listName:'药物类型列表' },
      })
    }
...
}))(Form.create()(DrugTypeCreateForm))
