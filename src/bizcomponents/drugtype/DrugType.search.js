...
import styles from './DrugType.search.less'
...
class DrugTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const drugTypeIds = selectedRows.map((item) => { return item.id })
    console.log('drugTypeIds', drugTypeIds)
    const parameters = { drugTypeIds }
    dispatch({
      type: `${owner.type}/removeDrugTypeList`,
      payload: { id: owner.id, type: 'drugType', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'drugType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'drugType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'药物类型列表' } })
  ...
  render() {
    ...
    const {DrugTypeTable} = GlobalComponents;
    const {DrugTypeConfirmationTable} = GlobalComponents;
    const {DrugTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DrugTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 药物类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DrugTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(DrugTypeSearch)
