...
import styles from './EmployeeDrivingLicense.preference.less'
...
const internalImageListOf = (employeeDrivingLicense) =>{
...
const internalSettingListOf = (employeeDrivingLicense) =>{
...
const internalLargeTextOf = (employeeDrivingLicense) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeDrivingLicenseService[candidateServiceName] 
...
const executeTrans = (employeeDrivingLicense,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=employeeDrivingLicense.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_employeeDrivingLicense/doJob",payload: payload})
...
const buildTransferModal = (employeeDrivingLicense,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeDrivingLicense,targetComponent)}
...
const internalRenderExtraHeader = (employeeDrivingLicense) =>{
	return null
}
const internalRenderExtraFooter = (employeeDrivingLicense) =>{
	return null
}
...
const internalSummaryOf = (employeeDrivingLicense,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{employeeDrivingLicense.id}</Description> 
<Description term="员工">{employeeDrivingLicense.employee==null?"未分配":employeeDrivingLicense.employee.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"员工","vehicleServiceCompanyEmployee","requestCandidateEmployee",
	      "transferToAnotherEmployee","anotherEmployeeId",employeeDrivingLicense.employee?employeeDrivingLicense.employee.id:"")} 
...
<Description term="有效期至">{ moment(employeeDrivingLicense.expirationDate).format('YYYY-MM-DD')}</Description> 
        ...
        {buildTransferModal(employeeDrivingLicense,targetComponent)}
      </DescriptionList>
	)
...
class EmployeeDrivingLicensePreference extends Component {
...
}))(Form.create()(EmployeeDrivingLicensePreference))
