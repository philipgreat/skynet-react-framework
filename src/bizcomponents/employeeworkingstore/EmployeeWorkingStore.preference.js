...
import styles from './EmployeeWorkingStore.preference.less'
...
const internalImageListOf = (employeeWorkingStore) =>{
...
const internalSettingListOf = (employeeWorkingStore) =>{
...
const internalLargeTextOf = (employeeWorkingStore) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeWorkingStoreService[candidateServiceName] 
...
const executeTrans = (employeeWorkingStore,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=employeeWorkingStore.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_employeeWorkingStore/doJob",payload: payload})
...
const buildTransferModal = (employeeWorkingStore,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeWorkingStore,targetComponent)}
...
const internalRenderExtraHeader = (employeeWorkingStore) =>{
	return null
}
const internalRenderExtraFooter = (employeeWorkingStore) =>{
	return null
}
...
const internalSummaryOf = (employeeWorkingStore,targetComponent) =>{
...
<Description term="员工">{employeeWorkingStore.employee==null?"未分配":employeeWorkingStore.employee.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"员工","employee","requestCandidateEmployee",
	      "transferToAnotherEmployee","anotherEmployeeId",employeeWorkingStore.employee?employeeWorkingStore.employee.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务网点">{employeeWorkingStore.store==null?"未分配":employeeWorkingStore.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务网点","store","requestCandidateStore",
	      "transferToAnotherStore","anotherStoreId",employeeWorkingStore.store?employeeWorkingStore.store.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="终止日期">{ moment(employeeWorkingStore.terminatedDate).format('YYYY-MM-DD')}</Description> 
        ...
        {buildTransferModal(employeeWorkingStore,targetComponent)}
      </DescriptionList>
	)
...
class EmployeeWorkingStorePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"员工工作的网点",cardsFor: "employeeWorkingStore",cardsSource: this.props.employeeWorkingStore,
  		subItems: [
...
}))(Form.create()(EmployeeWorkingStorePreference))
