...
class EmployeeWorkingStoreSearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'员工工作的网点列表' } })
  ...
  render() {
    ...
    const {EmployeeWorkingStoreModalTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeWorkingStoreModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工工作的网点列表`}>
        <Card bordered={false}>
