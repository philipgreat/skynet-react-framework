...
import styles from './FileInspectionPlateNumberPattern.preference.less'
...
const internalImageListOf = (fileInspectionPlateNumberPattern) =>{
...
const internalSettingListOf = (fileInspectionPlateNumberPattern) =>{
...
const internalLargeTextOf = (fileInspectionPlateNumberPattern) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = FileInspectionPlateNumberPatternService[candidateServiceName] 
...
const executeTrans = (fileInspectionPlateNumberPattern,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=fileInspectionPlateNumberPattern.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_fileInspectionPlateNumberPattern/doJob",payload: payload})
...
const buildTransferModal = (fileInspectionPlateNumberPattern,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(fileInspectionPlateNumberPattern,targetComponent)}
...
const internalRenderExtraHeader = (fileInspectionPlateNumberPattern) =>{
	return null
}
const internalRenderExtraFooter = (fileInspectionPlateNumberPattern) =>{
	return null
}
...
const internalSummaryOf = (fileInspectionPlateNumberPattern,targetComponent) =>{
...
<Description term="商户">{fileInspectionPlateNumberPattern.company==null?"未分配":fileInspectionPlateNumberPattern.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",fileInspectionPlateNumberPattern.company?fileInspectionPlateNumberPattern.company.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(fileInspectionPlateNumberPattern,targetComponent)}
      </DescriptionList>
	)
...
class FileInspectionPlateNumberPatternPreference extends Component {
...
}))(Form.create()(FileInspectionPlateNumberPatternPreference))
