...
import styles from './FormAction.preference.less'
...
const internalImageListOf = (formAction) =>{
...
const internalSettingListOf = (formAction) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = FormActionService[candidateServiceName] 
...
const executeTrans = (formAction,targetComponent) =>{
...
const buildTransferModal = (formAction,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(formAction,targetComponent)}
...
const internalRenderExtraHeader = (formAction) =>{
	return null
}
const internalRenderExtraFooter = (formAction) =>{
	return null
}
...
class FormActionPreference extends Component {
...
}))(Form.create()(FormActionPreference))
