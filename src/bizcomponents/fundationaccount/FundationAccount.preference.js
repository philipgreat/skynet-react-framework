...
import styles from './FundationAccount.preference.less'
...
const internalImageListOf = (fundationAccount) =>{
...
const internalSettingListOf = (fundationAccount) =>{
...
const internalLargeTextOf = (fundationAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = FundationAccountService[candidateServiceName] 
...
const executeTrans = (fundationAccount,targetComponent) =>{
...
const buildTransferModal = (fundationAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(fundationAccount,targetComponent)}
...
const internalRenderExtraHeader = (fundationAccount) =>{
	return null
}
const internalRenderExtraFooter = (fundationAccount) =>{
	return null
}
...
const internalSummaryOf = (fundationAccount,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{fundationAccount.id}</Description> 
<Description term="名称">{fundationAccount.name}</Description> 
<Description term="账户余额">{fundationAccount.amount}</Description> 
<Description term="帐户数据">{fundationAccount.accountData==null?"未分配":fundationAccount.accountData.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"帐户数据","accountData","requestCandidateAccountData",
	      "transferToAnotherAccountData","anotherAccountDataId",fundationAccount.accountData?fundationAccount.accountData.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(fundationAccount,targetComponent)}
      </DescriptionList>
	)
...
class FundationAccountPreference extends Component {
...
}))(Form.create()(FundationAccountPreference))
