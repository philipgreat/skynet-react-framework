...
import FundationAccountDetailsBase from './FundationAccountDetails.base'
...
class FundationAccountDetailsCreateForm extends Component {
  ...
  executeCandidateTransactionTypeSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = FundationAccountDetailsService.requestCandidateTransactionType("transactionType", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = FundationAccountDetailsBase
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'fundationAccountDetails',listName:'平台基金账户明细列表' },
      })
    }
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.fundationAccount} {...formItemLayout}>
                  {getFieldDecorator('fundationAccountId', {
                    ...
                    rules: [{ required: true, message: '请输入平台基金账户' }],
                  })(
