...
import styles from './FundationAccountDetails.preference.less'
...
const internalImageListOf = (fundationAccountDetails) =>{
...
const internalSettingListOf = (fundationAccountDetails) =>{
...
const internalLargeTextOf = (fundationAccountDetails) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = FundationAccountDetailsService[candidateServiceName] 
...
const executeTrans = (fundationAccountDetails,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=fundationAccountDetails.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_fundationAccountDetails/doJob",payload: payload})
...
const buildTransferModal = (fundationAccountDetails,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(fundationAccountDetails,targetComponent)}
...
const internalRenderExtraHeader = (fundationAccountDetails) =>{
	return null
}
const internalRenderExtraFooter = (fundationAccountDetails) =>{
	return null
}
...
const internalSummaryOf = (fundationAccountDetails,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{fundationAccountDetails.id}</Description> 
<Description term="摘要">{fundationAccountDetails.summary}</Description> 
<Description term="账户余额">{fundationAccountDetails.amount}</Description> 
<Description term="交易类型">{fundationAccountDetails.transactionType==null?"未分配":fundationAccountDetails.transactionType.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"交易类型","transactionType","requestCandidateTransactionType",
	      "transferToAnotherTransactionType","anotherTransactionTypeId",fundationAccountDetails.transactionType?fundationAccountDetails.transactionType.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="平台基金账户">{fundationAccountDetails.fundationAccount==null?"未分配":fundationAccountDetails.fundationAccount.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"平台基金账户","fundationAccount","requestCandidateFundationAccount",
	      "transferToAnotherFundationAccount","anotherFundationAccountId",fundationAccountDetails.fundationAccount?fundationAccountDetails.fundationAccount.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="相关订单">{fundationAccountDetails.relatedMainOrder==null?"未分配":fundationAccountDetails.relatedMainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"相关订单","mainOrder","requestCandidateRelatedMainOrder",
	      "transferToAnotherRelatedMainOrder","anotherRelatedMainOrderId",fundationAccountDetails.relatedMainOrder?fundationAccountDetails.relatedMainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="日期时间">{ moment(fundationAccountDetails.datetime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(fundationAccountDetails,targetComponent)}
      </DescriptionList>
	)
...
class FundationAccountDetailsPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"平台基金账户明细",cardsFor: "fundationAccountDetails",cardsSource: this.props.fundationAccountDetails,
  		subItems: [
...
}))(Form.create()(FundationAccountDetailsPreference))
