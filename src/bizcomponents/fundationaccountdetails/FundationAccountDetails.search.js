...
class FundationAccountDetailsSearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'平台基金账户明细列表' } })
  ...
  render() {
    ...
    const {FundationAccountDetailsModalTable} = GlobalComponents;
...
<Menu.Item key="fundationAccount">关联平台基金账户</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <FundationAccountDetailsModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 平台基金账户明细列表`}>
        <Card bordered={false}>
