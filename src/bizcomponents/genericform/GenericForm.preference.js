...
import styles from './GenericForm.preference.less'
...
const internalImageListOf = (genericForm) =>{
...
const internalSettingListOf = (genericForm) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = GenericFormService[candidateServiceName] 
...
const executeTrans = (genericForm,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_genericForm/doJob",payload: payload})
...
const buildTransferModal = (genericForm,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(genericForm,targetComponent)}
...
const internalRenderExtraHeader = (genericForm) =>{
	return null
}
const internalRenderExtraFooter = (genericForm) =>{
	return null
}
...
class GenericFormPreference extends Component {
...
}))(Form.create()(GenericFormPreference))
