...
import styles from './HandOverChecklistItem.preference.less'
...
const internalImageListOf = (handOverChecklistItem) =>{
...
const internalSettingListOf = (handOverChecklistItem) =>{
...
const internalLargeTextOf = (handOverChecklistItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = HandOverChecklistItemService[candidateServiceName] 
...
const executeTrans = (handOverChecklistItem,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=handOverChecklistItem.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_handOverChecklistItem/doJob",payload: payload})
...
const buildTransferModal = (handOverChecklistItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(handOverChecklistItem,targetComponent)}
...
const internalRenderExtraHeader = (handOverChecklistItem) =>{
	return null
}
const internalRenderExtraFooter = (handOverChecklistItem) =>{
	return null
}
...
const internalSummaryOf = (handOverChecklistItem,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{handOverChecklistItem.id}</Description> 
<Description term="检查问题">{handOverChecklistItem.question==null?"未分配":handOverChecklistItem.question.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"检查问题","availableHandOverItem","requestCandidateQuestion",
	      "transferToAnotherQuestion","anotherQuestionId",handOverChecklistItem.question?handOverChecklistItem.question.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="年检订单">{handOverChecklistItem.mainOrder==null?"未分配":handOverChecklistItem.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",handOverChecklistItem.mainOrder?handOverChecklistItem.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(handOverChecklistItem,targetComponent)}
      </DescriptionList>
	)
...
class HandOverChecklistItemPreference extends Component {
...
}))(Form.create()(HandOverChecklistItemPreference))
