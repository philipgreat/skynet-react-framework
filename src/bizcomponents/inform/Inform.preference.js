...
import styles from './Inform.preference.less'
...
const internalImageListOf = (inform) =>{
...
const internalSettingListOf = (inform) =>{
...
const internalLargeTextOf = (inform) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = InformService[candidateServiceName] 
...
const executeTrans = (inform,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_inform/doJob",payload: payload})
...
const buildTransferModal = (inform,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(inform,targetComponent)}
...
const internalRenderExtraHeader = (inform) =>{
	return null
}
const internalRenderExtraFooter = (inform) =>{
	return null
}
...
const internalSummaryOf = (inform,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{inform.id}</Description> 
<Description term="举报人">{inform.informer==null?"未分配":inform.informer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"举报人","customer","requestCandidateInformer",
	      "transferToAnotherInformer","anotherInformerId",inform.informer?inform.informer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="活动评论">{inform.campaignReview==null?"未分配":inform.campaignReview.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"活动评论","campaignReview","requestCandidateCampaignReview",
	      "transferToAnotherCampaignReview","anotherCampaignReviewId",inform.campaignReview?inform.campaignReview.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="书评">{inform.bookReview==null?"未分配":inform.bookReview.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"书评","bookReview","requestCandidateBookReview",
	      "transferToAnotherBookReview","anotherBookReviewId",inform.bookReview?inform.bookReview.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="创建时间">{ moment(inform.creationTime).format('YYYY-MM-DD')}</Description> 
<Description term="评论">{inform.comments}</Description> 
	
        {buildTransferModal(inform,targetComponent)}
      </DescriptionList>
	)
...
class InformPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.inform
    const cardsData = {cardsName:"举报",cardsFor: "inform",cardsSource: this.props.inform,
  		subItems: [
...
export default connect(state => ({
  inform: state._inform,
}))(Form.create()(InformPreference))
