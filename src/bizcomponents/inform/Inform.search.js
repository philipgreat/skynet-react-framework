...
import styles from './Inform.search.less'
...
class InformSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const informIds = selectedRows.map((item) => { return item.id })
    console.log('informIds', informIds)
    const parameters = { informIds }
    dispatch({
      type: `${owner.type}/removeInformList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'inform' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'inform', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'举报列表' } })
  ...
  render() {
    ...
    const {InformTable} = GlobalComponents;
    const {InformModalTable} = GlobalComponents;
    const {InformSearchForm} = GlobalComponents;
...
<Menu.Item key="informer">关联举报人</Menu.Item>
<Menu.Item key="campaignReview">关联活动评论</Menu.Item>
<Menu.Item key="bookReview">关联书评</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <InformModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 举报列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <InformSearchForm {...this.props} />
            </div>
...
export default Form.create()(InformSearch)
