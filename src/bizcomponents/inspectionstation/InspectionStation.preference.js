...
import styles from './InspectionStation.preference.less'
...
const internalImageListOf = (inspectionStation) =>{
...
const internalSettingListOf = (inspectionStation) =>{
...
const internalLargeTextOf = (inspectionStation) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = InspectionStationService[candidateServiceName] 
...
const executeTrans = (inspectionStation,targetComponent) =>{
...
const buildTransferModal = (inspectionStation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(inspectionStation,targetComponent)}
...
const internalRenderExtraHeader = (inspectionStation) =>{
	return null
}
const internalRenderExtraFooter = (inspectionStation) =>{
	return null
}
...
const internalSummaryOf = (inspectionStation,targetComponent) =>{
...
<Description term="所在城市">{inspectionStation.addressCity==null?"未分配":inspectionStation.addressCity.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"所在城市","city","requestCandidateAddressCity",
	      "transferToAnotherAddressCity","anotherAddressCityId",inspectionStation.addressCity?inspectionStation.addressCity.id:"")} 
...
<Description term="联系人手机">{inspectionStation.contactMobile}</Description> 
        ...
        {buildTransferModal(inspectionStation,targetComponent)}
      </DescriptionList>
	)
...
class InspectionStationPreference extends Component {
...
}))(Form.create()(InspectionStationPreference))
