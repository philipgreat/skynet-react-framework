...
import styles from './InsuranceServiceAccount.preference.less'
...
const internalImageListOf = (insuranceServiceAccount) =>{
...
const internalSettingListOf = (insuranceServiceAccount) =>{
...
const internalLargeTextOf = (insuranceServiceAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = InsuranceServiceAccountService[candidateServiceName] 
...
const executeTrans = (insuranceServiceAccount,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=insuranceServiceAccount.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_insuranceServiceAccount/doJob",payload: payload})
...
const buildTransferModal = (insuranceServiceAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(insuranceServiceAccount,targetComponent)}
...
const internalRenderExtraHeader = (insuranceServiceAccount) =>{
	return null
}
const internalRenderExtraFooter = (insuranceServiceAccount) =>{
	return null
}
...
const internalSummaryOf = (insuranceServiceAccount,targetComponent) =>{
...
<Description term="商户">{insuranceServiceAccount.merchant==null?"未分配":insuranceServiceAccount.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",insuranceServiceAccount.merchant?insuranceServiceAccount.merchant.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务人员">{insuranceServiceAccount.responsibleWorker==null?"未分配":insuranceServiceAccount.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",insuranceServiceAccount.responsibleWorker?insuranceServiceAccount.responsibleWorker.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="对账单">{insuranceServiceAccount.account==null?"未分配":insuranceServiceAccount.account.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"对账单","account","requestCandidateAccount",
	      "transferToAnotherAccount","anotherAccountId",insuranceServiceAccount.account?insuranceServiceAccount.account.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(insuranceServiceAccount,targetComponent)}
      </DescriptionList>
	)
...
class InsuranceServiceAccountPreference extends Component {
...
}))(Form.create()(InsuranceServiceAccountPreference))
