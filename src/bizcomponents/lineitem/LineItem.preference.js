...
const internalImageListOf = (lineItem) =>{
  ...
  const imageList = [
	   {"title":'封面图片',"imageLocation":lineItem.coverImage},
]
...
<Description term="摘要">{lineItem.summary}</Description> 
<Description term="商品类型">{lineItem.skuType}</Description> 
<Description term="商品ID">{lineItem.skuId}</Description> 
<Description term="商品链接">{lineItem.skuLink}</Description> 
<Description term="原价">{lineItem.rawPrice}</Description> 
<Description term="折扣">{lineItem.itemDiscount}</Description> 
<Description term="主订单">{lineItem.mainOrder==null?"未分配":lineItem.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"主订单","mainOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",lineItem.mainOrder?lineItem.mainOrder.id:"")} 
...
class LineItemPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"订单项",cardsFor: "lineItem",cardsSource: this.props.lineItem,
  		subItems: [
