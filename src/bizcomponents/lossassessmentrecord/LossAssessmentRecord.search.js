...
import styles from './LossAssessmentRecord.search.less'
...
class LossAssessmentRecordSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const lossAssessmentRecordIds = selectedRows.map((item) => { return item.id })
    console.log('lossAssessmentRecordIds', lossAssessmentRecordIds)
    const parameters = { lossAssessmentRecordIds }
    dispatch({
      type: `${owner.type}/removeLossAssessmentRecordList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'lossAssessmentRecord' },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'定损记录列表' } })
  ...
  render() {
    ...
    const {LossAssessmentRecordTable} = GlobalComponents;
    const {LossAssessmentRecordModalTable} = GlobalComponents;
    const {LossAssessmentRecordSearchForm} = GlobalComponents;
...
<Menu.Item key="lossDiscount">关联定损折扣</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <LossAssessmentRecordModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <LossAssessmentRecordSearchForm {...this.props} />
            </div>
...
export default Form.create()(LossAssessmentRecordSearch)
