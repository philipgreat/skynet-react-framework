...
import styles from './LossAssessmentRecord.updateform.less'
...
class LossAssessmentRecordUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        recordTime: moment(item.recordTime),
  ...
  render() {
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextLossAssessmentRecordUpdateRow`,
        payload: {
          id: owner.id,
          type: 'lossAssessmentRecord',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新定损记录"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新定损记录"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.lossComment} {...formItemLayout}>
                  {getFieldDecorator('lossComment', {
                    initialValue: selectedRow.lossComment,
...
}))(Form.create()(LossAssessmentRecordUpdateForm))
