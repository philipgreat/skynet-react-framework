...
import styles from './LossDiscount.dashboard.less'
...
const internalImageListOf = (lossDiscount) =>{
...
const internalSettingListOf = (lossDiscount) =>{
...
const internalLargeTextOf = (lossDiscount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LossDiscountService[candidateServiceName] 
...
const executeTrans = (lossDiscount,targetComponent) =>{
...
const buildTransferModal = (lossDiscount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(lossDiscount,targetComponent)}
...
const internalRenderExtraHeader = (lossDiscount) =>{
	return null
}
const internalRenderExtraFooter = (lossDiscount) =>{
	return null
}
...
const internalSummaryOf = (lossDiscount,targetComponent) =>{
...
class LossDiscountDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, lossAssessmentRecordAccessiable, lossAssessmentRecordCount } = this.props.lossDiscount
...
}))(Form.create()(LossDiscountDashboard))
