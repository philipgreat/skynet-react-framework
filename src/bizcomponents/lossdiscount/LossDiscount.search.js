...
import styles from './LossDiscount.search.less'
...
class LossDiscountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const lossDiscountIds = selectedRows.map((item) => { return item.id })
    console.log('lossDiscountIds', lossDiscountIds)
    const parameters = { lossDiscountIds }
    dispatch({
      type: `${owner.type}/removeLossDiscountList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'lossDiscount' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'lossDiscount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'损失的折扣列表' } })
  ...
  render() {
    ...
    const {LossDiscountTable} = GlobalComponents;
    const {LossDiscountModalTable} = GlobalComponents;
    const {LossDiscountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <LossDiscountModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 损失的折扣列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <LossDiscountSearchForm {...this.props} />
            </div>
...
export default Form.create()(LossDiscountSearch)
