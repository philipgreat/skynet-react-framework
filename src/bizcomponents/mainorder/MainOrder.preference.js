...
import styles from './MainOrder.preference.less'
...
const internalImageListOf = (mainOrder) =>{
...
const internalSettingListOf = (mainOrder) =>{
...
const internalLargeTextOf = (mainOrder) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MainOrderService[candidateServiceName] 
...
const executeTrans = (mainOrder,targetComponent) =>{
...
const buildTransferModal = (mainOrder,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(mainOrder,targetComponent)}
...
const internalRenderExtraHeader = (mainOrder) =>{
	return null
}
const internalRenderExtraFooter = (mainOrder) =>{
	return null
}
...
const internalSummaryOf = (mainOrder,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{mainOrder.id}</Description> 
<Description term="标题">{mainOrder.title}</Description> 
<Description term="主订单状态">{mainOrder.mainOrderStatus}</Description> 
<Description term="创建时间">{ moment(mainOrder.createTime).format('YYYY-MM-DD')}</Description> 
<Description term="最后更新时间">{ moment(mainOrder.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
<Description term="用户">{mainOrder.customer==null?"未分配":mainOrder.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"用户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",mainOrder.customer?mainOrder.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="订单总价">{mainOrder.originalAmount}</Description> 
<Description term="实付款金额">{mainOrder.actualAmount}</Description> 
	
        {buildTransferModal(mainOrder,targetComponent)}
      </DescriptionList>
	)
...
class MainOrderPreference extends Component {
...
}))(Form.create()(MainOrderPreference))
