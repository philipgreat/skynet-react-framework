...
import styles from './MainOrderAccount.preference.less'
...
const internalImageListOf = (mainOrderAccount) =>{
...
const internalSettingListOf = (mainOrderAccount) =>{
...
const internalLargeTextOf = (mainOrderAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MainOrderAccountService[candidateServiceName] 
...
const executeTrans = (mainOrderAccount,targetComponent) =>{
...
const buildTransferModal = (mainOrderAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(mainOrderAccount,targetComponent)}
...
const internalRenderExtraHeader = (mainOrderAccount) =>{
	return null
}
const internalRenderExtraFooter = (mainOrderAccount) =>{
	return null
}
...
const internalSummaryOf = (mainOrderAccount,targetComponent) =>{
...
<Description term="对账单">{mainOrderAccount.account==null?"未分配":mainOrderAccount.account.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"对账单","account","requestCandidateAccount",
	      "transferToAnotherAccount","anotherAccountId",mainOrderAccount.account?mainOrderAccount.account.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(mainOrderAccount,targetComponent)}
      </DescriptionList>
	)
...
class MainOrderAccountPreference extends Component {
...
}))(Form.create()(MainOrderAccountPreference))
