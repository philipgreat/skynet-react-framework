...
import styles from './MainOrderPayment.preference.less'
...
const internalImageListOf = (mainOrderPayment) =>{
...
const internalSettingListOf = (mainOrderPayment) =>{
...
const internalLargeTextOf = (mainOrderPayment) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MainOrderPaymentService[candidateServiceName] 
...
const executeTrans = (mainOrderPayment,targetComponent) =>{
...
const buildTransferModal = (mainOrderPayment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(mainOrderPayment,targetComponent)}
...
const internalRenderExtraHeader = (mainOrderPayment) =>{
	return null
}
const internalRenderExtraFooter = (mainOrderPayment) =>{
	return null
}
...
const internalSummaryOf = (mainOrderPayment,targetComponent) =>{
...
<Description term="支付金额">{mainOrderPayment.paidAmount}</Description> 
<Description term="付款状态">{mainOrderPayment.paymentStatus}</Description> 
<Description term="微信支付交易ID">{mainOrderPayment.wechatTransactionId}</Description> 
<Description term="微信支付ID">{mainOrderPayment.wechatPrepayId}</Description> 
...
<Description term="主订单">{mainOrderPayment.mainOrder==null?"未分配":mainOrderPayment.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"主订单","mainOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",mainOrderPayment.mainOrder?mainOrderPayment.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(mainOrderPayment,targetComponent)}
      </DescriptionList>
	)
...
class MainOrderPaymentPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"主订单支付",cardsFor: "mainOrderPayment",cardsSource: this.props.mainOrderPayment,
  		subItems: [
...
}))(Form.create()(MainOrderPaymentPreference))
