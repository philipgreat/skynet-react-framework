...
import styles from './MemberRightsDisplay.dashboard.less'
...
const internalImageListOf = (memberRightsDisplay) =>{
...
const internalSettingListOf = (memberRightsDisplay) =>{
...
const internalLargeTextOf = (memberRightsDisplay) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberRightsDisplayService[candidateServiceName] 
...
const executeTrans = (memberRightsDisplay,targetComponent) =>{
...
const buildTransferModal = (memberRightsDisplay,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberRightsDisplay,targetComponent)}
...
const internalRenderExtraHeader = (memberRightsDisplay) =>{
	return null
}
const internalRenderExtraFooter = (memberRightsDisplay) =>{
	return null
}
...
const internalSummaryOf = (memberRightsDisplay,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{memberRightsDisplay.id}</Description> 
<Description term="名称">{memberRightsDisplay.name}</Description> 
<Description term="摘要">{memberRightsDisplay.summary}</Description> 
<Description term="协议名称">{memberRightsDisplay.agreementName}</Description> 
<Description term="协议类型">{memberRightsDisplay.agreementType}</Description> 
<Description term="协议Id">{memberRightsDisplay.agreementId}</Description> 
	
        {buildTransferModal(memberRightsDisplay,targetComponent)}
      </DescriptionList>
	)
...
class MemberRightsDisplayDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"会员权利显示",cardsFor: "memberRightsDisplay",cardsSource: this.props.memberRightsDisplay,
  		subItems: [
{name: 'memberRightsDisplayItemList', displayName:'会员权利显示项',type:'memberRightsDisplayItem',count:memberRightsDisplayItemCount,addFunction: true},
...
export default connect(state => ({
  memberRightsDisplay: state._memberRightsDisplay,
}))(Form.create()(MemberRightsDisplayDashboard))
