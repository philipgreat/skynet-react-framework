...
export default {
  ...
  namespace: '_memberRightsDisplay',
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
      	const modelName = 'memberRightsDisplay'
  ...
  effects: {
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._memberRightsDisplay)
      ...
      const data = yield call(MemberRightsDisplayService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {MemberRightsDisplayService} = GlobalComponents;
      yield put({ type: 'showLoading', payload })
      const data = yield call(MemberRightsDisplayService.load, payload.id, payload.parameters)
    ...
    *doJob({ payload }, { call, put }) { 
      ...
      if(!MemberRightsDisplayService[serviceNameToCall]){
      	handleClientError("找不到后台服务: "+serviceNameToCall)
      	return;
      }
      
      const data = yield call(MemberRightsDisplayService[serviceNameToCall], id, parameters)
    ...
    *updateMemberRightsDisplayItem({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(MemberRightsDisplayService.updateMemberRightsDisplayItem, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
