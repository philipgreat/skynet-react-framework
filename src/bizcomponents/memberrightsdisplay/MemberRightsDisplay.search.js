...
import styles from './MemberRightsDisplay.search.less'
...
class MemberRightsDisplaySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberRightsDisplayIds = selectedRows.map((item) => { return item.id })
    console.log('memberRightsDisplayIds', memberRightsDisplayIds)
    const parameters = { memberRightsDisplayIds }
    dispatch({
      type: `${owner.type}/removeMemberRightsDisplayList`,
      payload: { id: owner.id, type: 'memberRightsDisplay', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberRightsDisplay' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberRightsDisplay', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员权利显示列表' } })
  ...
  render() {
    ...
    const {MemberRightsDisplayTable} = GlobalComponents;
    const {MemberRightsDisplayModalTable} = GlobalComponents;
    const {MemberRightsDisplaySearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberRightsDisplayModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员权利显示列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberRightsDisplaySearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberRightsDisplaySearch)
