...
import styles from './MemberRightsDisplayItem.preference.less'
...
const internalImageListOf = (memberRightsDisplayItem) =>{
...
const internalSettingListOf = (memberRightsDisplayItem) =>{
...
const internalLargeTextOf = (memberRightsDisplayItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberRightsDisplayItemService[candidateServiceName] 
...
const executeTrans = (memberRightsDisplayItem,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberRightsDisplayItem.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberRightsDisplayItem/doJob",payload: payload})
...
const buildTransferModal = (memberRightsDisplayItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberRightsDisplayItem,targetComponent)}
...
const internalRenderExtraHeader = (memberRightsDisplayItem) =>{
	return null
}
const internalRenderExtraFooter = (memberRightsDisplayItem) =>{
	return null
}
...
const internalSummaryOf = (memberRightsDisplayItem,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{memberRightsDisplayItem.id}</Description> 
<Description term="图像">{memberRightsDisplayItem.image}</Description> 
<Description term="摘要">{memberRightsDisplayItem.summary}</Description> 
	
        {buildTransferModal(memberRightsDisplayItem,targetComponent)}
      </DescriptionList>
	)
...
class MemberRightsDisplayItemPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.memberRightsDisplayItem
    const cardsData = {cardsName:"会员权利显示项",cardsFor: "memberRightsDisplayItem",cardsSource: this.props.memberRightsDisplayItem,
  		subItems: [
...
export default connect(state => ({
  memberRightsDisplayItem: state._memberRightsDisplayItem,
}))(Form.create()(MemberRightsDisplayItemPreference))
