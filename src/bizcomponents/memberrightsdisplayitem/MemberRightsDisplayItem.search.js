...
import styles from './MemberRightsDisplayItem.search.less'
...
class MemberRightsDisplayItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberRightsDisplayItemIds = selectedRows.map((item) => { return item.id })
    console.log('memberRightsDisplayItemIds', memberRightsDisplayItemIds)
    const parameters = { memberRightsDisplayItemIds }
    dispatch({
      type: `${owner.type}/removeMemberRightsDisplayItemList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberRightsDisplayItem' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberRightsDisplayItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员权利显示项列表' } })
  ...
  render() {
    ...
    const {MemberRightsDisplayItemTable} = GlobalComponents;
    const {MemberRightsDisplayItemModalTable} = GlobalComponents;
    const {MemberRightsDisplayItemSearchForm} = GlobalComponents;
...
<Menu.Item key="memberRights">关联会员的权利</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberRightsDisplayItemModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员权利显示项列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberRightsDisplayItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberRightsDisplayItemSearch)
