...
import styles from './MemberServiceBundleSku.preference.less'
...
const internalImageListOf = (memberServiceBundleSku) =>{
...
const internalSettingListOf = (memberServiceBundleSku) =>{
...
const internalLargeTextOf = (memberServiceBundleSku) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberServiceBundleSkuService[candidateServiceName] 
...
const executeTrans = (memberServiceBundleSku,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberServiceBundleSku.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberServiceBundleSku/doJob",payload: payload})
...
const buildTransferModal = (memberServiceBundleSku,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberServiceBundleSku,targetComponent)}
...
const internalRenderExtraHeader = (memberServiceBundleSku) =>{
	return null
}
const internalRenderExtraFooter = (memberServiceBundleSku) =>{
	return null
}
...
const internalSummaryOf = (memberServiceBundleSku,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{memberServiceBundleSku.id}</Description> 
<Description term="会员服务名称">{memberServiceBundleSku.memberServiceName}</Description> 
<Description term="描述">{memberServiceBundleSku.description}</Description> 
<Description term="定价">{memberServiceBundleSku.listPrice}</Description> 
<Description term="售价">{memberServiceBundleSku.salePrice}</Description> 
<Description term="服务周期(月)">{memberServiceBundleSku.servicePeriodMonths}</Description> 
<Description term="会员产品">{memberServiceBundleSku.memberProduct==null?"未分配":memberServiceBundleSku.memberProduct.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会员产品","memberServiceProduct","requestCandidateMemberProduct",
	      "transferToAnotherMemberProduct","anotherMemberProductId",memberServiceBundleSku.memberProduct?memberServiceBundleSku.memberProduct.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(memberServiceBundleSku,targetComponent)}
      </DescriptionList>
	)
...
class MemberServiceBundleSkuPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.memberServiceBundleSku
    const cardsData = {cardsName:"会员服务包",cardsFor: "memberServiceBundleSku",cardsSource: this.props.memberServiceBundleSku,
  		subItems: [
...
export default connect(state => ({
  memberServiceBundleSku: state._memberServiceBundleSku,
}))(Form.create()(MemberServiceBundleSkuPreference))
