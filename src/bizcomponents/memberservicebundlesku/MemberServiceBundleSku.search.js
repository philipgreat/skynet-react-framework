...
import styles from './MemberServiceBundleSku.search.less'
...
class MemberServiceBundleSkuSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberServiceBundleSkuIds = selectedRows.map((item) => { return item.id })
    console.log('memberServiceBundleSkuIds', memberServiceBundleSkuIds)
    const parameters = { memberServiceBundleSkuIds }
    dispatch({
      type: `${owner.type}/removeMemberServiceBundleSkuList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceBundleSku' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceBundleSku', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员服务包列表' } })
  ...
  render() {
    ...
    const {MemberServiceBundleSkuTable} = GlobalComponents;
    const {MemberServiceBundleSkuModalTable} = GlobalComponents;
    const {MemberServiceBundleSkuSearchForm} = GlobalComponents;
...
<Menu.Item key="memberProduct">关联会员产品</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberServiceBundleSkuModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员服务包列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberServiceBundleSkuSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberServiceBundleSkuSearch)
