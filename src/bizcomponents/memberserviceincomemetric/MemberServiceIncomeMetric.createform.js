...
import styles from './MemberServiceIncomeMetric.createform.less'
...
class MemberServiceIncomeMetricCreateForm extends Component {
  state = {
  ...
  executeCandidateBookPlazaSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = MemberServiceIncomeMetricService.requestCandidateBookPlaza("bookPlaza", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          ...
          payload: { id: owner.id, type: 'memberServiceIncomeMetric', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'memberServiceIncomeMetric',listName:'会员服务费收入分成配置列表' },
      })
    }
    ...
    return (
      <PageHeaderLayout
        title="新建一个会员服务费收入分成配置"
        content="新建一个会员服务费收入分成配置"
...
}))(Form.create()(MemberServiceIncomeMetricCreateForm))
