...
import styles from './MemberServiceIncomeMetric.preference.less'
...
const internalImageListOf = (memberServiceIncomeMetric) =>{
...
const internalSettingListOf = (memberServiceIncomeMetric) =>{
...
const internalLargeTextOf = (memberServiceIncomeMetric) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {MemberServiceIncomeMetricService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberServiceIncomeMetricService[candidateServiceName] 
...
const executeTrans = (memberServiceIncomeMetric,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberServiceIncomeMetric.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberServiceIncomeMetric/doJob",payload: payload})
...
const buildTransferModal = (memberServiceIncomeMetric,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberServiceIncomeMetric,targetComponent)}
...
const internalRenderExtraHeader = (memberServiceIncomeMetric) =>{
	return null
}
const internalRenderExtraFooter = (memberServiceIncomeMetric) =>{
	return null
}
...
const internalSummaryOf = (memberServiceIncomeMetric,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{memberServiceIncomeMetric.id}</Description> 
<Description term="服务网点分成比">{memberServiceIncomeMetric.storeRate}</Description> 
<Description term="平台分成比">{memberServiceIncomeMetric.platformRate}</Description> 
<Description term="图书天地">{memberServiceIncomeMetric.bookPlaza==null?"未分配":memberServiceIncomeMetric.bookPlaza.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"图书天地","bookPlaza","requestCandidateBookPlaza",
	      "transferToAnotherBookPlaza","anotherBookPlazaId",memberServiceIncomeMetric.bookPlaza?memberServiceIncomeMetric.bookPlaza.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(memberServiceIncomeMetric,targetComponent)}
      </DescriptionList>
	)
...
class MemberServiceIncomeMetricPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.memberServiceIncomeMetric
    const cardsData = {cardsName:"会员服务费收入分成配置",cardsFor: "memberServiceIncomeMetric",cardsSource: this.props.memberServiceIncomeMetric,
  		subItems: [
...
export default connect(state => ({
  memberServiceIncomeMetric: state._memberServiceIncomeMetric,
}))(Form.create()(MemberServiceIncomeMetricPreference))
