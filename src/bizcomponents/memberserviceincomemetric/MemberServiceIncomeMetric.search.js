...
import styles from './MemberServiceIncomeMetric.search.less'
...
class MemberServiceIncomeMetricSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberServiceIncomeMetricIds = selectedRows.map((item) => { return item.id })
    console.log('memberServiceIncomeMetricIds', memberServiceIncomeMetricIds)
    const parameters = { memberServiceIncomeMetricIds }
    dispatch({
      type: `${owner.type}/removeMemberServiceIncomeMetricList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceIncomeMetric' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceIncomeMetric', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员服务费收入分成配置列表' } })
  ...
  render() {
    ...
    const {MemberServiceIncomeMetricTable} = GlobalComponents;
    const {MemberServiceIncomeMetricModalTable} = GlobalComponents;
    const {MemberServiceIncomeMetricSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberServiceIncomeMetricModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员服务费收入分成配置列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberServiceIncomeMetricSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberServiceIncomeMetricSearch)
