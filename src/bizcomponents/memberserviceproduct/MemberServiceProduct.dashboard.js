...
const internalImageListOf = (memberServiceProduct) =>{

  const imageList = [
	   {"title":'产品封面图片',"imageLocation":memberServiceProduct.productCoverImage},
]
...
const internalSettingListOf = (memberServiceProduct) =>{
...
const internalLargeTextOf = (memberServiceProduct) =>{

	return(<div> 
   <Card title={`产品描述`} ><pre>{memberServiceProduct.productDescription}</pre></Card>
</div>)
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberServiceProductService[candidateServiceName] 
...
const executeTrans = (memberServiceProduct,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberServiceProduct.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberServiceProduct/doJob",payload: payload})
...
const buildTransferModal = (memberServiceProduct,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberServiceProduct,targetComponent)}
...
const internalRenderExtraHeader = (memberServiceProduct) =>{
	return null
}
const internalRenderExtraFooter = (memberServiceProduct) =>{
	return null
}
...
const internalSummaryOf = (memberServiceProduct,targetComponent) =>{
...
<Description term="产品优先级">{memberServiceProduct.productPriority}</Description> 
<Description term="可借书最高总金额">{memberServiceProduct.bookBorrowingLimitPrice}</Description> 
<Description term="最大可借书册数">{memberServiceProduct.bookBorrowingCount}</Description> 
<Description term="购书折扣费率">{memberServiceProduct.bookPurchasingDiscountRate}</Description> 
<Description term="超期天数">{memberServiceProduct.overduePay}</Description> 
<Description term="最长免费借阅天数">{memberServiceProduct.freeBorrowingDays}</Description> 
	
        {buildTransferModal(memberServiceProduct,targetComponent)}
      </DescriptionList>
	)
...
class MemberServiceProductDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"会员服务产品",cardsFor: "memberServiceProduct",cardsSource: this.props.memberServiceProduct,
  		subItems: [
{name: 'memberServiceBundleSkuList', displayName:'会员服务包',type:'memberServiceBundleSku',count:memberServiceBundleSkuCount,addFunction: true},
{name: 'storeSlideList', displayName:'网点海报',type:'storeSlide',count:storeSlideCount,addFunction: true},
{name: 'customerList', displayName:'用户',type:'customer',count:customerCount,addFunction: false},
...
}))(Form.create()(MemberServiceProductDashboard))
