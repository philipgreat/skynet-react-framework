...
const displayColumns = [
  { title: 'ID', debugtype: 'string', dataIndex: 'id', width: '20', render: (text, record)=>(<Link to={`/memberServiceRevenue/${text}/dashboard`}>{text}</Link>) },
  { title: '会员', dataIndex: 'member', render: (text, record) => (record.member ? record.member.displayName : '暂无') },
  { title: '会员昵称', debugtype: 'string', dataIndex: 'memberName', width: '11' },
  ...
  { title: '平台服务收益百分比', debugtype: 'string', dataIndex: 'platformServiceRevenueRate', width: '7' },
  { title: '平台服务收益', dataIndex: 'platformServiceRevenue', className:'money', render: (text, record) => (`￥${text.toFixed(2)}`) },
...
const fieldLabels = {
  ...
  platformServiceRevenueRate: '平台服务收益百分比',
  platformServiceRevenue: '平台服务收益',
