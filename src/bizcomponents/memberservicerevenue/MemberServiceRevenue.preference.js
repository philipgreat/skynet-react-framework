...
import styles from './MemberServiceRevenue.preference.less'
...
const internalImageListOf = (memberServiceRevenue) =>{
...
const internalSettingListOf = (memberServiceRevenue) =>{
...
const internalLargeTextOf = (memberServiceRevenue) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberServiceRevenueService[candidateServiceName] 
...
const executeTrans = (memberServiceRevenue,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberServiceRevenue.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberServiceRevenue/doJob",payload: payload})
...
const buildTransferModal = (memberServiceRevenue,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberServiceRevenue,targetComponent)}
...
const internalRenderExtraHeader = (memberServiceRevenue) =>{
	return null
}
const internalRenderExtraFooter = (memberServiceRevenue) =>{
	return null
}
...
const internalSummaryOf = (memberServiceRevenue,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{memberServiceRevenue.id}</Description> 
<Description term="会员">{memberServiceRevenue.member==null?"未分配":memberServiceRevenue.member.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会员","customer","requestCandidateMember",
	      "transferToAnotherMember","anotherMemberId",memberServiceRevenue.member?memberServiceRevenue.member.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="会员昵称">{memberServiceRevenue.memberName}</Description> 
<Description term="服务开始日期">{ moment(memberServiceRevenue.serviceStartDate).format('YYYY-MM-DD')}</Description> 
<Description term="服务结束日期">{ moment(memberServiceRevenue.serviceEndDate).format('YYYY-MM-DD')}</Description> 
<Description term="当月会员服务费">{memberServiceRevenue.monthlyServiceFee}</Description> 
<Description term="服务网点">{memberServiceRevenue.store==null?"未分配":memberServiceRevenue.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务网点","store","requestCandidateStore",
	      "transferToAnotherStore","anotherStoreId",memberServiceRevenue.store?memberServiceRevenue.store.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="网点名字">{memberServiceRevenue.storeName}</Description> 
<Description term="网点服务次数">{memberServiceRevenue.storeServiceCount}</Description> 
...
class MemberServiceRevenuePreference extends Component {
...
export default connect(state => ({
  memberServiceRevenue: state._memberServiceRevenue,
}))(Form.create()(MemberServiceRevenuePreference))
