...
class MemberServiceRevenueSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberServiceRevenueIds = selectedRows.map((item) => { return item.id })
    console.log('memberServiceRevenueIds', memberServiceRevenueIds)
    const parameters = { memberServiceRevenueIds }
    dispatch({
      type: `${owner.type}/removeMemberServiceRevenueList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceRevenue' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceRevenue', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员服务收益列表' } })
  ...
  render() {
    ...
    const {MemberServiceRevenueTable} = GlobalComponents;
    const {MemberServiceRevenueModalTable} = GlobalComponents;
    const {MemberServiceRevenueSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberServiceRevenueModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员服务收益列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberServiceRevenueSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberServiceRevenueSearch)
