...
import styles from './MessageTemplate.preference.less'
...
const internalImageListOf = (messageTemplate) =>{
...
const internalSettingListOf = (messageTemplate) =>{
...
const internalLargeTextOf = (messageTemplate) =>{

	return(<div> 
   <Card title={`模板`} ><pre>{messageTemplate.template}</pre></Card>
</div>)
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {MessageTemplateService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = MessageTemplateService[candidateServiceName] 
...
const executeTrans = (messageTemplate,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_messageTemplate/doJob",payload: payload})
...
const buildTransferModal = (messageTemplate,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(messageTemplate,targetComponent)}
...
const internalRenderExtraHeader = (messageTemplate) =>{
	return null
}
const internalRenderExtraFooter = (messageTemplate) =>{
	return null
}
...
const internalSummaryOf = (messageTemplate,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{messageTemplate.id}</Description> 
<Description term="名称">{messageTemplate.name}</Description> 
<Description term="更新人">{messageTemplate.updatedBy==null?"未分配":messageTemplate.updatedBy.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"更新人","employee","requestCandidateUpdatedBy",
	      "transferToAnotherUpdatedBy","anotherUpdatedById",messageTemplate.updatedBy?messageTemplate.updatedBy.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="更新时间">{ moment(messageTemplate.updateTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(messageTemplate,targetComponent)}
      </DescriptionList>
	)
...
class MessageTemplatePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.messageTemplate
    const cardsData = {cardsName:"消息模板",cardsFor: "messageTemplate",cardsSource: this.props.messageTemplate,
  		subItems: [
...
export default connect(state => ({
  messageTemplate: state._messageTemplate,
}))(Form.create()(MessageTemplatePreference))
