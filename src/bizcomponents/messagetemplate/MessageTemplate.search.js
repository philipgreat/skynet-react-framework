...
class MessageTemplateSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  render() {
    ...
    const {MessageTemplateTable} = GlobalComponents;
    const {MessageTemplateModalTable} = GlobalComponents;
    const {MessageTemplateSearchForm} = GlobalComponents;
...
<Menu.Item key="updatedBy">关联更新人</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MessageTemplateModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
...
export default Form.create()(MessageTemplateSearch)
