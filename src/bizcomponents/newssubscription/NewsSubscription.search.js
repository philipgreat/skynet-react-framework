...
class NewsSubscriptionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const newsSubscriptionIds = selectedRows.map((item) => { return item.id })
    console.log('newsSubscriptionIds', newsSubscriptionIds)
    const parameters = { newsSubscriptionIds }
    dispatch({
      type: `${owner.type}/removeNewsSubscriptionList`,
      payload: { id: owner.id, type: 'newsSubscription', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'newsSubscription' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'newsSubscription', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'新闻订阅列表' } })
  ...
  render() {
    ...
    const {NewsSubscriptionTable} = GlobalComponents;
    const {NewsSubscriptionConfirmationTable} = GlobalComponents;
    const {NewsSubscriptionSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <NewsSubscriptionConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 新闻订阅列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <NewsSubscriptionSearchForm {...this.props} />
            </div>
...
export default Form.create()(NewsSubscriptionSearch)
