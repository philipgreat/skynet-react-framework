...
import styles from './OrderDiscountCoupon.preference.less'
...
const internalImageListOf = (orderDiscountCoupon) =>{
...
const internalSettingListOf = (orderDiscountCoupon) =>{
...
const internalLargeTextOf = (orderDiscountCoupon) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OrderDiscountCouponService[candidateServiceName] 
...
const executeTrans = (orderDiscountCoupon,targetComponent) =>{
...
const buildTransferModal = (orderDiscountCoupon,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(orderDiscountCoupon,targetComponent)}
...
const internalRenderExtraHeader = (orderDiscountCoupon) =>{
	return null
}
const internalRenderExtraFooter = (orderDiscountCoupon) =>{
	return null
}
...
const internalSummaryOf = (orderDiscountCoupon,targetComponent) =>{
...
<Description term="客户">{orderDiscountCoupon.customer==null?"未分配":orderDiscountCoupon.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"客户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",orderDiscountCoupon.customer?orderDiscountCoupon.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="年检订单">{orderDiscountCoupon.mainOrder==null?"未分配":orderDiscountCoupon.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",orderDiscountCoupon.mainOrder?orderDiscountCoupon.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(orderDiscountCoupon,targetComponent)}
      </DescriptionList>
	)
...
class OrderDiscountCouponPreference extends Component {
...
}))(Form.create()(OrderDiscountCouponPreference))
