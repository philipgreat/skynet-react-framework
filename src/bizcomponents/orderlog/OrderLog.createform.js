...
import styles from './OrderLog.createform.less'
import {mapBackToImageValues, mapFromImageValues} from '../../axios/tools'
import GlobalComponents from '../../custcomponents';
import OrderLogBase from './OrderLog.base'
...
const testValues = {
  title: '提交订单',
  description: '您的‘VIP会员’有效期已延长至2018-12-31',
...
class OrderLogCreateForm extends Component {
  state = {
  ...
  executeCandidateMainOrderSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = OrderLogService.requestCandidateMainOrder("mainOrder", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = OrderLogBase
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addOrderLog`,
          payload: { id: owner.id, type: 'orderLog', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'orderLog',listName:'Log列表' },
      })
    }
...
}))(Form.create()(OrderLogCreateForm))
