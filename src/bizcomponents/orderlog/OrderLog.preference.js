...
import styles from './OrderLog.preference.less'
...
const internalImageListOf = (orderLog) =>{
...
const internalSettingListOf = (orderLog) =>{
...
const internalLargeTextOf = (orderLog) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {OrderLogService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = OrderLogService[candidateServiceName] 
...
const executeTrans = (orderLog,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_orderLog/doJob",payload: payload})
...
const buildTransferModal = (orderLog,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(orderLog,targetComponent)}
...
const internalRenderExtraHeader = (orderLog) =>{
	return null
}
const internalRenderExtraFooter = (orderLog) =>{
	return null
}
...
const internalSummaryOf = (orderLog,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{orderLog.id}</Description> 
<Description term="标题">{orderLog.title}</Description> 
<Description term="描述">{orderLog.description}</Description> 
<Description term="最后更新时间">{ moment(orderLog.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
<Description term="主订单">{orderLog.mainOrder==null?"未分配":orderLog.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"主订单","mainOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",orderLog.mainOrder?orderLog.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(orderLog,targetComponent)}
      </DescriptionList>
	)
...
class OrderLogPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.orderLog
    const cardsData = {cardsName:"Log",cardsFor: "orderLog",cardsSource: this.props.orderLog,
  		subItems: [
...
export default connect(state => ({
  orderLog: state._orderLog,
}))(Form.create()(OrderLogPreference))
