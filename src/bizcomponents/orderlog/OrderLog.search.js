...
import styles from './OrderLog.search.less'
...
class OrderLogSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const orderLogIds = selectedRows.map((item) => { return item.id })
    console.log('orderLogIds', orderLogIds)
    const parameters = { orderLogIds }
    dispatch({
      type: `${owner.type}/removeOrderLogList`,
      payload: { id: owner.id, type: 'orderLog', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderLog' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderLog', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'Log列表' } })
  ...
  render() {
    ...
    const {OrderLogTable} = GlobalComponents;
    const {OrderLogModalTable} = GlobalComponents;
    const {OrderLogSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OrderLogModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: Log列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OrderLogSearchForm {...this.props} />
            </div>
...
export default Form.create()(OrderLogSearch)
