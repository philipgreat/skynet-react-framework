...
import styles from './OrderRatingResult.preference.less'
...
const internalImageListOf = (orderRatingResult) =>{
...
const internalSettingListOf = (orderRatingResult) =>{
...
const internalLargeTextOf = (orderRatingResult) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OrderRatingResultService[candidateServiceName] 
...
const executeTrans = (orderRatingResult,targetComponent) =>{
...
const buildTransferModal = (orderRatingResult,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(orderRatingResult,targetComponent)}
...
const internalRenderExtraHeader = (orderRatingResult) =>{
	return null
}
const internalRenderExtraFooter = (orderRatingResult) =>{
	return null
}
...
const internalSummaryOf = (orderRatingResult,targetComponent) =>{
...
<Description term="年检订单">{orderRatingResult.mainOrder==null?"未分配":orderRatingResult.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",orderRatingResult.mainOrder?orderRatingResult.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(orderRatingResult,targetComponent)}
      </DescriptionList>
	)
...
class OrderRatingResultPreference extends Component {
...
}))(Form.create()(OrderRatingResultPreference))
