...
import styles from './OrderReviewResult.preference.less'
...
const internalImageListOf = (orderReviewResult) =>{
...
const internalSettingListOf = (orderReviewResult) =>{
...
const internalLargeTextOf = (orderReviewResult) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OrderReviewResultService[candidateServiceName] 
...
const executeTrans = (orderReviewResult,targetComponent) =>{
...
const buildTransferModal = (orderReviewResult,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(orderReviewResult,targetComponent)}
...
const internalRenderExtraHeader = (orderReviewResult) =>{
	return null
}
const internalRenderExtraFooter = (orderReviewResult) =>{
	return null
}
...
const internalSummaryOf = (orderReviewResult,targetComponent) =>{
...
<Description term="年检订单">{orderReviewResult.mainOrder==null?"未分配":orderReviewResult.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",orderReviewResult.mainOrder?orderReviewResult.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(orderReviewResult,targetComponent)}
      </DescriptionList>
	)
...
class OrderReviewResultPreference extends Component {
...
}))(Form.create()(OrderReviewResultPreference))
