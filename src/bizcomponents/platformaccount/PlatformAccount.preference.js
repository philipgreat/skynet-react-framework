...
import styles from './PlatformAccount.preference.less'
...
const internalImageListOf = (platformAccount) =>{
...
const internalSettingListOf = (platformAccount) =>{
...
const internalLargeTextOf = (platformAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PlatformAccountService[candidateServiceName] 
...
const executeTrans = (platformAccount,targetComponent) =>{
...
const buildTransferModal = (platformAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(platformAccount,targetComponent)}
...
const internalRenderExtraHeader = (platformAccount) =>{
	return null
}
const internalRenderExtraFooter = (platformAccount) =>{
	return null
}
...
const internalSummaryOf = (platformAccount,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{platformAccount.id}</Description> 
<Description term="名称">{platformAccount.name}</Description> 
<Description term="账户余额">{platformAccount.amount}</Description> 
<Description term="帐户数据">{platformAccount.accountData==null?"未分配":platformAccount.accountData.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"帐户数据","accountData","requestCandidateAccountData",
	      "transferToAnotherAccountData","anotherAccountDataId",platformAccount.accountData?platformAccount.accountData.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(platformAccount,targetComponent)}
      </DescriptionList>
	)
...
class PlatformAccountPreference extends Component {
...
}))(Form.create()(PlatformAccountPreference))
