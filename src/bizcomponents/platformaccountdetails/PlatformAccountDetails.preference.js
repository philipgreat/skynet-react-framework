...
import styles from './PlatformAccountDetails.preference.less'
...
const internalImageListOf = (platformAccountDetails) =>{
...
const internalSettingListOf = (platformAccountDetails) =>{
...
const internalLargeTextOf = (platformAccountDetails) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PlatformAccountDetailsService[candidateServiceName] 
...
const executeTrans = (platformAccountDetails,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=platformAccountDetails.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_platformAccountDetails/doJob",payload: payload})
...
const buildTransferModal = (platformAccountDetails,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(platformAccountDetails,targetComponent)}
...
const internalRenderExtraHeader = (platformAccountDetails) =>{
	return null
}
const internalRenderExtraFooter = (platformAccountDetails) =>{
	return null
}
...
const internalSummaryOf = (platformAccountDetails,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{platformAccountDetails.id}</Description> 
<Description term="摘要">{platformAccountDetails.summary}</Description> 
<Description term="账户余额">{platformAccountDetails.amount}</Description> 
<Description term="交易类型">{platformAccountDetails.transactionType==null?"未分配":platformAccountDetails.transactionType.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"交易类型","transactionType","requestCandidateTransactionType",
	      "transferToAnotherTransactionType","anotherTransactionTypeId",platformAccountDetails.transactionType?platformAccountDetails.transactionType.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="平台账户">{platformAccountDetails.platformAccount==null?"未分配":platformAccountDetails.platformAccount.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"平台账户","platformAccount","requestCandidatePlatformAccount",
	      "transferToAnotherPlatformAccount","anotherPlatformAccountId",platformAccountDetails.platformAccount?platformAccountDetails.platformAccount.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="相关订单">{platformAccountDetails.relatedMainOrder==null?"未分配":platformAccountDetails.relatedMainOrder.displayName}
 <Icon type="swap" onClick={()=>
...
class PlatformAccountDetailsPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"平台账户明细",cardsFor: "platformAccountDetails",cardsSource: this.props.platformAccountDetails,
  		subItems: [
...
}))(Form.create()(PlatformAccountDetailsPreference))
