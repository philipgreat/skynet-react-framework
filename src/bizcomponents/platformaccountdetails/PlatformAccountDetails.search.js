...
class PlatformAccountDetailsSearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'平台账户明细列表' } })
  ...
  render() {
    ...
    const {PlatformAccountDetailsModalTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PlatformAccountDetailsModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 平台账户明细列表`}>
        <Card bordered={false}>
