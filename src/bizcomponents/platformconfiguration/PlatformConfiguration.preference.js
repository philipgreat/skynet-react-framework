...
import styles from './PlatformConfiguration.preference.less'
...
const internalImageListOf = (platformConfiguration) =>{
...
const internalSettingListOf = (platformConfiguration) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PlatformConfigurationService[candidateServiceName] 
...
const executeTrans = (platformConfiguration,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=platformConfiguration.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_platformConfiguration/doJob",payload: payload})
...
const buildTransferModal = (platformConfiguration,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(platformConfiguration,targetComponent)}
...
const internalRenderExtraHeader = (platformConfiguration) =>{
	return null
}
const internalRenderExtraFooter = (platformConfiguration) =>{
	return null
}
...
const internalSummaryOf = (platformConfiguration,targetComponent) =>{
...
class PlatformConfigurationPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.platformConfiguration
    const cardsData = {cardsName:"平台配置",cardsFor: "platformConfiguration",cardsSource: this.props.platformConfiguration,
  		subItems: [
...
export default connect(state => ({
  platformConfiguration: state._platformConfiguration,
}))(Form.create()(PlatformConfigurationPreference))
