...
class PlatformConfigurationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const platformConfigurationIds = selectedRows.map((item) => { return item.id })
    console.log('platformConfigurationIds', platformConfigurationIds)
    const parameters = { platformConfigurationIds }
    dispatch({
      type: `${owner.type}/removePlatformConfigurationList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'platformConfiguration' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'platformConfiguration', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'平台配置列表' } })
  ...
  render() {
    ...
    const {PlatformConfigurationTable} = GlobalComponents;
    const {PlatformConfigurationModalTable} = GlobalComponents;
    const {PlatformConfigurationSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PlatformConfigurationModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 平台配置列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PlatformConfigurationSearchForm {...this.props} />
            </div>
...
export default Form.create()(PlatformConfigurationSearch)
