...
import styles from './PreorderPromotion.preference.less'
...
const internalImageListOf = (preorderPromotion) =>{
...
const internalSettingListOf = (preorderPromotion) =>{
...
const internalLargeTextOf = (preorderPromotion) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PreorderPromotionService[candidateServiceName] 
...
const executeTrans = (preorderPromotion,targetComponent) =>{
...
const buildTransferModal = (preorderPromotion,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(preorderPromotion,targetComponent)}
...
const internalRenderExtraHeader = (preorderPromotion) =>{
	return null
}
const internalRenderExtraFooter = (preorderPromotion) =>{
	return null
}
...
const internalSummaryOf = (preorderPromotion,targetComponent) =>{
...
<Description term="产品名称">{preorderPromotion.product==null?"未分配":preorderPromotion.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品名称","availableProduct","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",preorderPromotion.product?preorderPromotion.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(preorderPromotion,targetComponent)}
      </DescriptionList>
	)
...
class PreorderPromotionPreference extends Component {
...
}))(Form.create()(PreorderPromotionPreference))
