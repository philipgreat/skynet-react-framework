...
const internalImageListOf = (printer) =>{
...
const internalSettingListOf = (printer) =>{
...
const internalLargeTextOf = (printer) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PrinterService[candidateServiceName] 
...
const executeTrans = (printer,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_printer/doJob",payload: payload})
...
const buildTransferModal = (printer,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(printer,targetComponent)}
...
const internalRenderExtraHeader = (printer) =>{
	return null
}
const internalRenderExtraFooter = (printer) =>{
	return null
}
...
const internalSummaryOf = (printer,targetComponent) =>{
...
<Description term="服务网点">{printer.store==null?"未分配":printer.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务网点","store","requestCandidateStore",
	      "transferToAnotherStore","anotherStoreId",printer.store?printer.store.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(printer,targetComponent)}
      </DescriptionList>
	)
...
class PrinterDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, printerTaskAccessiable, printerTaskCount } = this.props.printer
    const cardsData = {cardsName:"打印机",cardsFor: "printer",cardsSource: this.props.printer,
  		subItems: [
{name: 'printerTaskList', displayName:'打印机任务',type:'printerTask',count:printerTaskCount,addFunction: true, role: 'printerTask', accessiable: printerTaskAccessiable},
