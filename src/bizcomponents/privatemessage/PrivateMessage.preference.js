...
import styles from './PrivateMessage.preference.less'
...
const internalImageListOf = (privateMessage) =>{
...
const internalSettingListOf = (privateMessage) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PrivateMessageService[candidateServiceName] 
...
const executeTrans = (privateMessage,targetComponent) =>{
...
const buildTransferModal = (privateMessage,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(privateMessage,targetComponent)}
...
const internalRenderExtraHeader = (privateMessage) =>{
	return null
}
const internalRenderExtraFooter = (privateMessage) =>{
	return null
}
...
<Description term="名称">{privateMessage.name}</Description> 
<Description term="接收人">{privateMessage.deliveryTo==null?"未分配":privateMessage.deliveryTo.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"接收人","customer","requestCandidateDeliveryTo",
	      "transferToAnotherDeliveryTo","anotherDeliveryToId",privateMessage.deliveryTo?privateMessage.deliveryTo.id:"")} 
...
class PrivateMessagePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"私信消息",cardsFor: "privateMessage",cardsSource: this.props.privateMessage,
  		subItems: [
...
}))(Form.create()(PrivateMessagePreference))
