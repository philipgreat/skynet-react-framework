...
import styles from './PrivateMessage.search.less'
...
class PrivateMessageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const privateMessageIds = selectedRows.map((item) => { return item.id })
    console.log('privateMessageIds', privateMessageIds)
    const parameters = { privateMessageIds }
    dispatch({
      type: `${owner.type}/removePrivateMessageList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'privateMessage' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'privateMessage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'私信消息列表' } })
  ...
  render() {
    ...
    const {PrivateMessageTable} = GlobalComponents;
    const {PrivateMessageModalTable} = GlobalComponents;
    const {PrivateMessageSearchForm} = GlobalComponents;
...
<Menu.Item key="deliveryTo">关联接收人</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PrivateMessageModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 私信消息列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PrivateMessageSearchForm {...this.props} />
            </div>
...
export default Form.create()(PrivateMessageSearch)
