...
import styles from './ProductPrice.preference.less'
...
const internalImageListOf = (productPrice) =>{
...
const internalSettingListOf = (productPrice) =>{

	const optionList = [ 
	  {"title":'新能源车',"value":productPrice.greenVehicle,"parameterName":"greenVehicle"},
]
...
const internalLargeTextOf = (productPrice) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProductPriceService[candidateServiceName] 
...
const executeTrans = (productPrice,targetComponent) =>{
...
const buildTransferModal = (productPrice,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(productPrice,targetComponent)}
...
const internalRenderExtraHeader = (productPrice) =>{
	return null
}
const internalRenderExtraFooter = (productPrice) =>{
	return null
}
...
const internalSummaryOf = (productPrice,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{productPrice.id}</Description> 
<Description term="产品名称">{productPrice.product==null?"未分配":productPrice.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品名称","availableProduct","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",productPrice.product?productPrice.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="城市">{productPrice.city==null?"未分配":productPrice.city.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"城市","city","requestCandidateCity",
	      "transferToAnotherCity","anotherCityId",productPrice.city?productPrice.city.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="车辆类型">{productPrice.vehicleType}</Description> 
<Description term="年检费用">{productPrice.inspectionPrice}</Description> 
<Description term="尾气检测费">{productPrice.exhaustDetectionPrice}</Description> 
...
<Description term="描述">{productPrice.description}</Description> 
        ...
        {buildTransferModal(productPrice,targetComponent)}
      </DescriptionList>
	)
...
class ProductPricePreference extends Component {
...
}))(Form.create()(ProductPricePreference))
