...
import styles from './ProfitDistributeState.dashboard.less'
...
const internalImageListOf = (profitDistributeState) =>{
...
const internalSettingListOf = (profitDistributeState) =>{
...
const internalLargeTextOf = (profitDistributeState) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProfitDistributeStateService[candidateServiceName] 
...
const executeTrans = (profitDistributeState,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=profitDistributeState.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_profitDistributeState/doJob",payload: payload})
...
const buildTransferModal = (profitDistributeState,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(profitDistributeState,targetComponent)}
...
const internalRenderExtraHeader = (profitDistributeState) =>{
	return null
}
const internalRenderExtraFooter = (profitDistributeState) =>{
	return null
}
...
const internalSummaryOf = (profitDistributeState,targetComponent) =>{
...
class ProfitDistributeStateDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, undistributedProfitAccessiable, undistributedProfitCount } = this.props.profitDistributeState
...
}))(Form.create()(ProfitDistributeStateDashboard))
