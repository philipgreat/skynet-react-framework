...
import styles from './ProfitDistributeState.search.less'
...
class ProfitDistributeStateSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const profitDistributeStateIds = selectedRows.map((item) => { return item.id })
    console.log('profitDistributeStateIds', profitDistributeStateIds)
    const parameters = { profitDistributeStateIds }
    dispatch({
      type: `${owner.type}/removeProfitDistributeStateList`,
      payload: { id: owner.id, type: 'profitDistributeState', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'profitDistributeState' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'profitDistributeState', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'利润分配状态列表' } })
  ...
  render() {
    ...
    const {ProfitDistributeStateTable} = GlobalComponents;
    const {ProfitDistributeStateModalTable} = GlobalComponents;
    const {ProfitDistributeStateSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProfitDistributeStateModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 利润分配状态列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ProfitDistributeStateSearchForm {...this.props} />
            </div>
...
export default Form.create()(ProfitDistributeStateSearch)
