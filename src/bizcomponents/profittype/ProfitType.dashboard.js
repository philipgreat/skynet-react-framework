...
import styles from './ProfitType.dashboard.less'
...
const internalImageListOf = (profitType) =>{
...
const internalSettingListOf = (profitType) =>{
...
const internalLargeTextOf = (profitType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {ProfitTypeService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = ProfitTypeService[candidateServiceName] 
...
const executeTrans = (profitType,targetComponent) =>{
...
const buildTransferModal = (profitType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(profitType,targetComponent)}
...
const internalRenderExtraHeader = (profitType) =>{
	return null
}
const internalRenderExtraFooter = (profitType) =>{
	return null
}
...
const internalSummaryOf = (profitType,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{profitType.id}</Description> 
<Description term="名称">{profitType.name}</Description> 
<Description term="代码">{profitType.code}</Description> 
	
        {buildTransferModal(profitType,targetComponent)}
      </DescriptionList>
	)
...
class ProfitTypeDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, undistributedProfitAccessiable, undistributedProfitCount } = this.props.profitType
    const cardsData = {cardsName:"利润类型",cardsFor: "profitType",cardsSource: this.props.profitType,
  		subItems: [
...
export default connect(state => ({
  profitType: state._profitType,
}))(Form.create()(ProfitTypeDashboard))
