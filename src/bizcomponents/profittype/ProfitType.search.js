...
import styles from './ProfitType.search.less'
...
class ProfitTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const profitTypeIds = selectedRows.map((item) => { return item.id })
    console.log('profitTypeIds', profitTypeIds)
    const parameters = { profitTypeIds }
    dispatch({
      type: `${owner.type}/removeProfitTypeList`,
      payload: { id: owner.id, type: 'profitType', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'profitType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'profitType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'利润类型列表' } })
  ...
  render() {
    ...
    const {ProfitTypeTable} = GlobalComponents;
    const {ProfitTypeModalTable} = GlobalComponents;
    const {ProfitTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProfitTypeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 利润类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ProfitTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(ProfitTypeSearch)
