...
import styles from './Province.preference.less'
...
const internalImageListOf = (province) =>{
...
const internalSettingListOf = (province) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProvinceService[candidateServiceName] 
...
const executeTrans = (province,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_province/doJob",payload: payload})
...
const buildTransferModal = (province,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(province,targetComponent)}
...
const internalRenderExtraHeader = (province) =>{
	return null
}
const internalRenderExtraFooter = (province) =>{
	return null
}
...
class ProvincePreference extends Component {
...
}))(Form.create()(ProvincePreference))
