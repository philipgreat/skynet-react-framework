...
import styles from './RepairingAllowanceItem.preference.less'
...
const internalImageListOf = (repairingAllowanceItem) =>{
...
const internalSettingListOf = (repairingAllowanceItem) =>{
...
const internalLargeTextOf = (repairingAllowanceItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RepairingAllowanceItemService[candidateServiceName] 
...
const executeTrans = (repairingAllowanceItem,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=repairingAllowanceItem.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_repairingAllowanceItem/doJob",payload: payload})
...
const buildTransferModal = (repairingAllowanceItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(repairingAllowanceItem,targetComponent)}
...
const internalRenderExtraHeader = (repairingAllowanceItem) =>{
	return null
}
const internalRenderExtraFooter = (repairingAllowanceItem) =>{
	return null
}
...
const internalSummaryOf = (repairingAllowanceItem,targetComponent) =>{
...
<Description term="服务">{repairingAllowanceItem.service==null?"未分配":repairingAllowanceItem.service.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务","serviceVehicleRepairing","requestCandidateService",
	      "transferToAnotherService","anotherServiceId",repairingAllowanceItem.service?repairingAllowanceItem.service.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(repairingAllowanceItem,targetComponent)}
      </DescriptionList>
	)
...
class RepairingAllowanceItemPreference extends Component {
...
}))(Form.create()(RepairingAllowanceItemPreference))
