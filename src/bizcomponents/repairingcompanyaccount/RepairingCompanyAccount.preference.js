...
import styles from './RepairingCompanyAccount.preference.less'
...
const internalImageListOf = (repairingCompanyAccount) =>{
...
const internalSettingListOf = (repairingCompanyAccount) =>{
...
const internalLargeTextOf = (repairingCompanyAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RepairingCompanyAccountService[candidateServiceName] 
...
const executeTrans = (repairingCompanyAccount,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=repairingCompanyAccount.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_repairingCompanyAccount/doJob",payload: payload})
...
const buildTransferModal = (repairingCompanyAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(repairingCompanyAccount,targetComponent)}
...
const internalRenderExtraHeader = (repairingCompanyAccount) =>{
	return null
}
const internalRenderExtraFooter = (repairingCompanyAccount) =>{
	return null
}
...
const internalSummaryOf = (repairingCompanyAccount,targetComponent) =>{
...
<Description term="商户">{repairingCompanyAccount.merchant==null?"未分配":repairingCompanyAccount.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",repairingCompanyAccount.merchant?repairingCompanyAccount.merchant.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务人员">{repairingCompanyAccount.responsibleWorker==null?"未分配":repairingCompanyAccount.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",repairingCompanyAccount.responsibleWorker?repairingCompanyAccount.responsibleWorker.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="对账单">{repairingCompanyAccount.account==null?"未分配":repairingCompanyAccount.account.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"对账单","account","requestCandidateAccount",
	      "transferToAnotherAccount","anotherAccountId",repairingCompanyAccount.account?repairingCompanyAccount.account.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(repairingCompanyAccount,targetComponent)}
      </DescriptionList>
	)
...
class RepairingCompanyAccountPreference extends Component {
...
}))(Form.create()(RepairingCompanyAccountPreference))
