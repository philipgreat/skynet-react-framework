...
export default {
  ...
  effects: {
    ...
    *doJob({ payload }, { call, put }) { 
      ...
      if(!RoleService[serviceNameToCall]){
      	handleClientError("找不到后台服务: "+serviceNameToCall)
      	return;
      }
      
      const data = yield call(RoleService[serviceNameToCall], id, parameters)
    ...
    *addEmployeeWorkingStore({ payload }, { call, put }) {
      ...
      const location = { pathname: `/role/${id}/list/\EmployeeWorkingStoreList/员工工作的网点列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateEmployeeWorkingStore({ payload }, { call, put }) {
      ...
      const location = { pathname: `/role/${id}/list/\EmployeeWorkingStoreList/员工工作的网点列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
