...
import styles from './Role.preference.less'
...
const internalImageListOf = (role) =>{
...
const internalSettingListOf = (role) =>{

	const optionList = [ 
	  {"title":'角色Property1',"value":role.roleProperty1,"parameterName":"roleProperty1"},
  {"title":'角色Property2',"value":role.roleProperty2,"parameterName":"roleProperty2"},
]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RoleService[candidateServiceName] 
...
const executeTrans = (role,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_role/doJob",payload: payload})
...
const buildTransferModal = (role,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(role,targetComponent)}
...
const internalRenderExtraHeader = (role) =>{
	return null
}
const internalRenderExtraFooter = (role) =>{
	return null
}
...
class RolePreference extends Component {
