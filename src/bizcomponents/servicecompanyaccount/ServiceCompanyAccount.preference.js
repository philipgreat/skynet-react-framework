...
import styles from './ServiceCompanyAccount.preference.less'
...
const internalImageListOf = (serviceCompanyAccount) =>{
...
const internalSettingListOf = (serviceCompanyAccount) =>{
...
const internalLargeTextOf = (serviceCompanyAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServiceCompanyAccountService[candidateServiceName] 
...
const executeTrans = (serviceCompanyAccount,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=serviceCompanyAccount.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_serviceCompanyAccount/doJob",payload: payload})
...
const buildTransferModal = (serviceCompanyAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(serviceCompanyAccount,targetComponent)}
...
const internalRenderExtraHeader = (serviceCompanyAccount) =>{
	return null
}
const internalRenderExtraFooter = (serviceCompanyAccount) =>{
	return null
}
...
const internalSummaryOf = (serviceCompanyAccount,targetComponent) =>{
...
<Description term="商户">{serviceCompanyAccount.merchant==null?"未分配":serviceCompanyAccount.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",serviceCompanyAccount.merchant?serviceCompanyAccount.merchant.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务人员">{serviceCompanyAccount.responsibleWorker==null?"未分配":serviceCompanyAccount.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",serviceCompanyAccount.responsibleWorker?serviceCompanyAccount.responsibleWorker.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="对账单">{serviceCompanyAccount.account==null?"未分配":serviceCompanyAccount.account.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"对账单","account","requestCandidateAccount",
	      "transferToAnotherAccount","anotherAccountId",serviceCompanyAccount.account?serviceCompanyAccount.account.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(serviceCompanyAccount,targetComponent)}
      </DescriptionList>
	)
...
class ServiceCompanyAccountPreference extends Component {
...
}))(Form.create()(ServiceCompanyAccountPreference))
