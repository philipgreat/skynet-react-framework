...
import styles from './ServiceCompanyAuthenticationInfo.preference.less'
...
const internalImageListOf = (serviceCompanyAuthenticationInfo) =>{
...
const internalSettingListOf = (serviceCompanyAuthenticationInfo) =>{
...
const internalLargeTextOf = (serviceCompanyAuthenticationInfo) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServiceCompanyAuthenticationInfoService[candidateServiceName] 
...
const executeTrans = (serviceCompanyAuthenticationInfo,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=serviceCompanyAuthenticationInfo.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_serviceCompanyAuthenticationInfo/doJob",payload: payload})
...
const buildTransferModal = (serviceCompanyAuthenticationInfo,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(serviceCompanyAuthenticationInfo,targetComponent)}
...
const internalRenderExtraHeader = (serviceCompanyAuthenticationInfo) =>{
	return null
}
const internalRenderExtraFooter = (serviceCompanyAuthenticationInfo) =>{
	return null
}
...
const internalSummaryOf = (serviceCompanyAuthenticationInfo,targetComponent) =>{
...
<Description term="商户">{serviceCompanyAuthenticationInfo.serviceCompany==null?"未分配":serviceCompanyAuthenticationInfo.serviceCompany.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateServiceCompany",
	      "transferToAnotherServiceCompany","anotherServiceCompanyId",serviceCompanyAuthenticationInfo.serviceCompany?serviceCompanyAuthenticationInfo.serviceCompany.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(serviceCompanyAuthenticationInfo,targetComponent)}
      </DescriptionList>
	)
...
class ServiceCompanyAuthenticationInfoPreference extends Component {
...
}))(Form.create()(ServiceCompanyAuthenticationInfoPreference))
