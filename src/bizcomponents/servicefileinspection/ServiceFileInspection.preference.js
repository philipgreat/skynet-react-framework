...
import styles from './ServiceFileInspection.preference.less'
...
const internalImageListOf = (serviceFileInspection) =>{
...
const internalSettingListOf = (serviceFileInspection) =>{
...
const internalLargeTextOf = (serviceFileInspection) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServiceFileInspectionService[candidateServiceName] 
...
const executeTrans = (serviceFileInspection,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=serviceFileInspection.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_serviceFileInspection/doJob",payload: payload})
...
const buildTransferModal = (serviceFileInspection,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(serviceFileInspection,targetComponent)}
...
const internalRenderExtraHeader = (serviceFileInspection) =>{
	return null
}
const internalRenderExtraFooter = (serviceFileInspection) =>{
	return null
}
...
const internalSummaryOf = (serviceFileInspection,targetComponent) =>{
...
<Description term="服务人员">{serviceFileInspection.responsibleWorker==null?"未分配":serviceFileInspection.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",serviceFileInspection.responsibleWorker?serviceFileInspection.responsibleWorker.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务概述">{serviceFileInspection.serviceSummary}</Description> 
<Description term="检测站">{serviceFileInspection.inspectionStation==null?"未分配":serviceFileInspection.inspectionStation.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"检测站","inspectionStation","requestCandidateInspectionStation",
	      "transferToAnotherInspectionStation","anotherInspectionStationId",serviceFileInspection.inspectionStation?serviceFileInspection.inspectionStation.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="商户">{serviceFileInspection.merchant==null?"未分配":serviceFileInspection.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",serviceFileInspection.merchant?serviceFileInspection.merchant.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="年检订单">{serviceFileInspection.mainOrder==null?"未分配":serviceFileInspection.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",serviceFileInspection.mainOrder?serviceFileInspection.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(serviceFileInspection,targetComponent)}
      </DescriptionList>
	)
...
class ServiceFileInspectionPreference extends Component {
...
}))(Form.create()(ServiceFileInspectionPreference))
