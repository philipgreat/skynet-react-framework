...
import styles from './ServiceFileMovementC2m.preference.less'
...
const internalImageListOf = (serviceFileMovementC2m) =>{
...
const internalSettingListOf = (serviceFileMovementC2m) =>{
...
const internalLargeTextOf = (serviceFileMovementC2m) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServiceFileMovementC2mService[candidateServiceName] 
...
const executeTrans = (serviceFileMovementC2m,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=serviceFileMovementC2m.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_serviceFileMovementC2m/doJob",payload: payload})
...
const buildTransferModal = (serviceFileMovementC2m,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(serviceFileMovementC2m,targetComponent)}
...
const internalRenderExtraHeader = (serviceFileMovementC2m) =>{
	return null
}
const internalRenderExtraFooter = (serviceFileMovementC2m) =>{
	return null
}
...
const internalSummaryOf = (serviceFileMovementC2m,targetComponent) =>{
...
<Description term="服务人员">{serviceFileMovementC2m.responsibleWorker==null?"未分配":serviceFileMovementC2m.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",serviceFileMovementC2m.responsibleWorker?serviceFileMovementC2m.responsibleWorker.id:"")} 
...
<Description term="年检订单">{serviceFileMovementC2m.mainOrder==null?"未分配":serviceFileMovementC2m.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",serviceFileMovementC2m.mainOrder?serviceFileMovementC2m.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="商户">{serviceFileMovementC2m.merchant==null?"未分配":serviceFileMovementC2m.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",serviceFileMovementC2m.merchant?serviceFileMovementC2m.merchant.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(serviceFileMovementC2m,targetComponent)}
      </DescriptionList>
	)
...
class ServiceFileMovementC2mPreference extends Component {
...
}))(Form.create()(ServiceFileMovementC2mPreference))
