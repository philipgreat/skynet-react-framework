...
import styles from './ServiceInsuranceForInspection.preference.less'
...
const internalImageListOf = (serviceInsuranceForInspection) =>{
...
const internalSettingListOf = (serviceInsuranceForInspection) =>{
...
const internalLargeTextOf = (serviceInsuranceForInspection) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServiceInsuranceForInspectionService[candidateServiceName] 
...
const executeTrans = (serviceInsuranceForInspection,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=serviceInsuranceForInspection.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_serviceInsuranceForInspection/doJob",payload: payload})
...
const buildTransferModal = (serviceInsuranceForInspection,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(serviceInsuranceForInspection,targetComponent)}
...
const internalRenderExtraHeader = (serviceInsuranceForInspection) =>{
	return null
}
const internalRenderExtraFooter = (serviceInsuranceForInspection) =>{
	return null
}
...
const internalSummaryOf = (serviceInsuranceForInspection,targetComponent) =>{
...
<Description term="选定保险">{serviceInsuranceForInspection.orderedInsurance==null?"未分配":serviceInsuranceForInspection.orderedInsurance.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"选定保险","availableInsurance","requestCandidateOrderedInsurance",
	      "transferToAnotherOrderedInsurance","anotherOrderedInsuranceId",serviceInsuranceForInspection.orderedInsurance?serviceInsuranceForInspection.orderedInsurance.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务人员">{serviceInsuranceForInspection.responsibleWorker==null?"未分配":serviceInsuranceForInspection.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",serviceInsuranceForInspection.responsibleWorker?serviceInsuranceForInspection.responsibleWorker.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="商户">{serviceInsuranceForInspection.merchant==null?"未分配":serviceInsuranceForInspection.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",serviceInsuranceForInspection.merchant?serviceInsuranceForInspection.merchant.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="年检订单">{serviceInsuranceForInspection.mainOrder==null?"未分配":serviceInsuranceForInspection.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",serviceInsuranceForInspection.mainOrder?serviceInsuranceForInspection.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(serviceInsuranceForInspection,targetComponent)}
      </DescriptionList>
	)
...
class ServiceInsuranceForInspectionPreference extends Component {
...
}))(Form.create()(ServiceInsuranceForInspectionPreference))
