...
import styles from './ServiceOrderFilter.preference.less'
...
const internalImageListOf = (serviceOrderFilter) =>{
...
const internalSettingListOf = (serviceOrderFilter) =>{

	const optionList = [ 
	  {"title":'选中',"value":serviceOrderFilter.selected,"parameterName":"selected"},
]
...
const internalLargeTextOf = (serviceOrderFilter) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServiceOrderFilterService[candidateServiceName] 
...
const executeTrans = (serviceOrderFilter,targetComponent) =>{
...
const buildTransferModal = (serviceOrderFilter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(serviceOrderFilter,targetComponent)}
...
const internalRenderExtraHeader = (serviceOrderFilter) =>{
	return null
}
const internalRenderExtraFooter = (serviceOrderFilter) =>{
	return null
}
...
const internalSummaryOf = (serviceOrderFilter,targetComponent) =>{
...
<Description term="员工">{serviceOrderFilter.employee==null?"未分配":serviceOrderFilter.employee.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"员工","vehicleServiceCompanyEmployee","requestCandidateEmployee",
	      "transferToAnotherEmployee","anotherEmployeeId",serviceOrderFilter.employee?serviceOrderFilter.employee.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(serviceOrderFilter,targetComponent)}
      </DescriptionList>
	)
...
class ServiceOrderFilterPreference extends Component {
...
}))(Form.create()(ServiceOrderFilterPreference))
