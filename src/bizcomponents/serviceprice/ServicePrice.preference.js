...
import styles from './ServicePrice.preference.less'
...
const internalImageListOf = (servicePrice) =>{
...
const internalSettingListOf = (servicePrice) =>{

	const optionList = [ 
	  {"title":'是否提供服务',"value":servicePrice.serviceEnabled,"parameterName":"serviceEnabled"},
]
...
const internalLargeTextOf = (servicePrice) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ServicePriceService[candidateServiceName] 
...
const executeTrans = (servicePrice,targetComponent) =>{
...
const buildTransferModal = (servicePrice,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(servicePrice,targetComponent)}
...
const internalRenderExtraHeader = (servicePrice) =>{
	return null
}
const internalRenderExtraFooter = (servicePrice) =>{
	return null
}
...
const internalSummaryOf = (servicePrice,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{servicePrice.id}</Description> 
<Description term="合同">{servicePrice.contract==null?"未分配":servicePrice.contract.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"合同","contract","requestCandidateContract",
	      "transferToAnotherContract","anotherContractId",servicePrice.contract?servicePrice.contract.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务范围">{servicePrice.availableService==null?"未分配":servicePrice.availableService.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务范围","availableService","requestCandidateAvailableService",
	      "transferToAnotherAvailableService","anotherAvailableServiceId",servicePrice.availableService?servicePrice.availableService.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="产品名称">{servicePrice.product==null?"未分配":servicePrice.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品名称","availableProduct","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",servicePrice.product?servicePrice.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="后续服务价格">{servicePrice.otherPriceValue}</Description> 
        ...
        {buildTransferModal(servicePrice,targetComponent)}
      </DescriptionList>
	)
...
class ServicePricePreference extends Component {
...
}))(Form.create()(ServicePricePreference))
