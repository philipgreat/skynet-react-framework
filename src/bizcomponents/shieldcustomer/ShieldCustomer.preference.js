...
import styles from './ShieldCustomer.preference.less'
...
const internalImageListOf = (shieldCustomer) =>{
...
const internalSettingListOf = (shieldCustomer) =>{
...
const internalLargeTextOf = (shieldCustomer) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {ShieldCustomerService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = ShieldCustomerService[candidateServiceName] 
...
const executeTrans = (shieldCustomer,targetComponent) =>{
...
const buildTransferModal = (shieldCustomer,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(shieldCustomer,targetComponent)}
...
const internalRenderExtraHeader = (shieldCustomer) =>{
	return null
}
const internalRenderExtraFooter = (shieldCustomer) =>{
	return null
}
...
const internalSummaryOf = (shieldCustomer,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{shieldCustomer.id}</Description> 
<Description term="用户">{shieldCustomer.customer==null?"未分配":shieldCustomer.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"用户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",shieldCustomer.customer?shieldCustomer.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="屏蔽">{shieldCustomer.shield==null?"未分配":shieldCustomer.shield.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"屏蔽","customer","requestCandidateShield",
	      "transferToAnotherShield","anotherShieldId",shieldCustomer.shield?shieldCustomer.shield.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="创建时间">{ moment(shieldCustomer.creationTime).format('YYYY-MM-DD')}</Description> 
<Description term="评论">{shieldCustomer.comments}</Description> 
	
        {buildTransferModal(shieldCustomer,targetComponent)}
      </DescriptionList>
	)
...
class ShieldCustomerPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.shieldCustomer
    const cardsData = {cardsName:"屏蔽用户",cardsFor: "shieldCustomer",cardsSource: this.props.shieldCustomer,
  		subItems: [
...
export default connect(state => ({
  shieldCustomer: state._shieldCustomer,
}))(Form.create()(ShieldCustomerPreference))
