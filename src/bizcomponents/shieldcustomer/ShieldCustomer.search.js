...
import styles from './ShieldCustomer.search.less'
...
class ShieldCustomerSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const shieldCustomerIds = selectedRows.map((item) => { return item.id })
    console.log('shieldCustomerIds', shieldCustomerIds)
    const parameters = { shieldCustomerIds }
    dispatch({
      type: `${owner.type}/removeShieldCustomerList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'shieldCustomer' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'shieldCustomer', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'屏蔽用户列表' } })
  ...
  render() {
    ...
    const {ShieldCustomerTable} = GlobalComponents;
    const {ShieldCustomerModalTable} = GlobalComponents;
    const {ShieldCustomerSearchForm} = GlobalComponents;
...
<Menu.Item key="shield">关联屏蔽</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ShieldCustomerModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 屏蔽用户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ShieldCustomerSearchForm {...this.props} />
            </div>
...
export default Form.create()(ShieldCustomerSearch)
