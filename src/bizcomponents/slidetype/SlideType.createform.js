...
import styles from './SlideType.createform.less'
...
class SlideTypeCreateForm extends Component {
  state = {
  ...
  executeCandidatePlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = SlideTypeService.requestCandidatePlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const {fieldLabels} = SlideTypeBase
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addSlideType`,
          payload: { id: owner.id, type: 'slideType', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'slideType',listName:'海报类型列表' },
      })
    }
...
}))(Form.create()(SlideTypeCreateForm))
