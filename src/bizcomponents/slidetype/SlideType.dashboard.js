...
import styles from './SlideType.dashboard.less'
...
const internalImageListOf = (slideType) =>{
...
const internalSettingListOf = (slideType) =>{
...
const internalLargeTextOf = (slideType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {SlideTypeService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = SlideTypeService[candidateServiceName] 
...
const executeTrans = (slideType,targetComponent) =>{
...
const buildTransferModal = (slideType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(slideType,targetComponent)}
...
const internalRenderExtraHeader = (slideType) =>{
	return null
}
const internalRenderExtraFooter = (slideType) =>{
	return null
}
...
const internalSummaryOf = (slideType,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{slideType.id}</Description> 
<Description term="名称">{slideType.name}</Description> 
<Description term="代码">{slideType.code}</Description> 
	
        {buildTransferModal(slideType,targetComponent)}
      </DescriptionList>
	)
...
class SlideTypeDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, storeSlideCount } = this.props.slideType
    const cardsData = {cardsName:"海报类型",cardsFor: "slideType",cardsSource: this.props.slideType,
  		subItems: [
...
export default connect(state => ({
  slideType: state._slideType,
}))(Form.create()(SlideTypeDashboard))
