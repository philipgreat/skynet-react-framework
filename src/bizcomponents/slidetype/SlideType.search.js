...
class SlideTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const slideTypeIds = selectedRows.map((item) => { return item.id })
    console.log('slideTypeIds', slideTypeIds)
    const parameters = { slideTypeIds }
    dispatch({
      type: `${owner.type}/removeSlideTypeList`,
      payload: { id: owner.id, type: 'slideType', parameters },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'slideType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'海报类型列表' } })
  ...
  render() {
    ...
    const {SlideTypeTable} = GlobalComponents;
    const {SlideTypeModalTable} = GlobalComponents;
    const {SlideTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <SlideTypeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 海报类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <SlideTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(SlideTypeSearch)
