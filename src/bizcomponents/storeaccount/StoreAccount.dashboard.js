...
const internalImageListOf = (storeAccount) =>{
...
const internalSettingListOf = (storeAccount) =>{
...
const internalLargeTextOf = (storeAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = StoreAccountService[candidateServiceName] 
...
const executeTrans = (storeAccount,targetComponent) =>{
...
const buildTransferModal = (storeAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(storeAccount,targetComponent)}
...
const internalRenderExtraHeader = (storeAccount) =>{
	return null
}
const internalRenderExtraFooter = (storeAccount) =>{
	return null
}
...
const internalSummaryOf = (storeAccount,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{storeAccount.id}</Description> 
<Description term="名称">{storeAccount.name}</Description> 
<Description term="账户余额">{storeAccount.amount}</Description> 
<Description term="服务网点">{storeAccount.store==null?"未分配":storeAccount.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务网点","store","requestCandidateStore",
	      "transferToAnotherStore","anotherStoreId",storeAccount.store?storeAccount.store.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="帐户数据">{storeAccount.accountData==null?"未分配":storeAccount.accountData.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"帐户数据","accountData","requestCandidateAccountData",
	      "transferToAnotherAccountData","anotherAccountDataId",storeAccount.accountData?storeAccount.accountData.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(storeAccount,targetComponent)}
      </DescriptionList>
	)
...
class StoreAccountDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, storeAccountDetailsAccessiable, storeAccountDetailsCount } = this.props.storeAccount
...
}))(Form.create()(StoreAccountDashboard))
