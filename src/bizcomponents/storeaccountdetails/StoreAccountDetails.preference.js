...
import styles from './StoreAccountDetails.preference.less'
...
const internalImageListOf = (storeAccountDetails) =>{
...
const internalSettingListOf = (storeAccountDetails) =>{
...
const internalLargeTextOf = (storeAccountDetails) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = StoreAccountDetailsService[candidateServiceName] 
...
const executeTrans = (storeAccountDetails,targetComponent) =>{
...
const buildTransferModal = (storeAccountDetails,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(storeAccountDetails,targetComponent)}
...
const internalRenderExtraHeader = (storeAccountDetails) =>{
	return null
}
const internalRenderExtraFooter = (storeAccountDetails) =>{
	return null
}
...
const internalSummaryOf = (storeAccountDetails,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{storeAccountDetails.id}</Description> 
<Description term="摘要">{storeAccountDetails.summary}</Description> 
<Description term="账户余额">{storeAccountDetails.amount}</Description> 
<Description term="交易类型">{storeAccountDetails.transactionType==null?"未分配":storeAccountDetails.transactionType.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"交易类型","transactionType","requestCandidateTransactionType",
	      "transferToAnotherTransactionType","anotherTransactionTypeId",storeAccountDetails.transactionType?storeAccountDetails.transactionType.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="网点账户">{storeAccountDetails.storeAccount==null?"未分配":storeAccountDetails.storeAccount.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"网点账户","storeAccount","requestCandidateStoreAccount",
	      "transferToAnotherStoreAccount","anotherStoreAccountId",storeAccountDetails.storeAccount?storeAccountDetails.storeAccount.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="相关订单">{storeAccountDetails.relatedMainOrder==null?"未分配":storeAccountDetails.relatedMainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"相关订单","mainOrder","requestCandidateRelatedMainOrder",
	      "transferToAnotherRelatedMainOrder","anotherRelatedMainOrderId",storeAccountDetails.relatedMainOrder?storeAccountDetails.relatedMainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="日期时间">{ moment(storeAccountDetails.datetime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(storeAccountDetails,targetComponent)}
      </DescriptionList>
	)
...
class StoreAccountDetailsPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"网点账户明细",cardsFor: "storeAccountDetails",cardsSource: this.props.storeAccountDetails,
  		subItems: [
...
}))(Form.create()(StoreAccountDetailsPreference))
