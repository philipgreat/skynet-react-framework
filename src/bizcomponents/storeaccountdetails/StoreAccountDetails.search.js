...
class StoreAccountDetailsSearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'网点账户明细列表' } })
  ...
  render() {
    ...
    const {StoreAccountDetailsModalTable} = GlobalComponents;
...
<Menu.Item key="storeAccount">关联网点账户</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <StoreAccountDetailsModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 网点账户明细列表`}>
        <Card bordered={false}>
