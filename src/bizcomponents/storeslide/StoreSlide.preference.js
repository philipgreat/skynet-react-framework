...
import styles from './StoreSlide.preference.less'
...
const internalImageListOf = (storeSlide) =>{

  const imageList = [
	   {"title":'横幅图像',"imageLocation":storeSlide.bannerImage},
]
...
const internalSettingListOf = (storeSlide) =>{
...
const internalLargeTextOf = (storeSlide) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = StoreSlideService[candidateServiceName] 
...
const executeTrans = (storeSlide,targetComponent) =>{
...
const buildTransferModal = (storeSlide,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(storeSlide,targetComponent)}
...
const internalRenderExtraHeader = (storeSlide) =>{
	return null
}
const internalRenderExtraFooter = (storeSlide) =>{
	return null
}
...
const internalSummaryOf = (storeSlide,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{storeSlide.id}</Description> 
...
<Description term="书">{storeSlide.book==null?"未分配":storeSlide.book.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"书","book","requestCandidateBook",
	      "transferToAnotherBook","anotherBookId",storeSlide.book?storeSlide.book.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="活动">{storeSlide.campaign==null?"未分配":storeSlide.campaign.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"活动","campaign","requestCandidateCampaign",
	      "transferToAnotherCampaign","anotherCampaignId",storeSlide.campaign?storeSlide.campaign.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="会员服务产品">{storeSlide.memberServiceProduct==null?"未分配":storeSlide.memberServiceProduct.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会员服务产品","memberServiceProduct","requestCandidateMemberServiceProduct",
	      "transferToAnotherMemberServiceProduct","anotherMemberServiceProductId",storeSlide.memberServiceProduct?storeSlide.memberServiceProduct.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务网点">{storeSlide.store==null?"未分配":storeSlide.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务网点","store","requestCandidateStore",
	      "transferToAnotherStore","anotherStoreId",storeSlide.store?storeSlide.store.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(storeSlide,targetComponent)}
      </DescriptionList>
	)
...
class StoreSlidePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.storeSlide
    const cardsData = {cardsName:"网点海报",cardsFor: "storeSlide",cardsSource: this.props.storeSlide,
  		subItems: [
...
export default connect(state => ({
  storeSlide: state._storeSlide,
}))(Form.create()(StoreSlidePreference))
