...
import styles from './StoreSlide.search.less'
...
class StoreSlideSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const storeSlideIds = selectedRows.map((item) => { return item.id })
    console.log('storeSlideIds', storeSlideIds)
    const parameters = { storeSlideIds }
    dispatch({
      type: `${owner.type}/removeStoreSlideList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeSlide' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeSlide', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'网点海报列表' } })
  ...
  render() {
    ...
    const {StoreSlideTable} = GlobalComponents;
    const {StoreSlideModalTable} = GlobalComponents;
    const {StoreSlideSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <StoreSlideModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 网点海报列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <StoreSlideSearchForm {...this.props} />
            </div>
...
export default Form.create()(StoreSlideSearch)
