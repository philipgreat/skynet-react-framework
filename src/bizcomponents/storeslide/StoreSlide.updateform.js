...
import styles from './StoreSlide.updateform.less'
...
class StoreSlideUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const storeSlideId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, storeSlideId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateStoreSlide`,
          payload: {
            id: owner.id,
            type: 'storeSlide',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextStoreSlideUpdateRow`,
        payload: {
          id: owner.id,
          type: 'storeSlide',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新网点海报"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新网点海报"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.wxaLinkUrl} {...formItemLayout}>
                  {getFieldDecorator('wxaLinkUrl', {
                    initialValue: selectedRow.wxaLinkUrl,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.antdLinkUrl} {...formItemLayout}>
                  {getFieldDecorator('antdLinkUrl', {
                    initialValue: selectedRow.antdLinkUrl,
...
}))(Form.create()(StoreSlideUpdateForm))
