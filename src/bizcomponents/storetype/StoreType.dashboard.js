...
import styles from './StoreType.dashboard.less'
...
const internalImageListOf = (storeType) =>{
...
const internalSettingListOf = (storeType) =>{
...
const internalLargeTextOf = (storeType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = StoreTypeService[candidateServiceName] 
...
const executeTrans = (storeType,targetComponent) =>{
...
const buildTransferModal = (storeType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(storeType,targetComponent)}
...
const internalRenderExtraHeader = (storeType) =>{
	return null
}
const internalRenderExtraFooter = (storeType) =>{
	return null
}
...
const internalSummaryOf = (storeType,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{storeType.id}</Description> 
<Description term="名称">{storeType.name}</Description> 
<Description term="代码">{storeType.code}</Description> 
	
        {buildTransferModal(storeType,targetComponent)}
      </DescriptionList>
	)
...
class StoreTypeDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, storeCount } = this.props.storeType
    const cardsData = {cardsName:"网点类型",cardsFor: "storeType",cardsSource: this.props.storeType,
  		subItems: [
{name: 'storeList', displayName:'服务网点',type:'store',count:storeCount,addFunction: true},
...
export default connect(state => ({
  storeType: state._storeType,
}))(Form.create()(StoreTypeDashboard))
