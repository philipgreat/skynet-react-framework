...
import styles from './StoreType.search.less'
...
class StoreTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const storeTypeIds = selectedRows.map((item) => { return item.id })
    console.log('storeTypeIds', storeTypeIds)
    const parameters = { storeTypeIds }
    dispatch({
      type: `${owner.type}/removeStoreTypeList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'网点类型列表' } })
  ...
  render() {
    ...
    const {StoreTypeTable} = GlobalComponents;
    const {StoreTypeModalTable} = GlobalComponents;
    const {StoreTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <StoreTypeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 网点类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <StoreTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(StoreTypeSearch)
