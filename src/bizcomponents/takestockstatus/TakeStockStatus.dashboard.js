...
import styles from './TakeStockStatus.dashboard.less'
...
const internalImageListOf = (takeStockStatus) =>{
...
const internalSettingListOf = (takeStockStatus) =>{
...
const internalLargeTextOf = (takeStockStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {TakeStockStatusService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = TakeStockStatusService[candidateServiceName] 
...
const executeTrans = (takeStockStatus,targetComponent) =>{
...
const buildTransferModal = (takeStockStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(takeStockStatus,targetComponent)}
...
const internalRenderExtraHeader = (takeStockStatus) =>{
	return null
}
const internalRenderExtraFooter = (takeStockStatus) =>{
	return null
}
...
const internalSummaryOf = (takeStockStatus,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{takeStockStatus.id}</Description> 
<Description term="名称">{takeStockStatus.name}</Description> 
<Description term="代码">{takeStockStatus.code}</Description> 
	
        {buildTransferModal(takeStockStatus,targetComponent)}
      </DescriptionList>
	)
...
class TakeStockStatusDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, bookTakeStockPlanCount } = this.props.takeStockStatus
    const cardsData = {cardsName:"盘点状态",cardsFor: "takeStockStatus",cardsSource: this.props.takeStockStatus,
  		subItems: [
...
export default connect(state => ({
  takeStockStatus: state._takeStockStatus,
}))(Form.create()(TakeStockStatusDashboard))
