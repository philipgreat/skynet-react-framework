...
import styles from './TakeStockStatus.search.less'
...
class TakeStockStatusSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const takeStockStatusIds = selectedRows.map((item) => { return item.id })
    console.log('takeStockStatusIds', takeStockStatusIds)
    const parameters = { takeStockStatusIds }
    dispatch({
      type: `${owner.type}/removeTakeStockStatusList`,
      payload: { id: owner.id, type: 'takeStockStatus', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'takeStockStatus' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'takeStockStatus', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'盘点状态列表' } })
  ...
  render() {
    ...
    const {TakeStockStatusTable} = GlobalComponents;
    const {TakeStockStatusModalTable} = GlobalComponents;
    const {TakeStockStatusSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TakeStockStatusModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 盘点状态列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TakeStockStatusSearchForm {...this.props} />
            </div>
...
export default Form.create()(TakeStockStatusSearch)
