...
import styles from './TakeStoreResults.dashboard.less'
...
const internalImageListOf = (takeStoreResults) =>{
...
const internalSettingListOf = (takeStoreResults) =>{
...
const internalLargeTextOf = (takeStoreResults) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {TakeStoreResultsService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = TakeStoreResultsService[candidateServiceName] 
...
const executeTrans = (takeStoreResults,targetComponent) =>{
...
const buildTransferModal = (takeStoreResults,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(takeStoreResults,targetComponent)}
...
const internalRenderExtraHeader = (takeStoreResults) =>{
	return null
}
const internalRenderExtraFooter = (takeStoreResults) =>{
	return null
}
...
const internalSummaryOf = (takeStoreResults,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{takeStoreResults.id}</Description> 
<Description term="名称">{takeStoreResults.name}</Description> 
<Description term="代码">{takeStoreResults.code}</Description> 
	
        {buildTransferModal(takeStoreResults,targetComponent)}
      </DescriptionList>
	)
...
class TakeStoreResultsDashboard extends Component {
...
}))(Form.create()(TakeStoreResultsDashboard))
