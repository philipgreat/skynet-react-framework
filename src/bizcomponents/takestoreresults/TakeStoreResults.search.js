...
import styles from './TakeStoreResults.search.less'
...
class TakeStoreResultsSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const takeStoreResultsIds = selectedRows.map((item) => { return item.id })
    console.log('takeStoreResultsIds', takeStoreResultsIds)
    const parameters = { takeStoreResultsIds }
    dispatch({
      type: `${owner.type}/removeTakeStoreResultsList`,
      payload: { id: owner.id, type: 'takeStoreResults', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'takeStoreResults' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'takeStoreResults', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'盘点结果列表' } })
  ...
  render() {
    ...
    const {TakeStoreResultsTable} = GlobalComponents;
    const {TakeStoreResultsModalTable} = GlobalComponents;
    const {TakeStoreResultsSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TakeStoreResultsModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 盘点结果列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TakeStoreResultsSearchForm {...this.props} />
            </div>
...
export default Form.create()(TakeStoreResultsSearch)
