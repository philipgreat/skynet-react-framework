...
const internalImageListOf = (tokenInMemberServiceProduct) =>{
...
const internalSettingListOf = (tokenInMemberServiceProduct) =>{

	const optionList = [ 
	  {"title":'令牌激活',"value":tokenInMemberServiceProduct.tokenEnabled,"parameterName":"tokenEnabled"},
]
...
const internalLargeTextOf = (tokenInMemberServiceProduct) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TokenInMemberServiceProductService[candidateServiceName] 
...
const executeTrans = (tokenInMemberServiceProduct,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=tokenInMemberServiceProduct.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_tokenInMemberServiceProduct/doJob",payload: payload})
...
const buildTransferModal = (tokenInMemberServiceProduct,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(tokenInMemberServiceProduct,targetComponent)}
...
const internalRenderExtraHeader = (tokenInMemberServiceProduct) =>{
	return null
}
const internalRenderExtraFooter = (tokenInMemberServiceProduct) =>{
	return null
}
...
const internalSummaryOf = (tokenInMemberServiceProduct,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{tokenInMemberServiceProduct.id}</Description> 
<Description term="描述">{tokenInMemberServiceProduct.description}</Description> 
<Description term="创建时间">{ moment(tokenInMemberServiceProduct.createTime).format('YYYY-MM-DD')}</Description> 
<Description term="可用的权益">{tokenInMemberServiceProduct.availableToken==null?"未分配":tokenInMemberServiceProduct.availableToken.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"可用的权益","availableToken","requestCandidateAvailableToken",
	      "transferToAnotherAvailableToken","anotherAvailableTokenId",tokenInMemberServiceProduct.availableToken?tokenInMemberServiceProduct.availableToken.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="会员服务产品">{tokenInMemberServiceProduct.memberServiceProduct==null?"未分配":tokenInMemberServiceProduct.memberServiceProduct.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会员服务产品","memberServiceProduct","requestCandidateMemberServiceProduct",
	      "transferToAnotherMemberServiceProduct","anotherMemberServiceProductId",tokenInMemberServiceProduct.memberServiceProduct?tokenInMemberServiceProduct.memberServiceProduct.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(tokenInMemberServiceProduct,targetComponent)}
      </DescriptionList>
	)
...
}))(Form.create()(TokenInMemberServiceProductDashboard))
