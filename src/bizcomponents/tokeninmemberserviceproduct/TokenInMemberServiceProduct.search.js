...
class TokenInMemberServiceProductSearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'成员服务产品中的令牌列表' } })
  ...
  render() {
    ...
    const {TokenInMemberServiceProductModalTable} = GlobalComponents;
...
<Menu.Item key="availableToken">关联可用的权益</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TokenInMemberServiceProductModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 成员服务产品中的令牌列表`}>
        <Card bordered={false}>
