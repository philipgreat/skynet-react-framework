...
const internalImageListOf = (transactionType) =>{
...
const internalSettingListOf = (transactionType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TransactionTypeService[candidateServiceName] 
...
const executeTrans = (transactionType,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_transactionType/doJob",payload: payload})
...
const buildTransferModal = (transactionType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transactionType,targetComponent)}
...
const internalRenderExtraHeader = (transactionType) =>{
	return null
}
const internalRenderExtraFooter = (transactionType) =>{
	return null
}
...
<Description term="帐户数据">{transactionType.accountData==null?"未分配":transactionType.accountData.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"帐户数据","accountData","requestCandidateAccountData",
	      "transferToAnotherAccountData","anotherAccountDataId",transactionType.accountData?transactionType.accountData.id:"")} 
...
class TransactionTypeDashboard extends Component {
  ...
  render() {
    ...
    const { id,displayName, platformAccountDetailsAccessiable, fundationAccountDetailsAccessiable, storeAccountDetailsAccessiable, customerAccountTransactionAccessiable, platformAccountDetailsCount, fundationAccountDetailsCount, storeAccountDetailsCount, customerAccountTransactionCount } = this.props.transactionType
