...
import styles from './TransactionType.search.less'
...
class TransactionTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const transactionTypeIds = selectedRows.map((item) => { return item.id })
    console.log('transactionTypeIds', transactionTypeIds)
    const parameters = { transactionTypeIds }
    dispatch({
      type: `${owner.type}/removeTransactionTypeList`,
      payload: { id: owner.id, type: 'transactionType', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'transactionType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'transactionType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'交易类型列表' } })
  ...
  render() {
    ...
    const {TransactionTypeTable} = GlobalComponents;
    const {TransactionTypeModalTable} = GlobalComponents;
    const {TransactionTypeSearchForm} = GlobalComponents;
...
<Menu.Item key="accountData">关联帐户数据</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TransactionTypeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 交易类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TransactionTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(TransactionTypeSearch)
