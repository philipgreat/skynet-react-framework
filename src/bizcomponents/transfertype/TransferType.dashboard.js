...
import styles from './TransferType.dashboard.less'
...
const internalImageListOf = (transferType) =>{
...
const internalSettingListOf = (transferType) =>{
...
const internalLargeTextOf = (transferType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TransferTypeService[candidateServiceName] 
...
const executeTrans = (transferType,targetComponent) =>{
...
const buildTransferModal = (transferType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transferType,targetComponent)}
...
const internalRenderExtraHeader = (transferType) =>{
	return null
}
const internalRenderExtraFooter = (transferType) =>{
	return null
}
...
const internalSummaryOf = (transferType,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{transferType.id}</Description> 
<Description term="名称">{transferType.name}</Description> 
<Description term="代码">{transferType.code}</Description> 
	
        {buildTransferModal(transferType,targetComponent)}
      </DescriptionList>
	)
...
class TransferTypeDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"转移类型",cardsFor: "transferType",cardsSource: this.props.transferType,
  		subItems: [
{name: 'bookCopyTransferList', displayName:'图书副本迁移记录',type:'bookCopyTransfer',count:bookCopyTransferCount},
...
export default connect(state => ({
  transferType: state._transferType,
}))(Form.create()(TransferTypeDashboard))
