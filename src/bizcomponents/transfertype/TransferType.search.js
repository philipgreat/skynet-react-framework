...
import styles from './TransferType.search.less'
...
class TransferTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const transferTypeIds = selectedRows.map((item) => { return item.id })
    console.log('transferTypeIds', transferTypeIds)
    const parameters = { transferTypeIds }
    dispatch({
      type: `${owner.type}/removeTransferTypeList`,
      payload: { id: owner.id, type: 'transferType', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'transferType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'transferType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'转移类型列表' } })
  ...
  render() {
    ...
    const {TransferTypeTable} = GlobalComponents;
    const {TransferTypeModalTable} = GlobalComponents;
    const {TransferTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TransferTypeModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 转移类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TransferTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(TransferTypeSearch)
