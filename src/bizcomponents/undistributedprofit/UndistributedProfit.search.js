...
import styles from './UndistributedProfit.search.less'
...
class UndistributedProfitSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  render() {
    ...
    const {UndistributedProfitTable} = GlobalComponents;
    const {UndistributedProfitModalTable} = GlobalComponents;
    const {UndistributedProfitSearchForm} = GlobalComponents;
...
<Menu.Item key="profitType">关联利润类型</Menu.Item>
<Menu.Item key="profitDistributeState">关联利润分配状态</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <UndistributedProfitModalTable data={selectedRows} owner={owner} />
        </Modal>)
    }
...
export default Form.create()(UndistributedProfitSearch)
