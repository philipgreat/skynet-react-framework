...
import styles from './VehicleInfo.preference.less'
...
const internalImageListOf = (vehicleInfo) =>{
...
const internalSettingListOf = (vehicleInfo) =>{
...
const internalLargeTextOf = (vehicleInfo) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInfoService[candidateServiceName] 
...
const executeTrans = (vehicleInfo,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInfo/doJob",payload: payload})
...
const buildTransferModal = (vehicleInfo,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInfo,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInfo) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInfo) =>{
	return null
}
...
const internalSummaryOf = (vehicleInfo,targetComponent) =>{
...
<Description term="客户">{vehicleInfo.customer==null?"未分配":vehicleInfo.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"客户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",vehicleInfo.customer?vehicleInfo.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleInfo,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInfoPreference extends Component {
...
}))(Form.create()(VehicleInfoPreference))
