...
import styles from './VehicleInspectionInsuranceOrder.preference.less'
...
const internalImageListOf = (vehicleInspectionInsuranceOrder) =>{
...
const internalSettingListOf = (vehicleInspectionInsuranceOrder) =>{
...
const internalLargeTextOf = (vehicleInspectionInsuranceOrder) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInspectionInsuranceOrderService[candidateServiceName] 
...
const executeTrans = (vehicleInspectionInsuranceOrder,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleInspectionInsuranceOrder.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInspectionInsuranceOrder/doJob",payload: payload})
...
const buildTransferModal = (vehicleInspectionInsuranceOrder,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInspectionInsuranceOrder,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInspectionInsuranceOrder) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInspectionInsuranceOrder) =>{
	return null
}
...
const internalSummaryOf = (vehicleInspectionInsuranceOrder,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{vehicleInspectionInsuranceOrder.id}</Description> 
<Description term="保险产品">{vehicleInspectionInsuranceOrder.insurance==null?"未分配":vehicleInspectionInsuranceOrder.insurance.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"保险产品","availableInsurance","requestCandidateInsurance",
	      "transferToAnotherInsurance","anotherInsuranceId",vehicleInspectionInsuranceOrder.insurance?vehicleInspectionInsuranceOrder.insurance.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="年检订单">{vehicleInspectionInsuranceOrder.mainOrder==null?"未分配":vehicleInspectionInsuranceOrder.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",vehicleInspectionInsuranceOrder.mainOrder?vehicleInspectionInsuranceOrder.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleInspectionInsuranceOrder,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInspectionInsuranceOrderPreference extends Component {
...
}))(Form.create()(VehicleInspectionInsuranceOrderPreference))
