...
import styles from './VehicleInspectionOrderCharge.preference.less'
...
const internalImageListOf = (vehicleInspectionOrderCharge) =>{
...
const internalSettingListOf = (vehicleInspectionOrderCharge) =>{
...
const internalLargeTextOf = (vehicleInspectionOrderCharge) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInspectionOrderChargeService[candidateServiceName] 
...
const executeTrans = (vehicleInspectionOrderCharge,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleInspectionOrderCharge.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInspectionOrderCharge/doJob",payload: payload})
...
const buildTransferModal = (vehicleInspectionOrderCharge,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInspectionOrderCharge,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInspectionOrderCharge) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInspectionOrderCharge) =>{
	return null
}
...
const internalSummaryOf = (vehicleInspectionOrderCharge,targetComponent) =>{
...
<Description term="年检订单">{vehicleInspectionOrderCharge.mainOrder==null?"未分配":vehicleInspectionOrderCharge.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",vehicleInspectionOrderCharge.mainOrder?vehicleInspectionOrderCharge.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleInspectionOrderCharge,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInspectionOrderChargePreference extends Component {
...
}))(Form.create()(VehicleInspectionOrderChargePreference))
