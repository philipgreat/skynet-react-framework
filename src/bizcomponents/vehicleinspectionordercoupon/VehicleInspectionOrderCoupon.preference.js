...
import styles from './VehicleInspectionOrderCoupon.preference.less'
...
const internalImageListOf = (vehicleInspectionOrderCoupon) =>{
...
const internalSettingListOf = (vehicleInspectionOrderCoupon) =>{
...
const internalLargeTextOf = (vehicleInspectionOrderCoupon) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInspectionOrderCouponService[candidateServiceName] 
...
const executeTrans = (vehicleInspectionOrderCoupon,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleInspectionOrderCoupon.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInspectionOrderCoupon/doJob",payload: payload})
...
const buildTransferModal = (vehicleInspectionOrderCoupon,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInspectionOrderCoupon,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInspectionOrderCoupon) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInspectionOrderCoupon) =>{
	return null
}
...
const internalSummaryOf = (vehicleInspectionOrderCoupon,targetComponent) =>{
...
<Description term="客户">{vehicleInspectionOrderCoupon.customer==null?"未分配":vehicleInspectionOrderCoupon.customer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"客户","customer","requestCandidateCustomer",
	      "transferToAnotherCustomer","anotherCustomerId",vehicleInspectionOrderCoupon.customer?vehicleInspectionOrderCoupon.customer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="年检订单">{vehicleInspectionOrderCoupon.mainOrder==null?"未分配":vehicleInspectionOrderCoupon.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",vehicleInspectionOrderCoupon.mainOrder?vehicleInspectionOrderCoupon.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleInspectionOrderCoupon,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInspectionOrderCouponPreference extends Component {
...
}))(Form.create()(VehicleInspectionOrderCouponPreference))
