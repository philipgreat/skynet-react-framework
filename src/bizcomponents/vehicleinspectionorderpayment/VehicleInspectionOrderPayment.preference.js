...
import styles from './VehicleInspectionOrderPayment.preference.less'
...
const internalImageListOf = (vehicleInspectionOrderPayment) =>{
...
const internalSettingListOf = (vehicleInspectionOrderPayment) =>{
...
const internalLargeTextOf = (vehicleInspectionOrderPayment) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInspectionOrderPaymentService[candidateServiceName] 
...
const executeTrans = (vehicleInspectionOrderPayment,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleInspectionOrderPayment.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInspectionOrderPayment/doJob",payload: payload})
...
const buildTransferModal = (vehicleInspectionOrderPayment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInspectionOrderPayment,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInspectionOrderPayment) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInspectionOrderPayment) =>{
	return null
}
...
const internalSummaryOf = (vehicleInspectionOrderPayment,targetComponent) =>{
...
<Description term="年检订单">{vehicleInspectionOrderPayment.mainOrder==null?"未分配":vehicleInspectionOrderPayment.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",vehicleInspectionOrderPayment.mainOrder?vehicleInspectionOrderPayment.mainOrder.id:"")} 
...
<Description term="最后更新时间">{ moment(vehicleInspectionOrderPayment.lastUpdateTime).format('YYYY-MM-DD')}</Description> 
        ...
        {buildTransferModal(vehicleInspectionOrderPayment,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInspectionOrderPaymentPreference extends Component {
...
}))(Form.create()(VehicleInspectionOrderPaymentPreference))
