...
import styles from './VehicleInspectionOrderServiceLog.preference.less'
...
const internalImageListOf = (vehicleInspectionOrderServiceLog) =>{
...
const internalSettingListOf = (vehicleInspectionOrderServiceLog) =>{
...
const internalLargeTextOf = (vehicleInspectionOrderServiceLog) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInspectionOrderServiceLogService[candidateServiceName] 
...
const executeTrans = (vehicleInspectionOrderServiceLog,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleInspectionOrderServiceLog.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInspectionOrderServiceLog/doJob",payload: payload})
...
const buildTransferModal = (vehicleInspectionOrderServiceLog,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInspectionOrderServiceLog,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInspectionOrderServiceLog) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInspectionOrderServiceLog) =>{
	return null
}
...
const internalSummaryOf = (vehicleInspectionOrderServiceLog,targetComponent) =>{
...
<Description term="服务人员">{vehicleInspectionOrderServiceLog.responsibleWorker==null?"未分配":vehicleInspectionOrderServiceLog.responsibleWorker.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务人员","vehicleServiceCompanyEmployee","requestCandidateResponsibleWorker",
	      "transferToAnotherResponsibleWorker","anotherResponsibleWorkerId",vehicleInspectionOrderServiceLog.responsibleWorker?vehicleInspectionOrderServiceLog.responsibleWorker.id:"")} 
...
<Description term="年检订单">{vehicleInspectionOrderServiceLog.mainOrder==null?"未分配":vehicleInspectionOrderServiceLog.mainOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"年检订单","vehicleInspectionOrder","requestCandidateMainOrder",
	      "transferToAnotherMainOrder","anotherMainOrderId",vehicleInspectionOrderServiceLog.mainOrder?vehicleInspectionOrderServiceLog.mainOrder.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleInspectionOrderServiceLog,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInspectionOrderServiceLogPreference extends Component {
...
}))(Form.create()(VehicleInspectionOrderServiceLogPreference))
