...
import styles from './VehicleInspectionPlateNumberPattern.preference.less'
...
const internalImageListOf = (vehicleInspectionPlateNumberPattern) =>{
...
const internalSettingListOf = (vehicleInspectionPlateNumberPattern) =>{
...
const internalLargeTextOf = (vehicleInspectionPlateNumberPattern) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleInspectionPlateNumberPatternService[candidateServiceName] 
...
const executeTrans = (vehicleInspectionPlateNumberPattern,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleInspectionPlateNumberPattern.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleInspectionPlateNumberPattern/doJob",payload: payload})
...
const buildTransferModal = (vehicleInspectionPlateNumberPattern,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleInspectionPlateNumberPattern,targetComponent)}
...
const internalRenderExtraHeader = (vehicleInspectionPlateNumberPattern) =>{
	return null
}
const internalRenderExtraFooter = (vehicleInspectionPlateNumberPattern) =>{
	return null
}
...
const internalSummaryOf = (vehicleInspectionPlateNumberPattern,targetComponent) =>{
...
<Description term="商户">{vehicleInspectionPlateNumberPattern.company==null?"未分配":vehicleInspectionPlateNumberPattern.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",vehicleInspectionPlateNumberPattern.company?vehicleInspectionPlateNumberPattern.company.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleInspectionPlateNumberPattern,targetComponent)}
      </DescriptionList>
	)
...
class VehicleInspectionPlateNumberPatternPreference extends Component {
...
}))(Form.create()(VehicleInspectionPlateNumberPatternPreference))
