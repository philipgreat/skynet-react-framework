...
import styles from './VehicleRepairingAllowance.preference.less'
...
const internalImageListOf = (vehicleRepairingAllowance) =>{
...
const internalSettingListOf = (vehicleRepairingAllowance) =>{
...
const internalLargeTextOf = (vehicleRepairingAllowance) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleRepairingAllowanceService[candidateServiceName] 
...
const executeTrans = (vehicleRepairingAllowance,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleRepairingAllowance.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleRepairingAllowance/doJob",payload: payload})
...
const buildTransferModal = (vehicleRepairingAllowance,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleRepairingAllowance,targetComponent)}
...
const internalRenderExtraHeader = (vehicleRepairingAllowance) =>{
	return null
}
const internalRenderExtraFooter = (vehicleRepairingAllowance) =>{
	return null
}
...
const internalSummaryOf = (vehicleRepairingAllowance,targetComponent) =>{
...
<Description term="服务">{vehicleRepairingAllowance.service==null?"未分配":vehicleRepairingAllowance.service.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务","availableService","requestCandidateService",
	      "transferToAnotherService","anotherServiceId",vehicleRepairingAllowance.service?vehicleRepairingAllowance.service.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleRepairingAllowance,targetComponent)}
      </DescriptionList>
	)
...
class VehicleRepairingAllowancePreference extends Component {
...
}))(Form.create()(VehicleRepairingAllowancePreference))
