...
import styles from './VehicleRepairingPayment.preference.less'
...
const internalImageListOf = (vehicleRepairingPayment) =>{
...
const internalSettingListOf = (vehicleRepairingPayment) =>{
...
const internalLargeTextOf = (vehicleRepairingPayment) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleRepairingPaymentService[candidateServiceName] 
...
const executeTrans = (vehicleRepairingPayment,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleRepairingPayment.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleRepairingPayment/doJob",payload: payload})
...
const buildTransferModal = (vehicleRepairingPayment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleRepairingPayment,targetComponent)}
...
const internalRenderExtraHeader = (vehicleRepairingPayment) =>{
	return null
}
const internalRenderExtraFooter = (vehicleRepairingPayment) =>{
	return null
}
...
const internalSummaryOf = (vehicleRepairingPayment,targetComponent) =>{
...
<Description term="维修服务">{vehicleRepairingPayment.serviceVehicleRepairing==null?"未分配":vehicleRepairingPayment.serviceVehicleRepairing.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"维修服务","serviceVehicleRepairing","requestCandidateServiceVehicleRepairing",
	      "transferToAnotherServiceVehicleRepairing","anotherServiceVehicleRepairingId",vehicleRepairingPayment.serviceVehicleRepairing?vehicleRepairingPayment.serviceVehicleRepairing.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleRepairingPayment,targetComponent)}
      </DescriptionList>
	)
...
class VehicleRepairingPaymentPreference extends Component {
...
}))(Form.create()(VehicleRepairingPaymentPreference))
