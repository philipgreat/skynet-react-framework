...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
      	const modelName = 'vehicleServiceCompanyBusinessScope'
  ...
  effects: {
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._vehicleServiceCompanyBusinessScope)
    ...
    *doJob({ payload }, { call, put }) { 
      ...
      if(!VehicleServiceCompanyBusinessScopeService[serviceNameToCall]){
      	handleClientError("找不到后台服务: "+serviceNameToCall)
      	return;
      }
      
      const data = yield call(VehicleServiceCompanyBusinessScopeService[serviceNameToCall], id, parameters)
