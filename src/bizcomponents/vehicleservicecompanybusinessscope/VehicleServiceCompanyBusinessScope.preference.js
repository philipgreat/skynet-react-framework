...
import styles from './VehicleServiceCompanyBusinessScope.preference.less'
...
const internalImageListOf = (vehicleServiceCompanyBusinessScope) =>{
...
const internalSettingListOf = (vehicleServiceCompanyBusinessScope) =>{

	const optionList = [ 
	  {"title":'可用状态',"value":vehicleServiceCompanyBusinessScope.serviceAvailability,"parameterName":"serviceAvailability"},
]
...
const internalLargeTextOf = (vehicleServiceCompanyBusinessScope) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleServiceCompanyBusinessScopeService[candidateServiceName] 
...
const executeTrans = (vehicleServiceCompanyBusinessScope,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleServiceCompanyBusinessScope.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleServiceCompanyBusinessScope/doJob",payload: payload})
...
const buildTransferModal = (vehicleServiceCompanyBusinessScope,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleServiceCompanyBusinessScope,targetComponent)}
...
const internalRenderExtraHeader = (vehicleServiceCompanyBusinessScope) =>{
	return null
}
const internalRenderExtraFooter = (vehicleServiceCompanyBusinessScope) =>{
	return null
}
...
const internalSummaryOf = (vehicleServiceCompanyBusinessScope,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{vehicleServiceCompanyBusinessScope.id}</Description> 
<Description term="商户">{vehicleServiceCompanyBusinessScope.company==null?"未分配":vehicleServiceCompanyBusinessScope.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",vehicleServiceCompanyBusinessScope.company?vehicleServiceCompanyBusinessScope.company.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="服务范围">{vehicleServiceCompanyBusinessScope.availableService==null?"未分配":vehicleServiceCompanyBusinessScope.availableService.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"服务范围","availableService","requestCandidateAvailableService",
	      "transferToAnotherAvailableService","anotherAvailableServiceId",vehicleServiceCompanyBusinessScope.availableService?vehicleServiceCompanyBusinessScope.availableService.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleServiceCompanyBusinessScope,targetComponent)}
      </DescriptionList>
	)
...
class VehicleServiceCompanyBusinessScopePreference extends Component {
...
}))(Form.create()(VehicleServiceCompanyBusinessScopePreference))
