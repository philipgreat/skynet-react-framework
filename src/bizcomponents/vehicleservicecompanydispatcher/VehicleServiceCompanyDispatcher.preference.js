...
import styles from './VehicleServiceCompanyDispatcher.preference.less'
...
const internalImageListOf = (vehicleServiceCompanyDispatcher) =>{
...
const internalSettingListOf = (vehicleServiceCompanyDispatcher) =>{
...
const internalLargeTextOf = (vehicleServiceCompanyDispatcher) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = VehicleServiceCompanyDispatcherService[candidateServiceName] 
...
const executeTrans = (vehicleServiceCompanyDispatcher,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=vehicleServiceCompanyDispatcher.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_vehicleServiceCompanyDispatcher/doJob",payload: payload})
...
const buildTransferModal = (vehicleServiceCompanyDispatcher,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(vehicleServiceCompanyDispatcher,targetComponent)}
...
const internalRenderExtraHeader = (vehicleServiceCompanyDispatcher) =>{
	return null
}
const internalRenderExtraFooter = (vehicleServiceCompanyDispatcher) =>{
	return null
}
...
const internalSummaryOf = (vehicleServiceCompanyDispatcher,targetComponent) =>{
...
<Description term="商户">{vehicleServiceCompanyDispatcher.company==null?"未分配":vehicleServiceCompanyDispatcher.company.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商户","vehicleServiceCompany","requestCandidateCompany",
	      "transferToAnotherCompany","anotherCompanyId",vehicleServiceCompanyDispatcher.company?vehicleServiceCompanyDispatcher.company.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
	
        {buildTransferModal(vehicleServiceCompanyDispatcher,targetComponent)}
      </DescriptionList>
	)
...
class VehicleServiceCompanyDispatcherPreference extends Component {
...
}))(Form.create()(VehicleServiceCompanyDispatcherPreference))
